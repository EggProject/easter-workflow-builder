...
**Függő az a jóváhagyás, aminek a lépés sora `waiting_approval`, nem az, aminek a `decision` mezője NULL** (user döntés 2026-09-23). A döntés nélkül lezárt jóváhagyás (megszakítás, `fail_run`, a `sub_workflow` fa lezárása, a SPEC-004 5.8 időkorlátja) lépése `cancelled` vagy `failed`, a `decision` mezője viszont NULL marad, mert a lezárás nem emberi döntés; a `decision IS NULL` önmagában tehát a lezártat is függőnek mutatná. A `listPendingApprovals` ezért a `human_approval` sort a `step_run` sorára köti, és egyetlen lekérdezésben a `decision IS NULL` mellett a `step_run.status = 'waiting_approval'` feltétellel szűr, `requested_at_ms` szerint növekvő sorrendben. A lépés sorának lezárása és a lista ugyanabból a sorból olvas, így nincs ablak, amiben a lezárt jóváhagyás a lezárás után még a listában állna. Az azonosító szerinti olvasás (`getApproval`) a döntéstől és a lépés állapotától függetlenül adja a sort: erre épül a döntési végpont hibakódja, `not_found` csak nem létező azonosítóra, a már eldöntött vagy lezárt jóváhagyásra `conflict` (SPEC-005 4.2, C táblázat 18. sor).
...
| `HumanApprovalRepository`       | `requestApproval`, `decideApproval`, `getApproval`, `getApprovalForStep`, `listPendingApprovals`                             | a `decideApproval` és a lépés állapotváltás egy tranzakcióban fut                                                                                                                              |
