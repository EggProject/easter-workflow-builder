...
és lezárja a szabályozót (SPEC-004 10.2 1. pont). Ugyanez a kérés ma `500` választ kap `internal`
kóddal és `engine_shutting_down` hibaosztállyal, futás nem jön létre, és a kilépés 3031 ms. Az
`internal` kód a SPEC-005 8.3 táblázat "minden más" sorából jön (5.5), a `database_closed` sor indokával azonos
okból: a folyamat állapota, nem a kérésé. A regressziót a `run-shutdown-sequence.spec.ts` őrzi,
valódi HTTP kapcsolaton, félbe küldött kéréssel.
