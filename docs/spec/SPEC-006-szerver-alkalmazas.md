...
- **Nem módosítja a `protocol`, az `engine`, a `db` és az `agent` csomag felületét.** A szerver ezekre a meglévő felületekre képez le, egyetlen új séma, egyetlen új motor metódus és egyetlen új repository metódus nélkül. Ahol ez a leképezés ma nem teljes, az a 12. szekcióban áll nyitott kérdésként, nem a felület csendes bővítéseként. **Egy kimondott kivétel, a user 2026-09-23-i döntésével:** a `db` `HumanApprovalRepository` új, azonosító szerinti olvasó metódust kapott (`getApproval`), mert a jóváhagyás döntési végpontja enélkül nem tudta szétválasztani a nem létező azonosítót (`not_found`) a már eldöntött vagy döntés nélkül lezárt jóváhagyástól (`conflict`, SPEC-005 4.2, C táblázat 18. sor és 8.2). A mondat ennyiben felülírva, a többi felületre változatlanul áll.
- **Nem implementálja a felületet.** Az `apps/web` és a `packages/ui` külön spec.
- **Nem dönt életben tartó gyakoriságról, rotációs méretről és retencióról.** Egyikre sincs dokumentált forrásunk, ezért egyikre sem adunk számot; mindegyik a 12. szekcióban áll, kimondott addigi viselkedéssel. A portot és a fejlesztői origint a jelen spec szintén nem döntötte el; azt a **SPEC-008 3. szekciója** zárta le a user termékdöntésével, és az O-1 tétel ott van átvezetve.
...
- **Nem old meg olyan hiányzó rétegképességet, ami külön termékdöntést igényel.** A kifejezés kiértékelő és a sablon renderelő portnak ma nincs megvalósítása a repóban (M-9), a `providers` CRUD nem létezik, és az `expression_evaluator_unavailable` hibaág a SPEC-004 O-1 nyitott kérdése. A szerver ezeket nem találja ki, hanem a 6.6 szekció szerinti, kimondottan elutasító port implementációt adja. **A két port kérdése azóta lezárult:** a SPEC-004 O-1 a user 2026-09-23-i döntésével (Mustache és CEL) zárult, és a [`SPEC-010-sablon-es-kifejezes-nyelv.md`](SPEC-010-sablon-es-kifejezes-nyelv.md) 11.1 szekciója szerint az `engine-assembly` a szállított Mustache és CEL portot köti be, az elutasító portok megszűnnek.
...
**Az M-31, M-32 és M-33 tény nem itt, hanem a 6.5 szekcióban áll**, mert csak ott, az élő szórás tárgyalása mellett érthető, hogy miért döntőek. A számozás folytonos, tehát a spec bizonyítékai `M-1` ... `M-37` azonosítón hivatkozhatók; az M-34 ... M-37 a 2.3 szekció végén áll (2026-09-23, a leállási sorrend, a leállás közbeni futás indítás és a megszakítás javítása).
...
| M-34 | Az `http.Server` `server.close()` hívása _"closes all connections connected to this server which are not sending a request or waiting for a response"_ (v19.0.0 óta), a visszahívása viszont a `net.Server` szerint csak akkor fut, amikor _"all connections are ended"_. Egy nyitott SSE válasz "waiting for a response" állapotú, tehát a `close()` nem zárja, és amíg nyitva van, a visszahívás nem fut le. A telepített Node v26.7.0 `_http_server.js` forrása ugyanezt mutatja: a `close()` csak a `closeIdleConnections()`-t hívja, ami a befejezetlen üzenetű kapcsolatot kihagyja                                                                                                                                                                                                                                                                                                                                                                   | `https://nodejs.org/api/http.html#serverclosecallback`, `https://nodejs.org/api/net.html#serverclosecallback`, a telepített Node forrása (`process.binding('natives')._http_server`, 674 ... 710. sor)                 |
| M-35 | **Saját mérés, Node v26.7.0, 2026-09-23, a valódi `apps/server` kódján** (HTTP réteg, SSE réteg, jelkezelők, `runShutdownSequence`, valódi motor és fájl alapú adatbázis; egyedül a sablon renderelő port átengedő, hogy várakozó `human_approval` lépés létezhessen). A javítás előtt: a REST megszakítás után a futás `cancelled`, a `run_finished` sor a naplóban áll, de az SSE kapcsolaton élő keret nem jött; nyitott SSE kapcsolat mellett `SIGTERM` után a folyamat 30 másodperc alatt sem lépett ki, és a futás `running` maradt. A javítás után: a `run_finished` (`status: cancelled`) és a `run_interrupted` (`reason: graceful_shutdown`) keret `delivery: live` jelöléssel kimegy, a folyamat `0` kóddal kilép, a futás `interrupted`                                                                                                                                                                                                         | saját mérés, `docs/research/2026-09-23-megszakitas-leallas-meres.md`; a regressziót a `run-shutdown-sequence.spec.ts` valódi HTTP SSE kapcsolatos tesztje őrzi                                                         |
| M-36 | **Saját mérés, Node v26.7.0, 2026-09-23, a valódi `apps/server` kódján** (HTTP réteg, jelkezelők, `runShutdownSequence`, valódi motor és fájl alapú adatbázis; eltérés: hamis agent futtató, ami megszakítás nélkül 8 s, `interrupt()` után 3 s alatt zárja a lépést, és átengedő sablon renderelő). Egy agent lépés fut; egy második indító `POST` fejléce a `SIGTERM` előtt megy ki, a törzse 500 ms-mal utána. A javítás előtt: a válasz `200`, új futás indul, az agent lépése a jel után indul el és `interrupt()` hívást nem kap, a kilépés 8536 ms (a második kérés nélkül 3050 ms); mindkét futás `interrupted`. A javítás után: a válasz `500`, `internal` kód, `engine_shutting_down` hibaosztály, futás nem jön létre, a kilépés 3031 ms (két futás, mindkettő 3031 ms)                                                                                                                                                                          | saját mérés, `docs/research/2026-09-23-megszakitas-leallas-meres.md` 6. szekció; a regressziót a `run-shutdown-sequence.spec.ts` félbe küldött kéréses tesztje őrzi                                                    |
| M-37 | **Saját mérés, Node v26.7.0, 2026-09-23, a valódi `apps/server` kódján** (ugyanaz a felállás, mint az M-36-nál; a hamis agent `interrupt()` után 1000 ms alatt zárja a lépést). Egy futás három párhuzamos agent lépéssel, korlát 1, `POST /api/runs/<id>/interrupt`. A javítás előtt: a két sorban álló lépés a megszakítás után elindult és `interrupt()` nélkül végigfutott, agent hívás 3, a megszakító kérés 17043 ms után válaszolt, a három lépés `succeeded` egy `cancelled` futásban; egy csak sorban álló futás megszakítása 15030 ms, plusz egy agent hívás. A javítás után: agent hívás 1, a válasz 1012 ms, illetve 2 ms, a sorban álló lépések `cancelled`, `step_started` nélkül, a `run_finished` (`status: cancelled`) keret `delivery: live`. A leállás alatt befejezett indító kérés válasza `503`, `service_unavailable` kóddal                                                                                                         | saját mérés, `docs/research/2026-09-23-megszakitas-leallas-meres.md` 7. szekció; a regressziót a `packages/engine` `create-engine.spec.ts` korlátozott szabályozós tesztjei és a `run-shutdown-sequence.spec.ts` őrzik |
| M-38 | **Saját mérés, Node v26.7.0, 2026-09-24, a valódi `apps/server` kódján** (a `registerShutdownSignalHandlers` leállási sorrendje, valódi motor és fájl alapú adatbázis; eltérés: hamis agent futtató, ami az `interrupt()` nyugtáját 400 ms-mal később adja, és átengedő sablon renderelő). Egy `start -> sub_workflow` fa (gyerek, illetve gyerek és unoka) fut; a megszakító `POST` fejléce a `SIGTERM` előtt megy ki, a törzse a leállás alatt. Esetenként 5 futás. A javítás előtt: ha a fa az egyetlen aktív futás, `socket hang up`, a fa sora és eseménye `interrupted`; ha egy másik futás 1500 ms-ig áll le, a válasz `200`, a fa sora `cancelled`, a szülő lépés eseménye és üzenete `interrupted` (0/5 egyezés). A javítás után mindkét esetben `503`, `service_unavailable`, `(engine_shutting_down)`, a fa sora és eseménye `interrupted` (5/5). Előbb megszakítás, közben `SIGTERM`: előtte és utána is `200`, a fa egyezően `cancelled` (5/5) | saját mérés, `docs/research/2026-09-23-megszakitas-leallas-meres.md` 7. szekció, tizenegyedik kör; a regressziót a `packages/engine` `create-engine.spec.ts` és a `run-shutdown-sequence.spec.ts` őrzi                 |
...
**A végpont kezelő `invalid_request` hibaosztálya `400`** (SPEC-005 8.2, 8.3). A kezelők a törzs és a query séma hibáját (`zodErrorToProtocolErrorBody`) és az ismeretlen `providerId` értéket `(invalid_request)` záró zárójellel adják; 2026-09-26-ig ez `500 internal` lett, mert a leképezés csak a törzs olvasás JSON hibáját (5.4) ismerte, és a motor 4.2, 4.5, 4.6, 4.8, 8.2 és 11.3 validációs hibaosztályai is `internal`-ra estek `unprocessable` helyett (SPEC-005 8.3 "Két leképezési hiba"). A motor osztályainak listája `satisfies readonly EngineErrorKind[]` kötéssel áll, tehát egy átnevezett hibaosztály a `typecheck` kapun bukik.

**A törzs `errorClass` mezője** (SPEC-005 8.5). A kezelő hibaágából a `buildProtocolErrorBody` állítja elő a teljes törzset: a `code` a fenti leképezésből, az `errorClass` a záró zárójel hibaosztályából, de csak akkor, ha a protokoll `ProtocolErrorClassSchema` szótárában áll (`.safeParse()`); különben a kulcs hiányzik. A `404`/`405` útvonal szintű ág, a törzs olvasás JSON hibája és a kezeletlen kivétel `500` ága nem kap `errorClass` mezőt, mert egyik sem a szótár tagja. A szótár sodródás védelme az `error-class-drift-protection` megvalósítás nélküli regressziós téma (ugyanaz a minta, mint az `enum-drift-protection`).
...
4. **A fejlesztői origin konkrét értéke konfiguráció, nem a spec része**, ezért sztringet nem adunk: a `EASTER_STREAM_DEV_ORIGIN` env változó **opcionális**, alapérték nélkül. Ha hiányzik, a szerver egyetlen CORS fejlécet sem küld, ami pontosan az 1. pont és a 13. szekció 8. kritériumának "nincs konfigurált origin" esete. A jelen pont korábban kötelezőnek nevezte a változót, ami ellentmondott az 1. pontnak, a 8. kritériumnak és a ténylegesen megírt kódnak (`apps/server/src/server-config/environment-variable-name.ts`); a SPEC-008 3.2 szekciója ezt a kód és az 1. pont javára oldotta fel.

**A port és a fejlesztői elrendezés lezárva (2026-09-05, SPEC-008 3. szekció).** A user termékdöntése: a szerver portja `3001`, a Vite dev szerver az alapértelmezett `5173` porton marad, és a **dev REST hívás a Vite proxyn megy**. Ebből következően a fenti CORS hatókör **változatlan**: fejlesztéskor egyedül a stream jön más originről, tehát az engedélyt az `/api` előtagra nem kell kiterjeszteni. A port kötelező env változó marad, alapérték nélkül (`EASTER_SERVER_PORT`); a `3001` érték telepítési konfiguráció, nem literál a kódban.
...
sequenceDiagram
    ...
    P->>H: server.close, uj kapcsolat nem jon be, a visszahivast meg nem varja
    P->>E: shutdown
    E->>S: run_interrupted, elo keret a meg nyitott SSE kapcsolaton
    E-->>P: Outcome ShutdownSummary
    P->>S: minden nyitott SSE nyelo lezarasa
    P->>H: closeAllConnections
    H-->>P: a server.close visszahivasa
    P->>D: close
...
| 2   | `server.close()`, a visszahívás megvárása **nélkül** | Új kapcsolat nem jön be, és a tétlen kapcsolatok lezárulnak (M-17). A visszahívás csak akkor fut le, amikor **minden** kapcsolat véget ért (M-34); a nyitott SSE kapcsolat magától sosem ér véget, tehát a megvárása itt a leállást örökre elakasztaná (M-35).                                                                                                                                                                                       |
| 3   | `engine.shutdown()`                                  | **A motor az SSE nyelők lezárása előtt áll le**, mert a `shutdown` a futásonként beírt `run_interrupted` eseményt élőben is kiadja (SPEC-004 10.2), és ennek a még nyitott SSE kapcsolaton kell kimennie; lezárt kapcsolaton a lezáró keret nem érné el a klienst (M-35). Új kapcsolat a 2. lépés óta nem jön be, tehát indító kérést csak egy már folyamatban lévő kérés hozhat, és azt a motor a leállás kezdete óta elutasítja (lásd lent, M-36). |
| 4   | minden SSE nyelő lezárása                            | A 3. lépés lezáró keretei után. A stream kapcsolatok soha nem járnak le maguktól, tehát a 2. lépés visszahívása nélkülük soha nem futna le. A nyilvántartás ürül.                                                                                                                                                                                                                                                                                    |
| 5   | `server.closeAllConnections()`                       | A dokumentáció szerint erőszakos, és óvatosságot kér (M-17). Itt indokolt: a 4. lépés után csak olyan kapcsolat maradhat, ami félbeszakadt kérést hordoz, és arra várni a leállást akadályozná.                                                                                                                                                                                                                                                      |
| 6   | a 2. lépés visszahívásának megvárása                 | A 4. és az 5. lépés után minden kapcsolat véget ért, tehát lefut.                                                                                                                                                                                                                                                                                                                                                                                    |
| 7   | `database.close()`                                   | A motor után, mert a `shutdown` még ír az adatbázisba. Zárás után minden `DatabaseContext` művelet `database_closed` hibát ad (SPEC-003).                                                                                                                                                                                                                                                                                                            |
| 8   | `process.exitCode` beállítása                        | Nulla, ha minden lépés sikerült; `1`, ha az `engine.shutdown()` hibaágat adott. **`process.exit()` hívás nincs** (4.4, M-21, M-24).                                                                                                                                                                                                                                                                                                                  |

**A 2026-09-23 előtti sorrend két mért hibát adott, ezért változott.** A korábbi menet a
`server.close()` visszahívását megvárta, és csak utána zárta az SSE nyelőket, majd hívta a motort.
Nyitott SSE kapcsolat mellett a visszahívás sosem futott le, tehát `SIGTERM` után a folyamat 30
másodperc alatt sem lépett ki, a motor leállása el sem indult, és a futás `running` állapotban
maradt az adatbázisban (M-35). Ha a várakozás nem akadt volna el, az SSE nyelők akkor is a motor
előtt zárultak volna, tehát a `run_interrupted` keret élőben nem mehetett ki. A mostani sorrend
mindkét hibát megszünteti, és a SPEC-004 10.2 szerződését (futásonként egy `run_interrupted`
esemény) az élő nézetre is kiterjeszti.

**A jel pillanatában folyamatban lévő kérés, mérve (M-36).** A 2. lépés csak az új és a tétlen
kapcsolatokat zárja (M-17, M-34), tehát egy a jel előtt fejléccel megkezdett REST kérés a 3. lépés
alatt még lefut. A 2026-09-23 előtti motor ilyenkor új futást indított, aminek az agent lépése a
jel után indult el és `interrupt()` hívást nem kapott: az adatbázis végállapota helyes volt (a
globális `recoverInterruptedRuns('graceful_shutdown')` ezt a futást is `interrupted` állapotba
vitte), a kilépés viszont a lépés teljes hosszával késett, a mérésben 3050 ms helyett 8536 ms-ig.
A javítás óta a motor a leállás kezdetén, az aktív futások lekérdezése ELŐTT letiltja az új futást
és lezárja a szabályozót (SPEC-004 10.2 1. pont). Ugyanez a kérés ekkor `500` választ kapott
`internal` kóddal és `engine_shutting_down` hibaosztállyal, futás nem jött létre, és a kilépés 3031
ms volt. Az `internal` kód a SPEC-005 8.3 táblázat "minden más" sorából jött, mert a protokoll
szótárában nem volt átmeneti elérhetetlenséget jelentő kód. **2026-09-23 óta a válasz `503`,
`service_unavailable` kóddal** (SPEC-005 8.2, 8.3; RFC 9110 15.6.4): a leálló szerver a folyamat
átmeneti állapota, nem a kérés hibája. Mérve: `503 Service Unavailable`, a törzsben
`engine_shutting_down` hibaosztály, futás nem jön létre, a kilépés 3035 ms (M-37). A regressziót a
`run-shutdown-sequence.spec.ts` őrzi, valódi HTTP kapcsolaton, félbe küldött kéréssel.

**A jel pillanatában folyamatban lévő megszakító kérés, mérve (M-38).** Ugyanez az ablak a
`POST /api/runs/{runId}/interrupt` kérést is éri. A 2026-09-24 előtti motor a leállás alatt
beérkezett megszakítást végrehajtotta: ha a megszakított fa volt az egyetlen aktív futás, a
kérés a fa leállására várt, az 5. lépés a kapcsolatot a válasz előtt zárta, és a kliens
`socket hang up` hibát kapott; ha egy másik futás tovább tartotta a leállást, a válasz `200` lett,
a fa sora viszont `cancelled`, miközben a szülő lépés eseménye `interrupted` (SPEC-004 9.
szekció). A user döntése (2026-09-24) szerint a leállás alatt érkező megszakítás ugyanúgy
`503`, `service_unavailable` választ kap `engine_shutting_down` hibaosztállyal, mint az indító
kérés, és semmit nem ír. Ha a törzs a 3. lépés alatt érkezik, a válasz még ekkor kimegy, a 4. és
az 5. lépés előtt, mert a motor a megszakítást nem várakoztatja, hanem azonnal elutasítja; a
leállási sorrend ehhez nem változott. A regressziót a
`run-shutdown-sequence.spec.ts` őrzi, valódi HTTP kapcsolaton, félbe küldött megszakító
kéréssel.
...
| `engine-assembly`       | a motor kilenc portjának megvalósítása (óra, azonosító, környezet olvasó, esemény kiadó, leíró kereső, elutasító kifejezés és sablon port) és a `createEngine` hívás; a két elutasító port helyére a SPEC-010 11.1 a `workflow-language` csomag Mustache és CEL portját köti be |
...
| `error-mapping`         | a SPEC-005 8.3 táblázat: az `Outcome` üzenetéből a hibaosztály név kiolvasása és `ProtocolErrorCode` értékre képzése, plusz a hiba törzs összeállítása a 8.5 `errorClass` mezővel                                                                                               |
...
| a leállás lezárja a nyitott streameket, a lezáró keret után          | valódi HTTP SSE kapcsolat egy várakozó futásra, majd a `shutdown-sequence` futtatása; a `run_interrupted` keret `live`, utána a folyam véget ér |
...
| --- | -------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| O-1 | A szerver portja és a fejlesztői origin                                                | **Lezárva, 2026-09-05 (SPEC-008 3. szekció).** A user termékdöntése: a szerver portja `3001`, a Vite dev szerver az alapértelmezett `5173` porton marad, és a dev REST hívás a Vite proxyn megy, tehát az 5.7 CORS hatóköre változatlanul a `STREAM_PATH` útvonalra szűkül. A **port kötelező** env változó marad, alapérték nélkül; ha hiányzik, a szerver a 4.3 szerint `fatal` naplóval és `1` kóddal kilép (M-11). A **fejlesztői origin opcionális**, az 5.7 1. pontja és a 13. szekció 8. kritériuma szerint | lezárva                                                                                                                                                                                                                                              |
...
40. `SIGINT` és `SIGTERM` esetén a 8.2 nyolc lépése ebben a sorrendben fut, és mindkét jelre ugyanaz. Két külön teszteset.
41. A leállás minden nyitott SSE kapcsolat nyelőjét az `engine.shutdown()` **után** zárja le, tehát a `shutdown` által kiadott `run_interrupted` keret a még nyitott kapcsolaton `delivery: live` jelöléssel kimegy, és a leállás nyitott SSE kapcsolat mellett sem akad el. Futtatott teszt igazolja valódi HTTP SSE kapcsolattal (2026-09-23 óta; a korábbi, fordított sorrendet előíró változat a 8.2 szerint mért hibát adott).
