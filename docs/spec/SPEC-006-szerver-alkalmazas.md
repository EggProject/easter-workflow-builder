...
**Az M-31, M-32 és M-33 tény nem itt, hanem a 6.5 szekcióban áll**, mert csak ott, az élő szórás tárgyalása mellett érthető, hogy miért döntőek. A számozás folytonos, tehát a spec bizonyítékai `M-1` ... `M-35` azonosítón hivatkozhatók; az M-34 és az M-35 a 2.3 szekció végén áll (2026-09-23, a leállási sorrend javítása).
...
| 3   | `engine.shutdown()`                                  | **A motor az SSE nyelők lezárása előtt áll le**, mert a `shutdown` a futásonként beírt `run_interrupted` eseményt élőben is kiadja (SPEC-004 10.2), és ennek a még nyitott SSE kapcsolaton kell kimennie; lezárt kapcsolaton a lezáró keret nem érné el a klienst (M-35). Új kapcsolat a 2. lépés óta nem jön be, tehát új futást csak egy már folyamatban lévő kérés indíthat (lásd lent). |
...
**Maradék kockázat, kimondva.** A 2. lépés csak az új és a tétlen kapcsolatokat zárja (M-17,
M-34); egy a jel pillanatában már folyamatban lévő REST kérés a 3. lépés alatt még lefuthat, és
elvileg új futást indíthat a leálló motorba. Ennek adatbázis oldali végállapota ugyanaz, mint a
durva leállásé: a sort vagy a 3. lépés globális `recoverInterruptedRuns('graceful_shutdown')`
hívása, vagy a következő indulás helyreállítása viszi `interrupted` állapotba (SPEC-004 10.2). Hogy
egy ilyen futás élő agent lépése mennyivel késlelteti a folyamat kilépését, **nem mért**.
