...
| --------------------- | ---------------------------------------------------------------- |
| `invalid_request`     | a kérés nem volt érvényes (a mező megnevezése nélkül, lásd O-10) |
...
**Ahol a mondat nem hordozza a teendőt, az nyitott pont (O-10).** A szerver szövege eddig megnevezte a hibás mezőt (`invalid_request`, SPEC-005 7.5) és a hibaosztályt (`unprocessable`: `no_default_provider`, a gráf szemantikai hibái, például `graph_cycle_detected`, SPEC-005 8.3), a SPEC-008 5.4 pedig erre építve nem másolja le a gráf szemantikai validációját a kliensbe. A döntés óta ezek a felületen nem látszanak; a legkisebb, a döntéssel következetes megoldás áll (csak a mondat), a részlet kezelését a user dönti el (15. szekció O-10).
...
| O-10 | A szerver hibaüzenetének részlete (a hibás mező útvonala, a hibaosztály neve) kell-e a felhasználó teendőjéhez, és ha igen, milyen alakban | **csak a kód magyar mondata látszik** (user döntés 2026-09-24, 8.4): a `message` mezőt a kliens eldobja. Ahol a teendő a részletből derülne ki, ott elvész: a futás indításának `unprocessable` hibája (`no_default_provider`: alapértelmezett provider kell; `graph_cycle_detected` és a többi gráf szemantikai hibaosztály: a gráfot kell javítani, SPEC-008 5.4 és 15. szekció 12. kritérium), és a mentés `invalid_request` hibája a szerver által megnevezett mezővel. A kliens oldali ellenőrzés (a mentés előtti séma, a kötelező indító mezők) a mezőszintű esetek nagy részét a küldés előtt elkapja                                                                                                                                                                                                                                                    | a user döntése: marad a puszta mondat, vagy a zárt szótárú hibaosztály (SPEC-005 8.3 táblázata) saját magyar mondatot kap, vagy a részlet más alakban jelenik meg |
