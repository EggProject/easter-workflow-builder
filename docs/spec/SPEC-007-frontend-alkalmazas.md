...
- **Nem dönt portról, origin értékről, lapméretről, időkorlátról és a stream `retry` értékéről.** Egyikre sincs dokumentált forrásunk, ezért egyikre sem adunk számot; mindegyik a 15. szekcióban áll, kimondott addigi viselkedéssel. A port és az origin kérdését a **SPEC-008 3. szekciója** zárta le a user termékdöntésével, és az O-4 tétel ott van átvezetve.
...
- **Nem építi meg a fejlesztői Vite proxyt, és nem dönt a dev CORS elrendezésről.** A SPEC-006 5.7 és a SPEC-005 5.8 a **szerver** oldalán rendezi el a kérdést: a CORS engedély kizárólag a `STREAM_PATH` útvonalra vonatkozik, pontosan egy konfigurált dev originre, és a stream szándékosan nem megy proxyn át. A frontend oldali párja, egy `vite.config.ts` `server.proxy` szabály, a jelen spec megírásakor **nem volt megépíthető forrás nélkül**: a proxy `target` mezője konkrét portot követel, arra pedig sem a SPEC-006 O-1, sem a jelen spec O-4 nem adott számot, és tippelni tilos (`.claude/CLAUDE.md` 4.). A jelen spec ezért a kliens oldalon egyetlen utat ismer: minden hívás, REST és SSE egyaránt, a kötelező `VITE_API_ORIGIN` konfigurációra megy, proxy nélkül. **Ezt a SPEC-008 3. szekciója zárta le és váltotta ki** (a port `3001`, a dev szerver `5173`, a REST hívás proxyn megy, a konfiguráció `apiOrigin` és `streamOrigin` mezőre válik); az O-4 tétel ott van átvezetve.
...
| --------------------- | ---------------------------------------------------------------- |
| `invalid_request`     | a kérés nem volt érvényes (a mező megnevezése nélkül, lásd lent) |
...
**A felületen KIZÁRÓLAG a kód mondata áll, a szerver `message` mezője nem** (user döntés 2026-09-24, "Mindhárom javítás"). Korábban a `perform-route-request.ts` a mondat után fűzte a szerver szövegét (`` `${mondat}: ${message}` ``), ami két hibát adott: a mondat záró pontja után kettőspont állt (".:"), és a felületre belső szöveg került (azonosító, zárójeles hibaosztály, például `A(z) "<stepRunId>" ... (already_decided).`). A döntés három része:

1. **Írásjel.** A hibaág üzenete pontosan a táblázat mondata, utána semmi; minden mondat egyetlen záró ponttal végződik, kettőspont nélkül (`protocol-error-message.spec.ts`, `perform-route-request.spec.ts`).
2. **Megjelenítés.** Minden REST hívás `Outcome` hibaága a design system `danger` `Alert` blokkjában jelenik meg (`packages/ui` `Alert`, a forrás `Alert.jsx` szerinti variánssal és szereppel: a `danger` variáns `role="alert"`, tehát a képernyőolvasó bejelenti), nem nyers `<p role="alert">` elemben. A blokk cím nélküli, a mondat az üzenete. Modálisban a `<form>` elemen kívül, a modális törzs közvetlen gyerekeként áll, hogy a forrás `.modal__body > * + *` térköze elválassza a mezőktől. A maradék nyers `role="alert"` helyeket (nem REST eredetűek: hiányzó query paraméter, a mentés előtti helyi séma ellenőrzés, a pillanatkép helyi vetítése, a futás saját hibája, a szerkesztő panel két statikus figyelmeztetése) fájlonként megszámolva az `apps/web/src/greppable-invariants/` (18) tesztje őrzi.
3. **A szerver belső szövege nem jut a felületre.** A `ProtocolErrorBodySchema` a törzs alakját továbbra is teljes egészében ellenőrzi, a `message` mezőt viszont a kliens eldobja. Unit (`perform-route-request.spec.ts`: 409 `already_decided`, 404, mind a hat kód) és e2e teszt (`rest-error-paths.spec.ts`, `approval-prompt.spec.ts`, `workflow-list.spec.ts`: a nyers szöveg a mockban szerepel, a DOM-ban nem) igazolja. Naplózási út (konzol, logger) az `apps/web` alatt nincs, újat a döntés nem vezet be.

**Határok, kimondva.** (a) **Az űrlap mező hibái nem ide tartoznak**: azok a `.claude/CLAUDE.md` 11. szekciója szerint kizárólag a mező alatt, `aria-describedby` plusz `role="alert"` kötéssel állnak, változatlanul. (b) **A futás nézet "Várakozás a szerverre" jelzése** (átmeneti 502, 503, hálózati hiba korábbi értékkel) változatlanul a `warning` `Alert` (`role="status"`); a benne idézett hiba ugyanaz a mondat, szerver szöveg nélkül. (c) **A soronkénti műveletek `Toast` jelzése marad `Toast`** (a lista "Indítás", az előzmények "Megszakítás" és "Újraindítás", a szerkesztő "Mentés" hibája, `danger` variáns, "... sikertelen" címmel): ezek a design system kész, átmeneti értesítései, a döntés a nyers `<p role="alert">` helyekről szólt, és egy soronkénti műveletnek nincs olyan tartós helye, ahol egy `Alert` blokk állhatna. A szövegük ugyanúgy csak a kód mondata. (d) **A kliens saját üzenetei** (hálózati hiba, hibás JSON, hibás alak a mezőútvonallal, HTTP státuszos üzenet) nem szerver szövegek, változatlanok.

**Ismert okokra saját mondat (user döntés 2026-09-26, O-10 lezárva).** Ahol a kód mondata nem hordozza a teendőt (a futás indításának `unprocessable` hibája: alapértelmezett provider kell, vagy a gráfot kell javítani), a szerver hibaosztálya saját, érthető magyar mondatot kap; a nyers `message` továbbra sem jelenik meg. A döntés részei:

1. **A hibaosztály forrása strukturált mező, nem a szöveg.** A szerver a SPEC-005 8.5 zárt szótárába eső hibaosztályt a törzs `errorClass` mezőjében adja; a kliens a `message` zárójeles végét nem elemzi. A `perform-route-request.ts` protokoll hibaága: ha az `errorClass` jelen van, a `protocol-error-class-message.ts` mondata, különben a fenti táblázat kód mondata. A mező nélküli törzs (a mező előtti szerver válasz) a kód mondatát kapja; a szótáron kívüli `errorClass` érték a sémán elbukik, és "a szerver hibás választ adott (HTTP n)" üzenetet ad (a kettő egy repóban, egyszerre élesedik, SPEC-005 8.5).
2. **A leképezés kimerítő.** A 26 hibaosztály (a futás indításának minden validációs osztálya és a jóváhagyás `already_decided` ága, SPEC-005 8.5) mondata egy `Readonly<Record<ProtocolErrorClass, string>>` annotációjú táblában áll: egy új szótár tag fordítási hibát ad, amíg nincs mondata. A mondatok a felület szavaival nevezik meg az okot (a csomópont nevek a `graph-node-catalog` címkéi: Indítás, Ciklus, Elágazás, Szétosztás, Összefésülés, Hibakezelő, a Ciklus két kimenete Folytatás és Kilépés), egyetlen záró ponttal, kettőspont nélkül, mint a kód mondatai. Példák: `no_default_provider` "Nincs alapértelmezett provider beállítva.", `graph_cycle_detected` "A gráf Ciklus csomópont nélküli kört tartalmaz.", `unreachable_node` "A gráf egyik csomópontja nem érhető el az Indítás csomópontból.", `already_decided` "Ezt a jóváhagyást már eldöntötték.". Unit teszt minden osztályra (`protocol-error-class-message.spec.ts`), és a tábla lefedi a protokoll szótár minden tagját.
3. **Ez megjelenítés, nem validáció.** A gráf szemantikai ellenőrzése változatlanul csak a szerveren fut (SPEC-008 5.4); a kliens a szerver által megnevezett osztályhoz rendel mondatot.
4. **A mezőút nem kap mondatot.** A mentés `invalid_request` hibájában a szerver által megnevezett mező a felület nyelvén nem nevezhető meg egyértelműen: a mentés előtt a kliens ugyanazzal a protokoll sémával ellenőriz (`validate-graph-for-save.ts`, a mező alatti hibák a node inspectorban), a létrehozás és az átnevezés modálisa üres névvel nem küld, tehát a szerver mezőútja csak szerződés sértésnél jönne. Ilyenkor a kód mondata marad ("A kérés nem volt érvényes."), a mezőút nélkül (SPEC-005 8.5).

E2e: `rest-error-class.spec.ts` (futás indítás `no_default_provider` a láblécben, `graph_cycle_detected` a modálisban, mentés `malformed_node_config` a `Toast` üzenetében, mentés `invalid_request` a kód mondatával a mezőút nélkül, jóváhagyás `already_decided`); a nyers szöveg a mockban szerepel, a DOM-ban nem. A `c566213` e2e tesztjei változatlanul zöldek, mert az ő mockjaikban nincs `errorClass` mező.
...
| O-4  | Az API origin, amit a felület hív, és ebből következően a fejlesztői CORS és proxy elrendezés                                              | **Lezárva, 2026-09-05 (SPEC-008 3. szekció).** A user termékdöntése: a szerver portja `3001`, a Vite dev szerver az alapértelmezett `5173` porton marad, tehát a `server.proxy` szabály megépíthető, és a **dev REST hívás proxyn megy**. A SPEC-006 5.7 CORS hatóköre ezért **változatlan** (kizárólag a `STREAM_PATH`), és az `/api` előtagra nem terjed ki. A konfiguráció két mezőre válik: `apiOrigin` (a `rest-client` célja) és `streamOrigin` (a `stream-client` célja, ami a proxyt megkerüli, SPEC-005 5.8); mindkettő kötelező `VITE_` előtagú env változó, **alapérték nélkül** (M-11), és hiányzó változó esetén az alkalmazás konfigurációs hibaképernyőt rajzol, ami a változó **nevét** nevezi meg, az értékét soha. Éles használatban a kérdés nem merül fel: a szerver szolgálja ki a felépített felületet, tehát azonos origin, és nincs CORS | lezárva                                                                                                                           |
...
| O-10 | A szerver hibaüzenetének részlete (a hibás mező útvonala, a hibaosztály neve) kell-e a felhasználó teendőjéhez, és ha igen, milyen alakban | **Lezárva, 2026-09-26 (user döntés: "Ismert okokra saját mondat").** A szerver a SPEC-005 8.5 zárt szótárába eső hibaosztályt a törzs `errorClass` mezőjében adja, és a kliens ehhez saját magyar mondatot rendel (8.4); a nyers `message` továbbra sem jelenik meg, ismeretlen vagy hiányzó osztálynál a kód mondata marad. A mentés `invalid_request` hibájának mezője nem kap mondatot, mert a felület nyelvén nem nevezhető meg egyértelműen (8.4 4. pont)                                                                                                                                                                                                                                                                                                                                                                                                   | lezárva                                                                                                                           |
