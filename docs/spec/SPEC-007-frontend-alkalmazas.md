...
| Kimenet  | a `packages/ui` csomag tizenhat téma mappája, és az `apps/web` alkalmazás tizenkét téma mappája                                                                                                                                                                                        |
...
| --------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------- |
| `--ep-screen-md` (`768px`)  | a `.app-tn__navigation` kikerül a barból egy lenyíló menübe, amit a brand melletti gomb nyit; a bar többi eleme (brand, actions) marad | a token kommentje szerint ez a **tablet** határa     |
...
| `<Név>.spec.ts` | a komponens unit tesztje, `createRoot` és `act` felett, happy-dom környezetben, **JSX nélkül**, `createElement` hívásokkal |

**Miért `.spec.ts` és nem `.spec.tsx`.** A `vitest.config.ts` `coverage.exclude` listája a
`**/*.spec.ts` bejegyzést tartalmazza, `**/*.spec.tsx` bejegyzést nem (M-45), a `coverage.include`
viszont a `.tsx` fájlokat is felveszi. A `.claude/CLAUDE.md` 8. szekciója pedig kimondja, hogy a
kizárási lista nem bővíthető. Amíg ez a feszültség fel nem oldódik, a teszt fájlok `.ts`
kiterjesztést kapnak, és a React fát `createElement` hívásokkal építik, ami a `react` csomag
dokumentált API-ja. **Ez az O-1 nyitott kérdés**, és a `.claude/CLAUDE.md` 14. szekció 1. tétele.
...
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `stream_ready`        | eltárolja a `serverInstanceId` értéket; ha ez **eltér** a korábbitól, a réteg jelzi, hogy a szerver újraindult, és a képernyők újratöltik az adatot (SPEC-005 5.2) |
...
**A törlés megerősítése kötött, és a protokoll kényszeríti ki.** A `DELETE` törzsének `acknowledgeIrreversible` mezője Zod szinten a `true` literál (SPEC-005 4.2 A táblázat), tehát a felület nem tud véletlenül törölni. A modális az előzetes összefoglalóból megnevezi, hány futás, hány lépés futás, hány esemény és hány jóváhagyás vész el, és a megerősítés egy jelölőnégyzet bepipálásához kötött.
...
| a 12 komponens téma | fejenként a `.tsx`, a `.css` és a `.spec.tsx` fájl (6.2)                                                                    |
...
**A lista nem bővül.** A React komponens tesztek ezért `.spec.ts` fájlok, JSX nélkül, `createElement` hívásokkal (6.2). Az alternatíva a `.spec.tsx` név plusz egy `**/*.spec.tsx` sor a kizárási listán, ami a már meglévő `**/*.spec.ts` bejegyzés pontos analógja lenne (M-45), de a `.claude/CLAUDE.md` 8. szekciója a bővítést tiltja, tehát ez **felhasználói döntés**. Ez az **O-1** nyitott kérdés, és a `.claude/CLAUDE.md` 14. szekció 1. tétele.
...
| Egy `.spec.tsx` fájl bekerül a lefedettségi mérlegbe                       | a küszöb hamis okból bukik vagy hamis okból zöld                          | az O-1 lezárása, és kritérium, hogy a mérleg csak termékkódot tartalmaz                                          |
...
| --- | --------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| O-1 | A `.spec.tsx` fájlok és a `coverage.exclude` lista                                                  | **a lista nem bővül**, mert a `.claude/CLAUDE.md` 8. szekciója ezt tiltja: a komponens tesztek `.spec.ts` fájlok, JSX nélkül, `createElement` hívásokkal (6.2, 13.3). Az alternatíva egyetlen új sor lenne, a már meglévő `**/*.spec.ts` bejegyzés pontos analógja egy másik kiterjesztésre (M-45), ami nem termékkód kizárás; ezt viszont csak a user engedheti | user döntés a két út között; ha az alternatíva mellett dönt, a `.claude/CLAUDE.md` 8. szekció "a lista nem bővíthető" mondatát pontosítani kell, és a 14. szekció 1. tételét törölni |
| O-2 | A táblamotor: `@tanstack/react-table` `8.21.3`, `9.2.4`, vagy egyik sem                             | a `data-table` téma nem íródik meg, amíg a mérés le nem fut. A mérés a PLAN-008 F0 fázisában áll, és mindkét verziót próbálja React 19.2.8 ellen. **Ha egyik sem működik**, a téma motor nélkül, sima táblaként épül meg, ugyanazzal a bájtra azonos CSS-sel, rendezés és szűrés nélkül (6.3)                                                                    | a lefuttatott mérés, `docs/research/` alá vezetve, plusz a `docs/research/2026-08-26-toolchain.md` átvezetése. Az M-37 miatt a hivatalos doksi nem elég                              |
...
| O-4 | Az API origin, amit a felület hív                                                                   | kötelező `VITE_` előtagú env változó, **alapérték nélkül** (M-11). Ha hiányzik, az alkalmazás nem találgat: egy konfigurációs hibaképernyőt rajzol ki, ami megnevezi a hiányzó változó **nevét**, az értékét soha. Ez a SPEC-006 O-1 továbbvitele                                                                                                                | termékdöntés a szerver portjáról és a dev szerver portjáról, ami a SPEC-006 O-1 lezárása                                                                                             |
...
1. A `packages/ui/src` alatt pontosan a 12.1 listája szerinti tizenhat téma mappa áll közvetlenül, egy szint mélyen, plusz az `index.ts` barrel; egyetlen téma mappában sincs alkönyvtár.
2. Az `apps/web/src` alatt pontosan a 12.2 listája szerinti tizenkét téma mappa áll közvetlenül, egy szint mélyen, plusz az `index.ts` barrel és a `vite-env.d.ts` típus only fájl; egyetlen téma mappában sincs alkönyvtár.
...
12. A `packages/ui/src/topnav-shell/topnav-shell.css` kizárólag a `.app-tn`, az `.app-pagehead` és az `.app-content` szabályokat tartalmazza, **szabályonként bájtra azonosan** a forrással; a másik négy shell egyetlen szabálya sem került át.
...
25. Mind a tizenkét komponens téma mappa pontosan három fájlt tartalmaz: a `.tsx`, a `.css` és a `.spec.ts` fájlt; utóbbi JSX nélkül, `createElement` hívásokkal, amíg az O-1 másként nem dönt.
...
44. A `stream_ready` keret `serverInstanceId` értékének változása a képernyők adatának újratöltését váltja ki. Futtatott teszt igazolja (SPEC-005 5.2).
...
48. A workflow törlés megerősítő modálisa megnevezi, hány futás, hány lépés futás, hány esemény és hány jóváhagyás vész el, és a megerősítés jelölőnégyzethez kötött; a `DELETE` kérés törzse az `acknowledgeIrreversible: true` literált hordozza. Futtatott teszt igazolja.
49. A provider választó a `GET /api/providers` válaszából épül, és env változó **nevet** jelenít meg, értéket soha. Futtatott teszt igazolja.
...
52. Az `apps/web/e2e` alatt minden REST hívás `page.route()` mockon megy; valós backend szervert egyetlen teszt sem szólít meg. Greppel ellenőrizhető teszt igazolja, hogy nincs `http://localhost:` literál a REST hívásokhoz.
...
58. A `bun run test` nulla kilépési kóddal fut, és a lefedettség mind a négy metrikán 100 százalék. A `coverage.exclude` lista **egy sorral csökkent** (az `apps/web/src/main.ts` törölve), és **nem bővült**; kizárólag akkor bővülhet egyetlen `**/*.spec.tsx` sorral, ha a user az O-1 kérdést így zárja le.
