...
| 15  | `POST /api/runs/{runId}/interrupt`      | üres törzs                                                                    | `InterruptSummaryResponse`              | `engine.interruptRun`                             | `not_found`, `conflict`, `service_unavailable`                         |
...
**A 15. végpont a szabályos leállás alatt `503` választ ad** (user döntés 2026-09-24). A leállás kezdete után érkező megszakítás `service_unavailable` kódot kap, `engine_shutting_down` hibaosztállyal, és semmit nem ír: a futást a leállás zárja `interrupted` állapotba (SPEC-004 9. szekció "Megszakítás szabályos leállás közben"). Ugyanaz a hibaosztály és ugyanaz a kód, mint a 9. és a 16. végpont leállás közbeni elutasításánál. Mérve a valódi szerveren, hamis agenttel: a jel előtt fejléccel megkezdett, a leállás alatt befejezett megszakító kérés a javítás előtt `socket hang up` hibát (ha a fa volt az egyetlen aktív futás), illetve `200` választ és `cancelled` sort kapott, miközben a szülő lépés eseménye `interrupted` volt (ha egy másik futás tovább tartotta a leállást); utána mindkét esetben `503`, a kapcsolatok zárása előtt, és a fa sora és eseménye egyezően `interrupted` (`docs/research/2026-09-23-megszakitas-leallas-meres.md` 7. szekció, tizenegyedik kör).
...
Ugyanez az alak áll a REST hibaválasz törzsében és a `protocol_error` SSE keret törzsében (utóbbi egy `runId` mezővel kiegészítve). Nincs második hiba alak, nincs `details` szabad objektum, és nincs `stack` mező. **Az elhagyható `errorClass` mező csak a REST törzsben áll** (2026-09-26, 8.5): a `protocol_error` keret egyetlen előidéző oka (5.6 2. pont, nem egész `Last-Event-ID` fejléc) kapcsolat szintű hiba, ami nincs a 8.5 szótárában, tehát a keret sémája változatlan, és a közös mag (`code`, `message`) típusszintű egyezését az `event-stream` téma tesztje őrzi (38. kritérium).
...
| `service_unavailable` | `503` | a szerver átmenetileg nem fogad új futást és megszakítást, mert éppen leáll |
...
| `engine_shutting_down`                                                                                     | `service_unavailable` | a folyamat átmeneti állapota: a leálló motor új futást nem indít (SPEC-004 10.2 1. pont), és megszakítást nem hajt végre (SPEC-004 9. szekció, user döntés 2026-09-24) |
| minden motor eredetű validációs hibaosztály (a SPEC-004 4.2, 4.5, 4.6, 4.7, 4.8, 8.2 és 11.3 ellenőrzései) | `unprocessable`       | a gráf tárolható, de nem futtatható                                                                                                                                    |
| `invalid_request` (a végpont kezelő saját séma hibája és az ismeretlen `providerId`)                       | `invalid_request`     | a bejövő alak nem illeszkedik a sémára (8.2)                                                                                                                           |
...
**Két leképezési hiba, javítva 2026-09-26.** A szerver leképezése a motor validációs hibaosztályai közül csak a SPEC-004 4.7 táblázatát és a `no_default_provider` osztályt ismerte, tehát a `graph_cycle_detected`, a `loop_back_edge_outside_body`, a `loop_missing_branch_edge` (4.6), a `reserved_branch_key_misuse` (4.2), az `unbalanced_fan_out_scope` (4.5), a `missing_required_input` (4.8), az `insufficient_backoff_list` (8.2) és a 11.3 nyolc leíró alapú osztálya `500 internal` választ adott a táblázat szerinti `422 unprocessable` helyett; és a végpont kezelők `invalid_request` hibaága (a törzs vagy a query séma hibája, az ismeretlen `providerId`) szintén `500 internal` lett a 8.2 szerinti `400` helyett, mert a leképezés csak a törzs olvasás JSON hibáját ismerte. Mérve a javítás előtt a szerver saját `mapOutcomeMessageToErrorCode` függvényén: `graph_cycle_detected`, `missing_required_input` és a kezelő `invalid_request` üzenete mind `internal`, a `no_default_provider` `unprocessable`. A regressziót az `apps/server` `error-mapping` témájának tesztjei őrzik (a motor saját formázójával előállított üzenettel, osztályonként), plusz két valódi HTTP kéréses teszt a `create-http-server.spec.ts` fájlban.
...
**Kiszivárog, szándékosan**: a hibaosztály neve, ahogy az `Outcome` üzenetében áll, és 2026-09-26 óta a 8.5 szótárába eső osztály az `errorClass` mezőben is. Ez nem belső részlet, hanem a rendszer szótára: a felhasználó ebből érti meg, hogy a futása azért nem indult el, mert a gráfban kör van, nem azért, mert a szerver elromlott. **A webes felület 2026-09-24 óta (user döntés) nem jeleníti meg a `message` mezőt**; 2026-09-26 óta (user döntés, "Ismert okokra saját mondat") az `errorClass` saját magyar mondatát mutatja, és ahol a mező hiányzik, a `code` mondatát (SPEC-007 8.4, O-10 lezárva).
...
### 8.5 A hibaosztály géppel olvasható mezője (`errorClass`)

**Kiváltó ok.** A user 2026-09-26-i döntése ("Ismert okokra saját mondat", SPEC-007 O-10): a felület a szerver hibaosztályát (például `no_default_provider`, `graph_cycle_detected`) saját, érthető magyar mondatra képezi, a nyers `message` továbbra sem jelenik meg. Ehhez a kliensnek géppel olvashatóan kell tudnia a hibaosztályt.

**Miért strukturált mező, és nem a `message` elemzése.** A hibaosztály eddig csak a `message` szövegében, a záró zárójelben állt (F-22). A szöveg elemzése a kliensben törékeny: a zárójeles konvenció a szerver belső üzenet formátuma, nem szerződés, és egy megfogalmazás váltás csendben hibás mondatot adna. A szerver viszont már ma kiolvassa a hibaosztályt (8.3, `apps/server` `extract-trailing-error-class.ts`), tehát a legkisebb helyes megoldás, hogy ugyanezt az értéket strukturáltan is kiírja.

**A mező alakja.**

- `errorClass: ProtocolErrorClassSchema.optional()` a `ProtocolErrorBodySchema` `z.strictObject` alakjában, a `.readonly()` burkolón belül (7.3 szabályai: nincs `.default()`, nincs `.transform()`).
- A `ProtocolErrorClassSchema` zárt `z.enum` (`protocol-error` téma, `protocol-error-class.ts`), 26 taggal: a futás indításának minden validációs hibaosztálya (a motor `validateRun` és `prepareRun` útja: SPEC-004 4.2 `reserved_branch_key_misuse`; 4.5 `unbalanced_fan_out_scope`; 4.6 `graph_cycle_detected`, `loop_back_edge_outside_body`, `loop_missing_branch_edge`; 4.7 tíz sora, `invalid_start_node` ... `unsupported_join_merge_setting`; 4.8 `missing_required_input`; 8.2 `insufficient_backoff_list`; 11.1 `no_default_provider`; 11.3 nyolc leíró alapú osztálya), plusz a jóváhagyás `already_decided` ága (4.2 18. végpont). A lista a kódból gyűlt, a felhasználói teendőt hordozó végpontok (futás indítás és újraindítás, mentés, jóváhagyás, törlés) visszaadható hibaosztályaiból.
- **Elhagyható, nem `null`.** A szerver a mezőt csak akkor írja ki, ha a záró zárójel hibaosztálya a szótárban áll (`.safeParse()`); egyébként a kulcs hiányzik. A szótáron kívül marad szándékosan: `not_found`, `illegal_status_transition`, `engine_shutting_down`, `database_closed`, `invalid_request` és minden más, mert ezeknél a `code` mondata már hordozza, amit a felhasználó tehet, vagy nincs felhasználói teendő.
- **A mező előtti szerver válasz** (a mező nélkül) érvényes marad, és a kliens a `code` mondatát mutatja. **A szótáron kívüli `errorClass` érték** a `z.strictObject` sémán elbukik, és a kliens "a szerver hibás választ adott" üzenetet mutat: a kliens és a szerver egy repóban, egyszerre élesedik, tehát ez csak szerződés sértésnél fordulhat elő.

**A mezőút (`invalid_request`) nem kap strukturált mezőt.** A mentés útjain a kliens a küldés előtt ugyanazzal a protokoll sémával ellenőriz, amivel a szerver (SPEC-008 5.4, `validate-graph-for-save.ts`; a létrehozás és az átnevezés modálisa üres névvel nem küld), tehát a szerver mezőútja csak szerződés sértésnél jönne, és a felület nyelvére fordítása találgató leképezés lenne. A 40. kritérium `path` tilalma (fájl útvonal) ettől változatlan.

**Sodródás védelem.** A szótár a motor `EngineErrorKind` uniójának szándékos részhalmaza, plusz a `db` `already_decided` ága. Az `apps/server` `error-class-drift-protection` megvalósítás nélküli regressziós tesztje típusszinten igazolja, hogy az `already_decided` kivételével minden tag `EngineErrorKind` (`Exclude<ProtocolErrorClass, EngineErrorKind>` pontosan `'already_decided'`, a `typecheck` kapun), futásidőben pedig minden tagot átenged a motor `isEngineErrorKind` guardján, és a motor saját formázójával előállított üzenetből a szerver a tagot és az `unprocessable` kódot adja. Az `already_decided` futásidejű ága a valódi adatbázison futó `decide-approval.spec.ts` "már eldöntött" tesztje.
...
| `protocol-error` | a `ProtocolErrorCode` szótár, a `ProtocolErrorClass` szótár (8.5), a `ProtocolErrorBody` séma, a HTTP státusz leképezés, és a Zod hiba lista fordítása a boríték alakra |
...
38. Ugyanaz a `ProtocolErrorBody` alak áll a REST hibaválaszban és a `protocol_error` SSE keretben; két külön hiba alak nincs. A kizárólag REST oldali, elhagyható `errorClass` mező (8.1, 8.5) mellett a közös mag (`code`, `message`) típusszintű egyezését az `event-stream` téma tesztje őrzi.
