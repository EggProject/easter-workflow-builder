...
**A mezőcsoportokat NEM `fieldset` és `legend` tagolja.** A `legend` a szegélybe rajzolódó, böngészőnként eltérően elhelyezett elem, aminek a doboz belső térközét és szélességét nem lehet megbízhatóan vezérelni. A csoport a `fieldset`/`legend` párral **azonos** szemantikát kap: `role="group"` plusz `aria-labelledby`. Ez a W3C WAI hivatalos, dokumentált alternatívája (WAI Forms Tutorial "Grouping Controls", <https://www.w3.org/WAI/tutorials/forms/grouping/>; WCAG 2.2 ARIA17 technika, <https://www.w3.org/WAI/WCAG22/Techniques/aria/ARIA17>); az ARIA17 egyetlen megkötése is teljesül, mert a panelen nincs rádiógomb, csak jelölőnégyzet és szöveges mező.

**TILOS A CARD IN CARD, és nincs dupla belső térköz** (felhasználói kérés, 2026-09-09). A panel egy kártya alakú `.resizable-group` belsejében ül, tehát minden belső, szegély plusz lekerekítés plusz saját háttér kombinációjú doboz második kártya lenne. A korábbi `.inspector-section` ezen felül **nem is a design system `.card` osztálya volt**, hanem egy attól eltérő tokenkombinációjú, kitalált osztály (`docs/research/2026-09-08-design-system-audit.md` 6. szekció). Megszűnt. Ami a helyére lép, kizárólag kettő lehet:

- **`AccordionItem`** (`packages/ui` `accordion` téma, a design system átemelt komponense) a ritkán szerkesztett csoportoknak: natív `<h3>` fejlécbe ágyazott natív gomb `aria-expanded` és `aria-controls` attribútummal, `role="region"` törzs a fejlécre mutató névvel, natív `hidden` rejtéssel. A natív gomb miatt billentyűzetről (`Enter`, `Space`) is nyitható és zárható. A `plain` variáns épül be, nem a `bordered`/`separated`, mert azok kártya alakú dobozt rajzolnának.
- **megnevezett, doboz nélküli csoport** (`InspectorFieldGroup`, `.node-inspector__group`) a kisebb egységeknek (jelölőnégyzet lista, egy `agents` bejegyzés): tisztán elrendezés, a design system saját `.field__label` címkéjével, kártya chrome nélkül.

**A hibajelzés EGY szintű: a hibaüzenet kizárólag a hibás mező ALATT áll**, `aria-invalid` és `aria-describedby` kötéssel. A panel tetején álló összesítő **megszűnt** (felhasználói kérés, 2026-09-09; korábban a spec két szintű, "error summary" mintát írt elő). A hibaüzenet helye **mindig fenn van tartva** (a mező grid elrendezésének harmadik, legalább egy sornyi sora), tehát a megjelenése nem tolja el a panel többi mezőjét.

**A megjelenés szabálya**: a hibaüzenet akkor látszik, ha a mező **érintett** (`blur` már megtörtént) **és érvénytelen**, VAGY ha a gráf mentését már **legalább egyszer megkísérelték**, az sikertelen volt, **és a mező érvénytelen**. A szabály egy helyen, a `packages/ui` `field-error-visibility` témájában él, és a `TextField`, a `SelectField` és a `TextAreaField` egyaránt onnan veszi; a "már megkísérelték a beküldést" tényt a `NodeInspector` `isSaveAttempted` propja adja tovább a `FieldErrorVisibilityContext` kontextuson.

**A hozzáférhetőségi része hivatalos forrásokon áll.** Ugyanez az érték dönt az `aria-invalid` kitételéről is, mert a WCAG 2.2 ARIA21 technika szó szerint kimondja: "The aria-invalid attribute should not be set to 'true' before input validation is performed", és "Setting aria-invalid to 'false' is the same as not placing the attribute for the form control" (<https://www.w3.org/WAI/WCAG22/Techniques/aria/ARIA21>). A mező és az üzenet összekötése `aria-describedby`, a W3C WAI Forms Tutorial "User Notifications" szerint: "form fields can be associated with the corresponding error message using aria-describedby" (<https://www.w3.org/WAI/tutorials/forms/notifications/>). Ugyanez a tutorial nevezi meg a MEGJELENÉS eljuttatásának módját is (`role="alert"` konténer), ezért a látható üzenet ezt a szerepet viseli. **NEM MEGERŐSÍTETT, ezért nem is állítjuk**, hogy a WAI előírná, mikor kell megjelennie a hibaüzenetnek: a WCAG 3.3.1 Understanding lapja szerint "This criterion does not mandate any particular way in which errors should be displayed" (<https://www.w3.org/WAI/WCAG22/Understanding/error-identification>), tehát a fenti időzítés felhasználói termékdöntés, amit a hivatalos anyagok megengednek.

A mezőkhöz egy React kontextus viszi le a hibatérképet, hogy a tíz típus szerinti komponens szignatúrája ne hízzon egy csak áttovábbított proppal; a `join` `ai_synthesis` módja egy kötelező útvonal előtag propon át kapja meg, hogy a mezői a `settings` alatt állnak.

**A CSOPORTOSÍTÁS ELVE, tíz csomópont típusra végiggondolva** (felhasználói kérés, 2026-09-09: "gondold vegig a csoportositast hogy mi az amit valoszinuleg a userek keveset irnanak at es azokat rakd accordion panelbe es csak a fontos mezok legyen elol panel nelkul"). A besorolás kritériuma: **elöl, panel nélkül az áll, amit a felhasználó a típus majdnem minden példányán megír; panelbe az kerül, ami felülírás vagy csak olvasható érték, tehát alapesetben érintetlen marad.** Egy panel akkor éri meg, ha legalább három, ténylegesen ritkán szerkesztett mezőt fog össze - egyetlen mező elrejtése több chrome-ot adna (fejléc plusz kattintás), mint amennyi helyet megspórol.

| Típus            | Elöl, panel nélkül                                                       | Összecsukható panelben                                                                                                   |
| ---------------- | ------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------ |
| `start`          | a bemeneti mezők listája (a node teljes tartalma)                        | nincs                                                                                                                    |
| `agent_step`     | `promptTemplate`, `systemPrompt`, `providerId` plusz az öröklés leírása  | "Modell és futási korlátok", "Eszközök és környezet", "Al-agentek (agents)", "Skillek és MCP szerverek (csak olvasható)" |
| `branch`         | `expression`, az ágak listája, `defaultBranchKey`                        | nincs (a `defaultBranchKey` ritkább, de egyetlen mező)                                                                   |
| `fan_out`        | `itemsExpression`, `branchLabelTemplate`                                 | nincs (két mező, mindkettő a szétosztás lényege)                                                                         |
| `join`           | `mode` plusz a módhoz tartozó beállítás                                  | `ai_synthesis` módban az `AgentStepConfigFields` saját négy panelje                                                      |
| `loop`           | `maxIterations`, `continueExpression`                                    | nincs (a `maxIterations` kötelező, alapérték nélkül)                                                                     |
| `human_approval` | `title`, `bodyTemplate`, `timeoutMs`                                     | nincs (a `timeoutMs` ritkább, de egyetlen mező)                                                                          |
| `error_handler`  | `maxAttempts`, `backoffMs`, `handledErrorKinds`                          | nincs (a három EGYÜTT maga az újrapróbálkozási szabály)                                                                  |
| `sub_workflow`   | `targetWorkflowId`, `inputMapping`                                       | nincs (mindkettő kell a híváshoz)                                                                                        |
| `script`         | a motor elutasítás figyelmeztetése, `source`, a csak olvasható `runtime` | nincs (a figyelmeztetést tilos elrejteni)                                                                                |

Az `agents` bejegyzés űrlapja ugyanezt az elvet követi eggyel lejjebb: elöl a `kötelező` csoport (`description`, `prompt`), panelben a "Modell és korlátok", az "Eszközök és környezet" és a "Skillek és MCP szerverek (csak olvasható)". A bejegyzés kibontása MARAD a saját, `aria-expanded` jelzésű gombján, nem `AccordionItem`: az `AccordionItem` fejléce egyetlen natív gomb, a bejegyzés fejléce viszont az átnevező mezővel és két további gombbal együtt áll, interaktív vezérlőt pedig nem lehet gombba ágyazni.

**A panel minden gombja `sm` méretű**, a bezáró gomb pedig **ikon gomb** (`.btn--icon`), aminek a hozzáférhető nevét `aria-label` adja, mert nincs látható szövege (felhasználói kérés, 2026-09-09).

Az `agent_step` és a `join` `ai_synthesis` módja a teljes `AgentStepConfig` alakot szerkeszti, a fenti elv szerint:

| Hely                                              | Mezők                                                                                                              | Vezérlő                                                   |
| ------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------- |
| elöl, panel nélkül                                | `promptTemplate`, `systemPrompt`, `providerId`                                                                     | `TextAreaField`, mód választó plusz szöveg, `SelectField` |
| "Modell és futási korlátok" panel                 | `modelId`, `sessionMode`, `maxTurns`, `maxBudgetUsd`, `effort`, `thinking`, `permissionMode`, `enabledEngineHooks` | szövegmező, szám mező és `SelectField`, a zárt listákból  |
| "Eszközök és környezet" panel                     | `allowedTools`, `disallowedTools`, `agentTools`, `cwd`, `additionalDirectories`, `sandbox`, `structuredOutput`     | lista szerkesztő, jelölőnégyzet és nyers JSON szerkesztő  |
| "Al-agentek (agents)" panel                       | `agents`                                                                                                           | kulcsonkénti lista, saját szerkesztővel                   |
| "Skillek és MCP szerverek (csak olvasható)" panel | `skills`, `mcpServers`                                                                                             | olvasható érték, megnevezett okkal                        |

**Minden többsoros mező a design system `.textarea` komponensét viseli** (`packages/ui` `textarea` téma), nem az egysoros `.input` osztályt. A komponens korábban sosem lett átemelve, és a panel tizenhat helyen az `.input` osztályt adta `<textarea>` elemre, egy saját `resize: vertical` toldással; ez volt a felhasználó input panaszának oka (`docs/research/2026-09-08-design-system-audit.md` 4.4).
...
**Az ötödik állapotdarab a vászoné, nem a képernyőé: a mért csomópont méret** (2026-09-05-i mérés, valós chromium). A vezérelt mód ára, hogy a `nodes` prop minden renderen a domain alakból épül újra, tehát a React Flow saját `measured` mezője nélkül érkezik. A könyvtár `adoptUserNodes` függvénye viszont a belső node-ot a propként kapott objektumból építi újra, és a mért méretet is onnan veszi; ha az nem jut vissza, a mérés minden körben elvész. **A mért következmény**: mindkét csomópont tartósan `visibility: hidden` maradt (a `nodeHasDimensions` sosem teljesült), egyetlen él sem rajzolódott ki, és a méréshurok nem konvergált (1920 `observe` és 960 callback hívás az első másodpercben). Ezért a vászon a `dimensions` típusú változást **nem** vezeti vissza a domain állapotba, hanem saját, csomópont azonosító szerinti mért méret térképben tartja, és azt írja vissza a `nodes` propra.

**Ez nem sérti a 12.2 szabályt.** A mért geometriától függő ágak (`van-e mért méret`, `hordoz-e méretet a változás`) tiszta függvényben állnak, szintetikus bemenettel közvetlenül tesztelve, `measured.width`, `measured.height` és `getBoundingClientRect()` hivatkozás nélkül - pontosan a 12.2 által előírt `menu` téma minta szerint.

**A hatodik állapotdarab ugyanebből az okból a vászoné: a kiválasztott élek halmaza** (2026-09-06-i mérés, valós chromium). A kiválasztás pontosan olyan nézeti állapot, mint a mért méret: a `WorkflowEdgeInput` nem hordoz `selected` mezőt, tehát a vezérelt oda-vissza leképezés elnyeli. A csomópontnál ez nem tűnt fel, mert azt a `selectedNodeId` képernyő állapot amúgy is visszaírja a `Node.selected` mezőre; az élnél viszont nem volt ilyen visszaírás. **A mért következmény**: az élre kattintva az elem `class` attribútuma `selected` osztály nélkül maradt, ezért a React Flow dokumentált `deleteKeyCode` alapértelmezése (`Backspace`) sem talált törölni valót, és az él a billentyű lenyomása után is a vásznon maradt. A vászon ezért a `select` típusú él változást **nem** vezeti vissza a domain állapotba, hanem saját, azonosító halmazban tartja, és azt írja vissza az `edges` propra.

**Ugyanez vonatkozik a kiválasztás megszűnésére a csomópontnál.** A React Flow egy élre kattintva minden kiválasztott csomópontra `select: false` változást ad (`addSelectedEdges`), és ez a változás ugyanúgy elveszett a domain leképezésen: a csomópont kiválasztva maradt, tehát a `Backspace` a kiválasztott él helyett a **csomópontot** törölte, és vele kaszkádban a rá kötött éleket. A vászon ezért a csomópont `select` változását is nézeti állapotként kezeli, és a kiválasztás megszűnését a `selectedNodeId` gazdájának jelenti. A kiválasztás **felvétele** változatlanul az `onNodeClick` propon megy: a könyvtár a `select: true` változást a saját kattintás kezelőjében, az `onNodeClick` prop hívása **előtt** küldi el, tehát a két út nem versenyzik.

**A törlés billentyűje a könyvtár dokumentált alapértelmezésén marad** (`deleteKeyCode`, `Backspace`). Saját szám és saját billentyű leképezés nincs: a `.claude/CLAUDE.md` 4. szekciója szerint dokumentált szabály nélkül nem adunk értéket, itt pedig a dokumentált alapérték pontosan azt teszi, amit a felület igényel. A törlés a szerkesztett gráfot módosítja, tehát a mentetlen jelző rá is a szokásos úton, az `isGraphDirty` összehasonlításból áll elő.
...
**A panelek aránya perzisztálódik (2026-09-09-i felhasználói kérés, a korábbi döntés kimondott felülírása).** A korábbi szöveg itt azt mondta ki, hogy a `localStorage` írása "fel nem merült igény" lenne. Azóta felmerült, szó szerint: "legyen a layout ha lehet resizable, igy a user tudja beallitani maganak hogy mekkora tavolsagot akar. Ezt localstorage -be le kell menteni es betolteni a usernek amikor a layoutot hasznalja". A `.claude/CLAUDE.md` 5. szekció tilalma a **nem kért** konfigurálhatóságra vonatkozik, ez viszont kért, tehát a tilalom nem áll fenn.

A megvalósítás a `Resizable` komponens opcionális `onSizesChange` propja (értesítés, nem vezérelt `sizes` prop): a méret továbbra is a komponensé, a hívó ebből legfeljebb perzisztálni tud. A gráf szerkesztő ezt teszi (`apps/web/src/graph-editor/graph-editor-layout.ts`, `eggGraphEditorLayout` kulcs, a téma váltó `eggTheme` kulcsának betűzési mintája szerint). A hiányzó, a hibás JSON, a rossz alakú és a dobó tárolás (privát ablak, letiltott tárolás, tele kvóta) ág **mind** az alapértelmezésre esik vissza, a felület nem törik el; a `try`/`catch` mindkét irányban kötelező. A futás nézet ugyanezt a propot használhatja, ha megépül, saját kulccsal.

Az arány az `aria-valuemin={5}` és `aria-valuemax={95}` korlátok között mozog, ahogy a forrás komponens adja.
...
| `sdk_result`                                                                       | a négy token szám és a `numTurns`, plusz **providerfüggő költség mező** (lásd lent, user döntés 2026-09-23, pontosítva)                 |
...
**A költség providerenként eltérően jelenik meg (user döntés, 2026-09-23, pontosítva).** A korábbi, mindkét providerre egységes "Költség (SDK becslés)" felirat helyett a user ugyanaznapi, pontosító döntése ("MiniMaxnál ne látszódjon") szerint a megjelenés a lépés ténylegesen feloldott providerétől függ: `claude-subscription` mellett a mező látszik: összecsukva a design system `.accordion__meta` szlotjában **csak az összeg** áll, a Code szereppel (például `$0.0873`), a "Költség (SDK becslés)" felirat és a jelentését kimondó magyarázat a kinyitott törzsben (user döntés, 2026-09-24; a felirattal együtt a meta a 400 pixeles panelen a címkét is félig kitakarta, mérés: `docs/research/2026-09-23-transcript-panel-meresek.md` 14. szekció); `minimax` mellett a mező **sehol nem jelenik meg**, helyette a kinyitott nézetben egy mondat mondja ki, hogy erre a providerre az SDK nem számol valós költséget. **A megkülönböztetés forrása a lépés feloldott `StepRunRecord.providerId` mezője** (`packages/protocol/src/run/step-run-record.ts`), amit az engine a háromszintű feloldás eredményeként (`resolveEffectiveProvider`, SPEC-004 11.1) ír a lépés futás sorába a `beginStepRun` hívásban. A `RunEventRecord` maga NEM hordozza a providerId-t (a drótszintű mezők között nincs ilyen), ezért a sort megjelenítő `RunEventRow` ezt külön, kötelező propként kapja; a feloldás (a `record.stepRunId` alapján a hozzá tartozó `StepRunRecord.providerId` kikeresése) a transcript panel feladata (T-009-25). A modell név szövegéből való találgatás (pl. `includes('MiniMax')`) kizárt, mert törékeny és a modell név felülírható. Bizonyíték a két jelentés eltérésére, két forrással és az összes SPEC-000 artefaktum újraszámolásával: `docs/research/2026-09-23-sdk-koltseg-becsles.md`; a `providerId` forrásmezőre saját kódolvasás (`packages/protocol/src/run/step-run-record.ts`, `packages/engine/src/node-executor/begin-step-run.ts`, `packages/engine/src/provider-resolution/resolve-effective-provider.ts`). Normalizált, összegezhető költség oszlop továbbra sincs (SPEC-003, SPEC-004 F-8).
...
**Pontos alsó igazítás egyforma sormagassággal (2026-09-24).** A `scrollToRow` a még ki nem rajzolt sorokat a becsült magassággal számolja, és a pinelt `react-window@2.3.1` a görgetés után nem igazít, ha a kirajzolt sor mért magassága eltér a becsléstől. Ezért minden összecsukott sornak pontosan a becsült magasságúnak kell lennie: a sor fejléce pontosan egy szövegsor magas (`run-event-row.css`, `height: calc(1lh + 2 * 16px)`, a `--ep-text-small` sormagassága plusz az `accordion.css` két belső margója), így a "Nem tárolt" jelvény (a design system egyetlen mérete) sem növeli az átmeneti sort, hanem a belső margóba nyúlik; a tárolt és az átmeneti sor is 53 pixel. Az elvárás: követés közben és az "ugrás az aljára" gomb után az utolsó sor **teljes egészében** látszik, az alja a lista alján. A korábbi, a mért magasság minden változása után újragörgető gépezet (a `dfcaa38` és a `d598677` commitban) ezzel kivezetve. Mérés, elvetett alternatívák: `docs/research/2026-09-23-transcript-panel-meresek.md` 13. és 16. szekció.

**Kinyitás élő stream közben (2026-09-24, a kinyitásra 2026-09-25-én módosítva).** Egy sor fejlécének kattintása (minden kinyitási út `click` eseményt ad: egér, `Enter`, `Space`, csak `click`) szünetelteti a követést: a közben érkező sor nem görget, hanem a gomb számába kerül, és a szünet alatti jelentés a követést nem kapcsolhatja vissza. **A kinyitás szünete** addig tart, amíg a felhasználó vissza nem ér az aljára vagy meg nem nyomja az "ugrás az aljára" gombot (user döntés 2026-09-25, lásd lent). **A becsukás szünete** a mérésig tart: a sor új magasságát a lista csak a következő mérés után ismeri, addig a jelentései a becsukás előtti elrendezést írják le; a mérés után a predikátum dönt, a mért magassággal számolt utolsó jelentés szerint. Hogy a kattintás kinyitás vagy becsukás, azt a fejléc kattintás előtti `aria-expanded` értéke dönti el: a lista kattintás figyelője a React saját kezelője előtt fut, mert a telepített `react-dom` a gyökér tárolón figyel (`listenToAllSupportedEvents`). A váltások fejlécenként párosodnak: ha egy fejléc a szünet alatt páros számú kattintást kap (egy képkockán belüli ki-be csukás, vagy a kinyitott sor későbbi becsukása), a sor ugyanaz, mint a szünet előtt, és ha minden váltás visszaállt, a szünet a predikátum szerint zárul. A szünetnek mindig van kilépése: a mérés (becsukásnál), a páros kattintás, az "ugrás az aljára" gomb és a kézi visszatérés az aljára, és a nem látott sorok száma közben is nő. A követés visszakapcsolása önmagában nem görget: görgetést csak új esemény, átméretezés, a lista csatolása és a gomb indít.

**A kézi visszatérés az aljára is lezárja a szünetet (user döntés 2026-09-24).** Ha a szünet alatt egy jelentés szerint az utolsó sor nem látszik, és egy későbbi jelentés szerint a predikátum újra igaz (`visibleRows.stopIndex === rowCount - 1`), a szünet lezárul, a követés visszakapcsol, és a nem látott sorok száma nullázódik. Ez akkor is zár, ha a mérés sosem jön: egy fülváltás a kinyitott sort a mérése előtt leszereli (a rejtett sor 0 magasságát a `useDynamicRowHeight` nem tárolja), és a gyorsítótár utána nem változik; enélkül 375 pixelen a kinyitással egy feladatban váltott fül után a lista a kézi görgetés ellenére sem követett. Az alj elhagyásának feltétele azért kell, mert a kattintás előtti görgetés késve érkező jelentése a mérés előtti gyorsítótárral az utolsó sort láthatónak mutathatja; egy ilyen jelentés nem visszatérés. Ugyanezért nem visszatérés a kinyitott utolsó sor törzsének görgetése sem: az utolsó sor közben végig látszik, a lista az alját az első, nem követett új sorral hagyja el. Pixel küszöb és időzítő nincs. Mérés és regresszió: `docs/research/2026-09-23-transcript-panel-meresek.md` 17. és 18. szekció.

**Az utolsó sor kinyitása is megállítja a követést (user döntés 2026-09-25).** Ha a lista az alján áll, és a felhasználó a legutolsó sort nyitja ki, majd új sor érkezik, a kinyitott sor a helyén marad, és megjelenik az "Ugrás az aljára (N új esemény)" gomb, időzítéstől függetlenül, minden kinyitási úton (egér, `Space`, `Enter`, csak `click`). A korábbi szabály szerint a kinyitott utolsó sor a mérés utáni jelentés szerint "az alján állt", a követés megmaradt, és a következő sor a kinyitott sort a törzs plusz az új sor magasságával vitte feljebb (mérve: 309 = 256 + 53 pixel); ha viszont az új sor a mérés commitjában, a lezárás előtt érkezett, a sor a helyén maradt, tehát az eredmény időzítésfüggő volt. Ugyanez a szabály áll a nem utolsó sorra is: ha a kinyitott törzs rövid, és az utolsó sor a kinyitás után is látszik, a követés szintén szünetel. A becsukás viselkedése nem változott. Ezzel a korábbi "Javaslat, nem döntés" pont (a kinyitott utolsó sor helye) lezárult. Mérés és regresszió: `docs/research/2026-09-23-transcript-panel-meresek.md` 18. szekció.

**Nincs böngésző görgetés rögzítés (user döntés 2026-09-24).** A lista `overflow-anchor: none` értéket kap: a CSS Scroll Anchoring spec szerint ekkor a görgető dobozban nem választódik horgony, tehát a böngésző nem igazítja a görgetési helyet. A görgetést kizárólag a hook végzi. Bekapcsolt rögzítés mellett folyamatos streamnél a véletlen fázisú kinyitások egy részében a lista a hook nélkül 36 pixelt görgetett (mérve 9/80, kikapcsolva 0/80); időzítő nélküli lépéssorral a jelenség nem állítható elő, ezért a regressziós teszt a lista kiszámított `overflow-anchor` értékét őrzi (research 17. és 18. szekció).

**Ami ebből következik, kimondva.** A kinyitott sor a helyén marad, amíg a felhasználó vissza nem ér az aljára vagy meg nem nyomja a gombot, akár az utolsó sor az, akár egy korábbi. Az "ugrás az aljára" gomb a lista fejlécében jelenik meg, ezért a megjelenésekor a lista a gomb sáv magasságával lejjebb kerül (mérve: 36 pixel 1440x900-on); a sorok a listához képest a helyükön maradnak. Mérés és regresszió: `docs/research/2026-09-23-transcript-panel-meresek.md` 15-18. szekció.
...
**A megvalósítás pontosításai (PLAN-009 T-009-26, 2026-09-24).** A jelölés a design system `Badge` komponensének `outline` változata a sor `meta` szlotjában: a látható szövege ("Nem tárolt") már maga kimondja, hogy a sor nem kerül tárolásra, a `title` szövege a teljes magyarázat (1. szabály); az `outline` az egyetlen `Badge` változat, aminek a színe téma tokenből jön, tehát mindkét témában a felülethez illeszkedik. Az átmeneti keretnek `origin` mezője sincs (SPEC-005 5.4), a sor jelölő oszlopa (7.2 4. pont) viszont eredetet kér: ezt a `kind` határozza meg, a SPEC-003 6.4 felosztása szerint, egy kimerítő `Record` leképezésen át. A keret által nem hordozott normalizált mezők (az SDK üzenet típusa, az eszközhívás, a token számok) "nincs adat" értéket kapnak, a nyers `payload` változatlanul átkerül. A kulcs a perzisztált soroktól előtaggal különül el (`event-<id>` kontra `transient-<n>`), tehát a két kulcstér nem ütközhet. A 2. szabály mondata a transcript panel tetején, a lista és a várakozás jelzése fölött áll, a meglévő halvány kis szöveg mintájával, doboz nélkül, és a futás nézése alatt végig a helyén marad.
...
| Hely                     | Mit mutat                                                                                                                                                                                                                                                                                                                                                        |
| ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| a rajzon a csomóponton   | `waiting_approval` állapot, és hogy mióta vár (`PendingApproval.requestedAtMs`), abszolút időpontként: "10:32:05 óta vár"                                                                                                                                                                                                                                        |
| a futás nézet fejlécénél | a vezérlő sáv `warning` jelvénye ("jóváhagyásra vár"), az állapot jelvény mellett, ugyanabban a sorban                                                                                                                                                                                                                                                           |
| a jóváhagyás panelen     | a transcript sávban, a transcript fölött, egyszerre EGY jóváhagyás: felül a lapozó ("k / n"), alatta görgethető törzsként a kimondott figyelmeztetés, hogy a döntés visszavonhatatlan, és a látott jóváhagyás `title`, `body` és formázott `payload` értéke; alul a tapadó akciósávban csak az ő két gombja, "Jóváhagyás" és "Elutasítás", és a döntés eredménye |
...
**A megvalósult alak (PLAN-009 T-009-27 és annak javításai, 2026-09-24 és 2026-09-25, két független ellenőrzés nyomán).**

1. **A panel helye és a döntés gombjai.** A PLAN-009 5. szekciójának F6 sora szerint a jóváhagyás "a futás nézet fejlécébe és a transcript mellé" kerül. A fejlécben a jelzés áll (a fenti táblázat második sora), a döntési panel a transcript sávban, a transcript FÖLÖTT. A kettő a sávot egyenlő `flex-grow` szerint osztja meg, a panel legfeljebb a saját tartalmának magasságát kapja (`max-height: max-content`). **Mérve már egyetlen jóváhagyás tartalma is nagyobb a sáv felénél** mindhárom mért méreten, tehát a panel a gyakorlatban a sáv felét kapja. A vászon magassága nem függ a jóváhagyások számától: valódi Chromiumban mérve 0, 1, 4 és 10 jóváhagyással azonos 1440x900, 1440x600 és 375x812 méreten, és az `.app-content` nem görget (`docs/research/2026-09-24-jovahagyas-panel-helye.md` 8. szekció).

   **Egyszerre egy jóváhagyás** (user döntés 2026-09-25, a 2026-09-24-i döntési sáv helyett). A korábbi alak minden jóváhagyásnak egy döntési sort adott a panel alján; egy független ellenőrzés mérte, hogy a sorok nem köthetők a jóváhagyásukhoz (keskeny panelben a cím csonkolt, `fan_out` ágakon a cím minden ágon azonos, SPEC-004), négy jóváhagyásnál 1440x600-on a tartalomból 31 pixel maradt, tíznél nulla, és a "visszavonhatatlan" figyelmeztetés eltűnt (research 8. szekció "előtte" oszlopa). A panel ezért egyszerre egy jóváhagyást mutat, a design system két elemével:

   - **Lapozó** (`pagination`, bájtra azonosan átemelve, `packages/ui`): felül, nem görget, "k / n" alakban mutatja a helyet, az aktuális oldal `aria-current="page"`, a `<nav>` neve "Jóváhagyások lapozása", a két szélső gomb "Előző" és "Következő". A szomszéd oldalszámok száma nulla, mert a forrás alapértéke (1) mellett 375 pixelen, tíz jóváhagyásnál a "k / n" két sorba törik (mérve).
   - **A drawer törzs és lábléc szerkezete** (`drawer`, a CSS bájtra azonosan átemelve): a `.drawer__body` görgethető törzsében a "visszavonhatatlan" figyelmeztetés és a látott jóváhagyás teljes címe, törzse és `payload` értéke; a `.drawer__footer` tapadó akciósávjában csak az ő két gombja és a döntés eredménye. **A `Drawer` komponens egésze nem használható**: a forrás modális (`role="dialog"`, `aria-modal`, háttér réteg, fókusz csapda, a lap többi részének `inert` elszigetelése, a `body` görgetés zárja, `position: fixed`), a panel viszont a lap többi részével együtt használt, nem modális felület. A két osztály a forrás CSS-ében nem a `.drawer` alá szűkített, tehát önállóan is érvényes; a flex oszlop szülőt, amit a forrásban a `.drawer` ad, itt a panel szakasza adja. Kitalált osztály a szerkezethez nincs; az egyetlen fogyasztó oldali felülírás az akciósáv `align-items: center` értéke, hogy a több sorba törő eredmény szöveg a gombokat ne nyújtsa meg (`approval-prompt.css`).

   A gombok tehát mindig a LÁTOTT jóváhagyásra döntenek, és mérve 1, 4 és 10 jóváhagyásnál mindhárom méreten, mindkét témában teljesen látszanak görgetés nélkül; a görgethető törzs 1440x900-on 233, 1440x600-on 83, 375x812-n 150 pixel, a jóváhagyások számától függetlenül (research 8. szekció). **A kiválasztás szabálya** (`select-shown-approval.ts`, `use-approval-selection.ts`): alapból a legrégebbi kérés látszik (a lista a kérés időpontja szerint rendezett); a kiválasztás a jóváhagyás azonosítója, nem a helye, és a látott jóváhagyás azonnal rögzül, tehát élő frissítéskor egy elé érkező vagy előtte kikerülő jóváhagyás csak a "k" számot változtatja, a látott jóváhagyást nem; ha maga a látott jóváhagyás kerül ki (döntés nélkül lezárult, vagy egy másik lapon döntöttek róla), a legrégebbi látszik. **Döntés után a kiválasztás nem lép tovább**: az eredmény a látott jóváhagyás akciósávjában marad a futásváltásig (3. pont), a következőhöz a lapozóval lehet lépni. Az automatikus továbblépés elvetve, mert az eredményt azonnal elrejtené (ellentmondana a 2026-09-24-i user döntésnek), és egy a user kattintásától független ugrás lenne. Lapozáskor a törzs a tetejéről indul.

   **A "transcript mellé" pontos helye a tervben kétértelmű** (lehetne a transcript sávon belül, vagy egy harmadik, önálló sáv); a megvalósítás a legkevésbé invazív olvasatot követi, ami a meglévő két sávos `Resizable` és `Tabs` elrendezést nem változtatja. Ez nyitott pont, a 14.2 táblázat O-9 tétele.

2. **Élő frissítés.** A lista a 6.2 lépés futás listájának mintájára frissül: a veszteségmentes `subscribeToFrames` úton érkező jelző keretre (a nézett futás élő `approval_requested`, `approval_decided`, `step_finished`, `run_finished`, `run_interrupted` kerete és a `replay_complete`) összevont újratöltés indul, egyszerre legfeljebb egy kéréssel (`apps/web/src/approval-prompt/is-approval-list-change-frame.ts`, `use-pending-approvals.ts`). A döntés válasza után is újratöltődik, a kimeneteltől függetlenül.
3. **A döntés eredménye.** Egy elfogadott, vagy `conflict`/`not_found` hibával végleg elbukott döntés gombjai soha nem kapcsolnak vissza, akkor sem, ha a lista újratöltése elbukik; a jóváhagyás az eredménnyel együtt lapozható marad akkor is, ha a friss lista már nem tartalmazza. **Külön nyugtázás nincs** (user döntés 2026-09-24): az eredmény (siker vagy hiba) a futás nézet elhagyásáig vagy egy másik futásra váltásig látszik; magától nem tűnik el, ezt a Playwright Clock API-jával lefuttatott egy perc után is állító e2e teszt őrzi (`approval-prompt.spec.ts`). Csak az átmeneti hiba (hálózati hiba, 502, 503) engedi az újrapróbálást, mert ott nem tudni, megtörtént-e a döntés. Az eredmény a tapadó akciósávban, a gombok előtt áll, tehát görgetés nélkül látszik; keskeny panelben több sorba törik (a `conflict` üzenete 375 pixelen), és ennyivel a törzs kisebb (research 8. szekció).
4. **"Visszavonhatatlan".** A görgethető törzs tetején a design system `Alert` blokkja (`warning`) mondja ki: "A döntés visszavonhatatlan"; mérve minden méreten görgetés nélkül látszik, amíg nincs döntés.
...
**A három sáv között JS választ, nem CSS, és a JS literálokra saját sodródás védelem tartozik (2026-09-15, T-009-22).** A fenti kötött szabály (csak token érték lehet literál) változatlan, de a `media-query-breakpoint-invariant` teszt kizárólag CSS fájlokat vizsgál, a sáv váltás pedig nem lehet CSS: a `--ep-screen-md` alatti sáv `Tabs` komponenst kér a `Resizable` helyett (más DOM, más ARIA szemantika), a két osztott sáv között pedig a `Resizable` `direction` propja dönt, ami az elválasztó `aria-orientation` értékét is átállítja, tehát egy puszta CSS `flex-direction` váltás hamis állítást hagyna a hozzáférhetőségi fában. A választás ezért a `useRunViewLayoutBand` hook `matchMedia` bekötésén megy, a két query literál az `apps/web/src/run-view/run-view-layout-band.ts` fájlban áll, és az `apps/web/src/run-view/run-view-layout-band.spec.ts` regressziós tesztje **magából a `breakpoints.css` fájlból** olvasott token értékkel hasonlítja őket. A `run-view.css` fájlban emiatt egyetlen media query sincs.
...
**A vászon minden sávban a teljes rendelkezésre álló területet tölti ki**, a SPEC-007 5.2 "faltól falig" követelménye szerint; a `.app-content` magassága a viewport magasságából és a `60px` bar magasságából számítódik, mindkettő a design system saját értéke. **Ebből következően a futás nézeten is NULLA az `.app-content` belső margója** (2026-09-15, T-009-22), ugyanazzal az indoklással és ugyanazzal a hatókör szűkítéssel, mint a szerkesztőn: a `topnav-shell.css` egy önálló, `:has(> .run-view-screen)` szabálya nullázza, az oldalsó térközt pedig a screen két nem-vászon eleme (a fejléc és a rajzon nem szereplő lépés futások listája) kapja meg `--ep-layout-gutter` belső margóként. Az osztott elrendezés aránya a `localStorage` `eggRunViewLayout` kulcsán perzisztálódik, a szerkesztő mintája szerint (hiányzó, hibás JSON, rossz alakú és dobó tárolás esetén az alapértelmezés).

**A jóváhagyás panel mindhárom sávban a transcript oldalán áll, a vászon magassága nem függ tőle (T-009-27 javítás, 2026-09-24).** A két osztott sávban a transcript panelje fölött, a `--ep-screen-md` alatti fül sávban a "Transcript" fülön; a fejléc "jóváhagyásra vár" jelvénye mindhárom sávban látszik. A panel a transcript oldalt egyenlő `flex-grow` szerint osztja meg a transcripttel, legfeljebb a saját tartalma magasságáig, és a panelen belül egyszerre egy jóváhagyás látszik: a lapozó és a tapadó akciósáv nem görget, a törzs igen (8. szekció 1. pont), tehát media query és kitalált arány nincs hozzá. A korábbi alak (a panel a teljes osztott nézet fölött) a vásznat a jóváhagyások számával zsugorította, négy jóváhagyásnál nullára, és az `.app-content` görgetett; valódi Chromiumban mérve most 0, 1, 4 és 10 jóváhagyással a vászon magassága azonos (`docs/research/2026-09-24-jovahagyas-panel-helye.md` 8. szekció, regresszió: `apps/web/e2e/approval-prompt.spec.ts`).

**A gráf szerkesztőn az `.app-content` belső margója NULLA (2026-09-06, 2026-09-09-én kiterjesztve).** Az átemelt `.app-content` 80px alsó paddingje (`--ep-space-20`) a hosszú, görgetett listákra (workflow lista, futás előzmények) szánt légtér; a gráf szerkesztő viszont az `.app-content` tartalom dobozát tölti ki `flex: 1`-gyel, tehát a 80px levonódott a vászon és a dokkolt beállítás panel rendelkezésre álló magasságából, üres sávot hagyva alattuk a viewport aljáig. Ugyanez igaz az oldalsó 40px-re is: a felhasználó 2026-09-09-i kérése ("faltol falig, slim designt csinaljunk") szerint a szerkesztő nem egy hasáb az oldal közepén, hanem a teljes területet elfoglaló munkafelület. A `packages/ui/src/topnav-shell/topnav-shell.css` `.app-content:has(> .graph-editor-screen)` szabálya ezért kizárólag erre a screen-re **teljesen nullázza** a paddingot, mert egyedül a `GraphEditorScreen` gyökere ad megnevezett osztályt az `.app-content` közvetlen gyerekének. A szabály specificitása (0,2,0) magasabb, mint a `--ep-screen-md` alatti reszponzív `.app-content` szabályé (0,1,0), tehát a nullázás szűk nézetben is érvényes. Regresszió: `apps/web/e2e/graph-editor.spec.ts` és `apps/web/e2e/graph-editor-layout.spec.ts`.

**A szerkesztő elrendezése három ponton változik a felhasználó 2026-09-09-i kérésére.** Mindhárom a `docs/research/2026-09-08-design-system-audit.md` 9. szekciójában van tételesen dokumentálva, a design system elem oldali fedezetével együtt.

1. **Nincs kártya a szerkesztő körül, és tilos a card in card.** A design system `.resizable-group` szabálya kártya receptet visel (szegély, `--ep-radius-xl` lekerekítés, `--ep-bg-elevated` háttér), és ezen belül álltak a beállítás panel dobozai. Keret nélküli Resizable variáns a design systemben nincs (audit 8.1), a `resizable.css` pedig bájtazonossági teszt alatt áll, ezért a keret törlése a fogyasztó oldalán, erre az egy képernyőre szűkítve áll (`graph-editor-screen.css`), dokumentáló kommenttel.
2. **A státusz és a művelet gombok a képernyő aljához tapadó akciósávba kerülnek**, balra a státusz, jobbra a gombok. Ilyen minta a design systemben **nincs** (audit 5.4 és 8.1), ezért a `packages/ui/src/page-footer/` téma saját kiegészítés, a `.app-tn__bar` ragadós sávjának és a `.modal__footer--split` szerkezetének receptjéből, kizárólag design system tokenekkel. A felső eszköztár megszűnik.
3. **A gombok valódi split buttonban állnak, `sm` méretben (2026-09-09-én pontosítva).** A `.button-group` a design system létező eleme, bájtra azonosan átemelve (`packages/ui/src/button-group/`), a benne álló szerkezet pedig a forrás `button-group.html` "Mixed (split button)" mintáját követi, nem két egyenrangú, szöveges gombot: az elsődleges "Mentés" mellett egy szöveg nélküli, `.btn--icon` nyíl gomb (kötelező `aria-label`) nyitja a további műveletek (ma egyetlen: "Elrendezés") lenyíló menüjét, a már meglévő `packages/ui/src/menu/` `Menu`/`MenuItem` komponensen át. A `sm` méret a szabály a szerkesztő felületén; a modális és a popup gombok maradnak nagyobbak.

**Nagy felbontás: 4K-specifikus töréspont nincs.** A design system ezt szándékosan nem ad (audit 7.1, `eggproject-design` `SKILL.md`): a hivatalos minta a `max-width` sapka és a `--ep-layout-max-full` full-bleed szentinel. A szerkesztő ez utóbbit használja a `.app-tn` láncon át (`topnav-shell.css`, SPEC-007 5.2), tehát FHD-n, QHD-n és afölött is a teljes szélességet kapja. Új media query a szerkesztőbe nem kerül: a `--ep-screen-md` (768px) sáv marad a `graph-editor-screen.css` egyetlen media queryje.
...
| Téma                     | Mi kerül bele                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `resizable`              | a design system `resizable` komponensének átemelése, `.tsx` alakban, bájtra azonos CSS-sel, az `Enter` billentyű pótlásával (6.1), `direction` proppal, és az opcionális `onSizesChange` értesítő proppal (6.1, 2026-09-09)                                                                                                                                                                                                                                                                                                                   |
| `button-group`           | a design system `button-group` komponensének átemelése, bájtra azonos CSS-sel: összeragasztott gombcsoport (split button), `role="group"` és kötelező `aria-label`, plusz egy dokumentált kiegészítés a `Menu` render-prop triggerének `.menu-anchor` csomagolójára (a `Menu` a triggerét spanbe csomagolja, ami különben kimaradna a fúziós/lekerekítési CSS-ből) (10., 2026-09-09)                                                                                                                                                          |
| `page-footer`            | **saját kiegészítés, nem átemelés**: a képernyő aljához tapadó akciósáv, balra a státusz, jobbra a gombok. A design systemben ilyen minta nincs, a recept a `.app-tn__bar` és a `.modal__footer--split` elemekből áll össze, kizárólag design system tokenekkel (10., 2026-09-09, `docs/research/2026-09-08-design-system-audit.md` 9.2)                                                                                                                                                                                                      |
| `textarea`               | a design system `textarea` komponensének átemelése, `TextAreaField` alakban, a `text-field` `.field` burkolójában. Saját `min-height`, `resize: vertical` és `.is-error` állapot; a CSS bájtra azonos a forrással, egyetlen hozzáadott deklarációval (`box-sizing: border-box`, a testvér `.input` szabállyal azonos, mért okból). A `--sm` variáns `size` propként KI van vezetve (`TextField`/`SelectField` mintájára, felhasználói kérés 2026-09-09: "slim mode alatt --sm varianst ertettem"), a `--lg`/`--ghost` nincs (5.2, 2026-09-09) |
| `accordion`              | a design system `accordion` komponensének átemelése: `Accordion` konténer és `AccordionItem` panel, natív `<h3>` fejlécbe ágyazott natív gombbal (`aria-expanded`, `aria-controls`), `role="region"` törzzsel, natív `hidden` rejtéssel és `defaultOpen` proppal. A forrás `variant`, `type="single"` és `cloneElement` alapú indexlistája nincs átemelve (5.2, 2026-09-09)                                                                                                                                                                   |
| `field-error-visibility` | a hibaüzenet MEGJELENÉSI szabálya egy helyen, mind a három mezőnek: `useFieldErrorVisibility` plusz a `FieldErrorVisibilityContext`, amin a beküldést ismerő űrlap leadja a "már megkísérelték a beküldést" tényt (5.2, 2026-09-09)                                                                                                                                                                                                                                                                                                           |
| `pagination`             | a design system `pagination` komponensének átemelése, bájtra azonos CSS-sel, a React változat minden propjával (`page`, `pageCount`, `total`, `pageSize`, `siblings`, `onChange`, `variant`, `className`), plusz egy `labels` kiegészítéssel a forrás beégetett angol szövegeihez (alapértéke a forrás szövege); a jóváhagyás panel lapozója (8. szekció 1. pont, 2026-09-25)                                                                                                                                                                 |
| `drawer`                 | a design system `drawer` komponensének CSS-e bájtra azonosan, és a törzs plusz lábléc szerkezete (`DrawerSections`, a forrás `children` és `footer` propjával); a modális `Drawer` egésze nem, mert a felület nem modális panelben használja (8. szekció 1. pont, 2026-09-25)                                                                                                                                                                                                                                                                 |

A fájlszám a SPEC-007 25. kritériumának szabálya szerint alakul: exportált egységenként egy megvalósítás és egy `.spec.tsx`, plusz a téma egyetlen `.css` fájlja. A `packages/ui` `dependencies` mezője **nem bővül**.

**A csomag téma mappáinak VÉGÖSSZEGE nem itt dől el.** A jelen spec eredetileg egyetlen új témát (`resizable`) írt elő, és ebből "huszonhárom" végösszeget mondott ki; azóta a 2026-09-08-i design system audit és a rá épülő 2026-09-09-i felhasználói kérések további témákat hoztak. A tényleges darabszám egyeztetése a PLAN-009 T-009-32 (F8) zárásának hatóköre, ugyanúgy, ahogy az `apps/web` oldalon (lásd `apps/web/CLAUDE.md`). Kitalált szám ide nem kerül.
...
**Hat tétel lezárult, és a döntés minden érintett forrásdokumentumba át van vezetve.** Az azonosítók nem számozódnak újra, hogy a hivatkozások ne csússzanak el; a nyitott tételek a 14.2 táblázatban állnak.
...
### 14.2 A nyitva maradt három tétel

Egyik sem zárható le tippeléssel. Mindegyiknél áll, mi a viselkedés addig, és mi zárná le. Az O-7 és az O-8 az F7/F8 fázis hatóköre (reszponzív e2e mérés, illetve az e2e küszöb újramérése), nem az F0-F3 fázisé; az O-9 a T-009-27 javításakor merült fel (8. szekció 1. pont), és user döntés zárja le.
...
| O-8 | Az e2e lefedettségi küszöb új értéke                                                                                                                  | a mai, mért érték marad érvényben; a küszöb csak akkor követheti lefelé a mért értéket, ha a fedetlen tételek száma egyetlen metrikán sem nő, és a csökkenést fedett kód törlése okozza, tételes levezetéssel (12.5 2. pont, `.claude/CLAUDE.md` 8. szekció, user döntés 2026-09-09) | a PLAN-009 záró lépésének újramérése, a research fájlba vezetve                                                                                               |
| O-9 | A jóváhagyás panel helye: a PLAN-009 F6 sorának "a transcript mellé" mondata a transcript sávon belüli helyet, vagy egy harmadik, önálló sávot jelent | a panel a transcript sávban, a transcript fölött áll (8. szekció 1. pont), a 10. szekció két osztott sávja és a fül sáv változatlan                                                                                                                                                  | a user döntése arról, kér-e önálló sávot; ha igen, az a 10. szekció sáv tervének módosítása (`docs/research/2026-09-24-jovahagyas-panel-helye.md` 5. szekció) |
...
16b. A panelen **nincs `fieldset` és `legend`**, és **nincs kártya alakú szakasz doboz**: a mezőcsoportok vagy `AccordionItem` panelek (`role="region"`, a fejlécre mutató névvel), vagy `role="group"` plusz `aria-labelledby` szemantikájú, doboz nélküli csoportok; a fán egyetlen `fieldset`, egyetlen `.inspector-section` és egyetlen `.card` elem sincs. Futtatott unit és e2e teszt igazolja, mind a tíz csomópont típusra (5.2).

16c. A hibaüzenet a **hibás mező alatt** jelenik meg, `aria-invalid` és `aria-describedby` kötéssel, `role="alert"` szerepben, és a megjelenése nem tolja el a szomszédos mezőt (a fenntartott hely miatt). A panel tetején **nincs** összesítő. Futtatott e2e teszt méri mind a hármat (5.2).

16d. Egyetlen mező sem lóg ki a panelből: futtatott e2e teszt méri, hogy a panel törzsében álló `.input`, `.select` és `.textarea` elemek jobb széle egyike sem nyúlik túl a törzs jobb szélén (5.2). 17. A lépés szintű `providerId` felülírás a panelen szerkeszthető, és a `null` érték mellett a panel megnevezi, melyik providert örökli a lépés. 18. A `CLIENT_ROUTE_TABLE` pontosan négy bejegyzést tartalmaz, mind a négy fix útvonal; az illesztő **továbbra sem** tartalmaz paraméteres szegmens ágat (SPEC-007 34. kritériuma).
...
37. Az `sdk_assistant` sor a `toolName` és a `toolUseId` értéket, az `sdk_result` sor a négy token számot és a `numTurns` értéket mutatja. **A költség providerfüggő (user döntés, 2026-09-23, pontosítva)**: `claude-subscription` provider mellett összecsukva a meta szlotban csak az összeg áll, a kinyitott nézetben pedig külön, megnevezett mezőként, "Költség (SDK becslés)" felirattal és a jelentését kimondó magyarázattal (7.1, user döntés 2026-09-24); `minimax` provider mellett a mező **sehol nem jelenik meg**, és a kinyitott nézetben egy mondat mondja ki, miért. A megkülönböztetés forrása a lépés feloldott `StepRunRecord.providerId` mezője, nem a modell név szövege. Futtatott teszt igazolja mindkét ágat, és azt is, hogy MiniMax mellett a DOM-ban sehol nem marad költség szöveg.
...
53. A `packages/protocol/src` alatt **tíz** téma mappa áll, plusz a barrel; egyetlen téma mappában sincs alkönyvtár. Mind a három `CLAUDE.md` `## Fájlok` táblázata minden témát felsorol, és a `bun run docs:check` zöld. A `packages/ui/src` és az `apps/web/src` téma mappáinak **végösszege** a PLAN-009 T-009-32 (F8) zárásának hatóköre: a 2026-09-08-i design system audit és a rá épülő 2026-09-09-i felhasználói kérések a jelen spec eredeti tervénél több témát hoztak (11.1, 11.2), és kitalált számot ide nem írunk.
...
### Az él kiválasztása és törlése (2026-09-06-i kiegészítés)

A kritérium a lista végén, a következő szabad sorszámmal áll, hogy a 7 ... 62 sorszámok ne csússzanak el; topikusan a fenti "A gráf szerkesztő" csoporthoz tartozik (5.5).

63. Az él kiválasztása és a kiválasztás megszűnése a **vászon nézeti állapota**, nem a domain gráfé: a `select` típusú változás nem jut vissza az `onGraphChange`-en, hanem a vászon saját halmazába, onnan pedig az `edges` prop `selected` mezőjére. Ugyanígy a csomópont `select` változása sem kerül a domain állapotba, és a kiválasztás megszűnése a `selectedNodeId` gazdájához jut vissza. **Futtatott e2e teszt igazolja** mind a hármat, valós chromiumban: egy élre kattintva az él megkapja a `selected` osztályt és a vászonra kattintva elveszti; a kiválasztott él a `Backspace` billentyűre törlődik, a többi él és a csomópontok érintetlenül maradnak, és a mentetlen jelző megjelenik; a kiválasztott csomópont ugyanígy törlődik, a rá kötött éllel együtt; kiválasztott csomópont mellett az élre kattintva a `Backspace` az **élt** törli, nem a csomópontot. A törlés billentyűje a könyvtár dokumentált `deleteKeyCode` alapértelmezése, saját érték nélkül (5.5).

### A szerkesztő elrendezésének újratervezése (2026-09-09-i kiegészítés)

A kritériumok a lista végén, a következő szabad sorszámokkal állnak, hogy a 7 ... 63 sorszámok ne csússzanak el; topikusan a fenti "Az osztott elrendezés és a reszponzív terv" csoporthoz tartoznak (10.).

64. **A szerkesztő faltól falig ér, kártya keret nélkül.** Az `.app-content` mind a négy belső margója `0px` ezen a screen-en, a `.graph-editor-screen` szélessége megegyezik a viewport szélességével, és a `.resizable-group` számított stílusában nincs szegély (`0px`), nincs lekerekítés (`0px`) és nincs saját háttér (`rgba(0, 0, 0, 0)`). **Futtatott e2e teszt igazolja** mind a hetet, valós chromiumban (`apps/web/e2e/graph-editor-layout.spec.ts`).

65. **A státusz és a művelet gombok a képernyő aljához tapadó `PageFooter` sávban állnak**, balra a státusz, jobbra a gombok; a felső eszköztár (`.graph-editor-screen__toolbar`) megszűnt. A sáv `position: sticky` és `bottom: 0`, a teljes szélességet kitölti, az alja a viewport aljához ér, és a vászon alja a sáv tetejéhez. A `page-footer` téma **saját kiegészítés**, mert ilyen minta a design systemben nincs; a fájl fejléc kommentje és a `docs/research/2026-09-08-design-system-audit.md` 9.2 pontja megnevezi, melyik két létező receptből áll össze, és hogy minden érték design system token. Futtatott unit és e2e teszt igazolja.

66. **A gombok valódi split buttonban, `sm` méretben állnak (2026-09-09-én pontosítva).** A `packages/ui/src/button-group` CSS-e bájtra azonos a design system forrásával (a `@import` sor nélkül), a csoport `role="group"` szerepet és `aria-label` nevet visel; a szerkezet a forrás "Mixed (split button)" mintáját követi: az elsődleges "Mentés" gomb mellett egy `.btn--icon` nyíl trigger (`aria-label="További műveletek"`, `aria-haspopup="menu"`) nyitja a `Menu`/`MenuItem` lenyíló panelt, ami az "Elrendezés" műveletet adja, billentyűzettel is elérhetően. Mindkét gomb `font-size` értéke `12px` (`.btn--sm`), és a nyíl trigger bal széle a "Mentés" gomb jobb szélére esik (a fúzió a `button-group.css` `.menu-anchor`-ra adott, dokumentált kiegészítésén át, mert a `Menu` a triggerét spanbe csomagolja). Futtatott unit és e2e teszt igazolja mindet.

67. **A jobb oldali beállítás panel kizárólag csomópont kiválasztásra jelenik meg.** Kiválasztás nélkül sem a panel, sem a `role="separator"` elválasztó nincs a DOM-ban, a vászon panelje viszont felmountolva marad (a pásztázás és a nagyítás megőrzése miatt), és a vászon a teljes szélességet kapja. Kattintás után a panel megjelenik, és a vászon ténylegesen szűkül. Futtatott unit és e2e teszt igazolja.

68. **Az osztott elrendezés aránya a `localStorage` `eggGraphEditorLayout` kulcsán perzisztálódik.** Az elválasztó mozgatása azonnal ír, a képernyő újratöltése a mentett arányt állítja vissza. A hiányzó érték, a hibás JSON, a rossz alakú érték és a **dobó** `localStorage` (privát ablak, letiltott tárolás) ága mind az alapértelmezésre esik vissza, kivétel nélkül; a `try`/`catch` mindkét irányban kötelező. Futtatott unit teszt fedi mind a hat ágat, futtatott e2e teszt az írás és a visszatöltés teljes körét. Ez a 6.1 szekció korábbi, "nem perzisztálódik" döntésének kimondott felülírása, felhasználói kérésre.

69. **A hibaüzenet megjelenési szabálya kikényszerített.** Érintetlen mezőn nincs hibaüzenet akkor sem, ha az érték a séma szerint érvénytelen; érintés (`blur`) után megjelenik; és sikertelen mentési kísérlet után minden érvénytelen mező kiírja a hibáját, érintés nélkül is. Ugyanez az érték dönt az `aria-invalid` kitételéről (WCAG 2.2 ARIA21). Futtatott unit teszt fedi mind a négy ágat a `packages/ui` `field-error-visibility` témájában és a `NodeInspector` szintjén, futtatott e2e teszt az érintetlen és az érintett esetet valós böngészőben (5.2).

70. **A többsoros mezők a design system `.textarea` komponensét viselik.** A panelen egyetlen `<textarea>` sem hordozza az egysoros `.input` osztályt, és a `node-inspector.css` saját `resize: vertical` toldása megszűnt. Futtatott unit és e2e teszt igazolja (5.2, 11.1).

71. **Az összecsukható panel billentyűzetről is nyitható és zárható.** A fejléc natív `<button>` egy natív `<h3>` fejlécben, `aria-expanded` és `aria-controls` attribútummal; `Enter` és `Space` nyitja, illetve zárja, a törzs pedig natív `hidden` attribútummal tűnik el. Futtatott e2e teszt igazolja valós böngészőben (5.2, 11.1).

72. **A bezáró gomb ikon gomb, hozzáférhető névvel.** A `.btn--icon` és a `.btn--sm` osztályt viseli, látható szövege nincs, a hozzáférhető nevét `aria-label` adja, és a `getByRole('button', { name: 'Bezárás' })` locator megtalálja. A panel **minden** `.btn` gombja `sm` méretű. Futtatott unit és e2e teszt igazolja (5.2).

73. **A csoportosítás típusonként indokolt.** Mind a tíz csomópont típusra ki van mondva, mi marad elöl és mi kerül panelbe, és miért (5.2 táblázata); az indoklás a megvalósító komponensek fejléc kommentjében is ott áll. Nyolc típusnak nincs panelje, mert nincs legalább három ritkán szerkesztett mezője.
