...
| a jóváhagyás panelen     | a transcript sávban, a transcript fölött: a `title`, a `body`, és a `payload` formázott alakja, két gombbal: "Jóváhagyás" és "Elutasítás", fölöttük a kimondott figyelmeztetés, hogy a döntés visszavonhatatlan |
...
1. **A panel helye.** A PLAN-009 5. szekciójának F6 sora szerint a jóváhagyás "a futás nézet fejlécébe és a transcript mellé" kerül. A fejlécben a jelzés áll (a fenti táblázat második sora), a döntési panel a transcript sávban, a transcript FÖLÖTT. A kettő a sávot egyenlő `flex-grow` szerint osztja meg, a panel legfeljebb a saját tartalmának magasságát kapja (`max-height: max-content`), sok jóváhagyásnál legfeljebb a sáv felét, és maga görget. A vászon magassága így nem függ a jóváhagyások számától: valódi Chromiumban mérve 0, 1 és 4 jóváhagyással azonos 1440x900, 1440x600 és 375x812 méreten, és az `.app-content` nem görget (`docs/research/2026-09-24-jovahagyas-panel-helye.md`). **A "transcript mellé" pontos helye a tervben kétértelmű** (lehetne a transcript sávon belül, vagy egy harmadik, önálló sáv); a megvalósítás a legkevésbé invazív olvasatot követi, ami a meglévő két sávos `Resizable` és `Tabs` elrendezést nem változtatja. Ez nyitott pont: ha a user önálló sávot kér, az a 10. szekció sáv tervének módosítása.
...
3. **A döntés eredménye.** Egy elfogadott, vagy `conflict`/`not_found` hibával végleg elbukott döntés kártyájának gombjai soha nem kapcsolnak vissza, akkor sem, ha a lista újratöltése elbukik; a kártya az eredménnyel együtt a panelen marad akkor is, ha a friss lista már nem tartalmazza, amíg a user a "Rendben" gombbal nem nyugtázza. Csak az átmeneti hiba (hálózati hiba, 502, 503) engedi az újrapróbálást, mert ott nem tudni, megtörtént-e a döntés. A megjelenő eredményt a felület a görgetett panel látható területére görgeti.
...
**A jóváhagyás panel mindhárom sávban a transcript oldalán áll, a vászon magassága nem függ tőle (T-009-27 javítás, 2026-09-24).** A két osztott sávban a transcript panelje fölött, a `--ep-screen-md` alatti fül sávban a "Transcript" fülön; a fejléc "jóváhagyásra vár" jelvénye mindhárom sávban látszik. A panel a transcript oldalt egyenlő `flex-grow` szerint osztja meg a transcripttel, legfeljebb a saját tartalma magasságáig (8. szekció 1. pont), tehát media query és kitalált arány nincs hozzá. A korábbi alak (a panel a teljes osztott nézet fölött) a vásznat a jóváhagyások számával zsugorította, négy jóváhagyásnál nullára, és az `.app-content` görgetett; valódi Chromiumban mérve most 0, 1 és 4 jóváhagyással a vászon magassága azonos (`docs/research/2026-09-24-jovahagyas-panel-helye.md`, regresszió: `apps/web/e2e/approval-prompt.spec.ts`).
