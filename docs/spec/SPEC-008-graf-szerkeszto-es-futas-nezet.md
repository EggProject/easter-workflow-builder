...
**Igazítás a mért sormagassághoz (2026-09-24).** A `scrollToRow` a még ki nem rajzolt sorokat a becsült magassággal számolja, és a pinelt `react-window@2.3.1` a görgetés után nem igazít, ha a kirajzolt sor mért magassága eltér a becsléstől (az átmeneti sor egy pixellel magasabb a tárolt sornál). Ezért követés közben a lista sormagasság gyorsítótárának minden változása után a panel újra az aljára görget, amíg a felhasználó a legutóbbi követő görgetés óta nem nyúlt a listához (kerék, érintés, `pointerdown`, billentyű: a könyvtár 2.3.2-es javításának megszakító eseményei, plusz 2026-09-24 óta a `click`, mert a `Space` a sort a `keyup`-ra nyitja ki, az `element.click()` pedig csak `click`-et ad). A könyvtár javításától eltérően az igazítás nem egyetlen görgetéshez kötött és nincs határideje, mert időzítőt nem használunk. Egy kinyitott sor így a helyén marad, és ha az utolsó sort kitolja, a követés a fenti predikátum szerint kikapcsol. Az elvárás: követés közben és az "ugrás az aljára" gomb után az utolsó sor **teljes egészében** látszik, az alja a lista alján. Mérés, elvetett alternatívák és a verzióemelés helyett választott út: `docs/research/2026-09-23-transcript-panel-meresek.md` 13. szekció.

**Kinyitás élő stream közben (2026-09-24).** Egy sor kinyitása vagy becsukása után a lista a sor új magasságát csak a következő mérés után, a látható tartományt pedig csak egy további renderben jelenti, tehát addig a predikátum a kinyitás előtti helyzetet írja le. Ezért egy sor kinyitása vagy becsukása után a panel nem görget (sem új sor érkezésére, sem átméretezésre), amíg a mért magassággal számolt első jelentés meg nem érkezik; utána a predikátum a kinyitás utáni helyzetről dönt, és a közben érkezett sorokat vagy követi, vagy a gomb számában megnevezi. A kinyitott sor így bármely úton (egér, `Enter`, `Space`, csak `click`) és egy közben érkező új sor mellett is a helyén marad. Mérés és regresszió: `docs/research/2026-09-23-transcript-panel-meresek.md` 15. szekció.
