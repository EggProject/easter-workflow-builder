...
| `sdk_result`                                                                       | a négy token szám és a `numTurns`, plusz a **költség külön, megnevezett mezőként** (lásd lent, user döntés 2026-09-23)                  |
...
**A költség megjelenik, SDK becslésként (user döntés, 2026-09-23).** A user döntése ("Költség külön mezőként is látszódjon") **visszavonja** a korábbi tiltást, ami szerint költség sehol nem jelenhetett meg. Az `sdk_result` sor a nyers payload `total_cost_usd` értékét az összesítő sorban (a design system `.accordion__meta` szlotjában) és a kinyitott nézetben is külön, megnevezett mezőként mutatja. **A felirat "Költség (SDK becslés)", mindkét providernél ugyanaz**, mert a szám egyiknél sem a valós költség: az SDK a MiniMax-M3 modellt nem ismeri fel és a beépített, Claude listaárral egyező tartalék díjtétellel számol, a `claude-subscription` providernél pedig listaáras becslés, ami előfizetésnél a számlázás szempontjából nem releváns. A kinyitott nézet egy mondatban ki is mondja ezt. Bizonyíték, két forrással és az összes SPEC-000 artefaktum újraszámolásával: `docs/research/2026-09-23-sdk-koltseg-becsles.md`. Normalizált, összegezhető költség oszlop továbbra sincs (SPEC-003, SPEC-004 F-8).
...
37. Az `sdk_assistant` sor a `toolName` és a `toolUseId` értéket, az `sdk_result` sor a négy token számot és a `numTurns` értéket mutatja, **és a költséget külön, megnevezett mezőként**, az összesítő sorban és a kinyitott nézetben is, "Költség (SDK becslés)" felirattal és a jelentését kimondó magyarázattal (7.1). Futtatott teszt igazolja. **User döntés, 2026-09-23**: ez visszavonja a kritérium korábbi, "költség sehol nem jelenik meg" tiltását és az azt őrző greppes tesztet.
