...
3. **A gombok összeragasztott csoportban (split button) állnak, `sm` méretben.** A `.button-group` a design system létező eleme, bájtra azonosan átemelve (`packages/ui/src/button-group/`); a csoport azért kell, mert a felhasználó szerint később további műveletek érkeznek ide. A `sm` méret a szabály a szerkesztő felületén; a modális és a popup gombok maradnak nagyobbak.
...
| `button-group`           | a design system `button-group` komponensének átemelése, bájtra azonos CSS-sel: összeragasztott gombcsoport (split button), `role="group"` és kötelező `aria-label` (10., 2026-09-09)                                                                                                                                                                                                                            |
...
66. **A gombok összeragasztott csoportban (split button), `sm` méretben állnak.** A `packages/ui/src/button-group` CSS-e bájtra azonos a design system forrásával (a `@import` sor nélkül); a csoport `role="group"` szerepet és `aria-label` nevet visel, a két gomb `font-size` értéke `12px` (`.btn--sm`), és a második gomb bal széle az elsőé jobb szélére esik. Futtatott e2e teszt igazolja mind a négyet.
