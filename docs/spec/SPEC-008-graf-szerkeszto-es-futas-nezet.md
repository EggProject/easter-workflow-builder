...
**Kinyitás élő stream közben (2026-09-24).** Egy sor kinyitása vagy becsukása után a lista a sor új magasságát csak a következő mérés után ismeri, addig a jelentései a kinyitás előtti elrendezést írják le, tehát a predikátum értéke ekkor nem ismert. Ezért egy sor fejlécének kattintása (minden kinyitási út `click` eseményt ad: egér, `Enter`, `Space`, csak `click`) a mérésig kikapcsolja a követést: a közben érkező sor nem görget, hanem a gomb számába kerül, és a mérés előtti jelentés a követést nem kapcsolhatja vissza. A mérés után a predikátum dönt, a mért magassággal számolt utolsó jelentés szerint. Egy képkockán belüli ki-be csukás (páros számú kattintás ugyanazon a fejlécen) nem változtat a soron, tehát nem kapcsolja ki a követést. A várakozásnak mindig van kilépése: a mérés, a páros kattintás, az "ugrás az aljára" gomb és a kézi visszatérés az aljára, és a nem látott sorok száma közben is nő. A követés visszakapcsolása önmagában nem görget: görgetést csak új esemény, átméretezés, a lista csatolása és a gomb indít.

**A kézi visszatérés az aljára is lezárja a várakozást (user döntés 2026-09-24).** Ha a várakozás alatt egy jelentés szerint az utolsó sor nem látszik, és egy későbbi jelentés szerint a predikátum újra igaz (`visibleRows.stopIndex === rowCount - 1`), a várakozás lezárul, a követés visszakapcsol, és a nem látott sorok száma nullázódik. Ez akkor is zár, ha a mérés sosem jön: egy fülváltás a kinyitott sort a mérése előtt leszereli (a rejtett sor 0 magasságát a `useDynamicRowHeight` nem tárolja), és a gyorsítótár utána nem változik; enélkül 375 pixelen a kinyitással egy feladatban váltott fül után a lista a kézi görgetés ellenére sem követett. Az alj elhagyásának feltétele azért kell, mert a kattintás előtti görgetés késve érkező jelentése a mérés előtti gyorsítótárral az utolsó sort láthatónak mutathatja; egy ilyen jelentés nem visszatérés. Pixel küszöb és időzítő nincs. Mérés és regresszió: `docs/research/2026-09-23-transcript-panel-meresek.md` 17. szekció.

**Nincs böngésző görgetés rögzítés (user döntés 2026-09-24).** A lista `overflow-anchor: none` értéket kap: a CSS Scroll Anchoring spec szerint ekkor a görgető dobozban nem választódik horgony, tehát a böngésző nem igazítja a görgetési helyet. A görgetést kizárólag a hook végzi. Bekapcsolt rögzítés mellett az "ugrás az aljára" utáni első kinyitásoknál a lista a hook nélkül 36 pixelt görgetett (research 17. szekció).

**Ami ebből következik, kimondva.** A kinyitott sor a helyén marad, ha a kinyitás az utolsó sort kitolja a látható tartományból (a jellemző eset: egy nem utolsó sor nyílik ki). Ha az utolsó sor a kinyitás után is látható (az utolsó sor maga nyílik ki, vagy a kinyitott törzs rövid), a predikátum igaz, a követés megmarad, és a **következő új esemény** az aljára görget, tehát a kinyitott sort a törzs plusz az új sor magasságával viszi feljebb (mérve a mérő eszközzel: 309 = 256 + 53 pixel, 1440x900-on és 375x812-en, mindkét témában; a törzs magassága a payloadtól függ, a korábban mért 351 pixel egy másik payloadú sorra szólt; a `dfcaa38` előtt is így volt). Az "ugrás az aljára" gomb a lista fejlécében jelenik meg, ezért a megjelenésekor a lista a gomb sáv magasságával lejjebb kerül (mérve: 36 pixel 1440x900-on); a sorok a listához képest a helyükön maradnak. Mérés és regresszió: `docs/research/2026-09-23-transcript-panel-meresek.md` 15., 16. és 17. szekció.

**Javaslat, nem döntés.** Ha a kinyitott utolsó sornak is a helyén kell maradnia, az a predikátumtól eltérő szabály volna (a sor kinyitása a követést addig szüneteltetné, amíg a felhasználó vissza nem ér az aljára vagy meg nem nyomja a gombot); ez felhasználói döntést igényel.
