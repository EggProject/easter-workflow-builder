...
| a jóváhagyás panelen     | a transcript sávban, a transcript fölött: görgethető tartalomként a kimondott figyelmeztetés, hogy a döntés visszavonhatatlan, és jóváhagyásonként a `title`, a `body` és a `payload` formázott alakja; alatta a nem görgető döntési sáv, jóváhagyásonként egy sorral: a cím és két gomb, "Jóváhagyás" és "Elutasítás" |
...
**A megvalósult alak (PLAN-009 T-009-27 és annak javítása, 2026-09-24, egy független ellenőrzés nyomán).**

1. **A panel helye és a döntés gombjai.** A PLAN-009 5. szekciójának F6 sora szerint a jóváhagyás "a futás nézet fejlécébe és a transcript mellé" kerül. A fejlécben a jelzés áll (a fenti táblázat második sora), a döntési panel a transcript sávban, a transcript FÖLÖTT. A kettő a sávot egyenlő `flex-grow` szerint osztja meg, a panel legfeljebb a saját tartalmának magasságát kapja (`max-height: max-content`). **Mérve már egyetlen jóváhagyás tartalma is nagyobb a sáv felénél** mindhárom mért méreten, tehát a panel a gyakorlatban a sáv felét kapja. A vászon magassága nem függ a jóváhagyások számától: valódi Chromiumban mérve 0, 1 és 4 jóváhagyással azonos 1440x900, 1440x600 és 375x812 méreten, és az `.app-content` nem görget (`docs/research/2026-09-24-jovahagyas-panel-helye.md`).

   **A döntés gombjai mindig látszanak** (user döntés 2026-09-24). A panel két részből áll: felül a görgethető tartalom (a "visszavonhatatlan" figyelmeztetés és a kártyák: cím, törzs, `payload`), alul a nem görgető döntési sáv. **Melyik kártya gombjai látszanak: mindegyiké.** A döntési sáv MINDEN megjelenített jóváhagyásnak egy sort ad (a cím egy sorra csonkolva, mellette a két gomb, alatta az eredmény), mert a cél az, hogy minden függő döntés görgetés nélkül elérhető legyen; egy kártyánként a tartalom aljához tapadó gombsor ezt nem adná meg, mert a görgetett tartalomban csak a látott kártya gombjai lennének elérhetők. A sor `group` szerepkörű, a neve a jóváhagyás címe, mert minden sorban ugyanaz a két gomb felirat áll. A hely hiányát a tartalom fizeti meg: mérve 1 és 4 jóváhagyásnál minden gomb teljesen látszik mindhárom méreten, de 1440x600-on négy jóváhagyásnál a görgethető tartalom 31 pixel. **Ahol ez nem teljesíthető:** a döntési sáv jóváhagyásonként nő, és egy méretfüggő darabszám fölött (mérve 1440x900-on 9, 1440x600-on 4, 375x812-n 6 az utolsó, ami elfér) maga sem fér el a panelben. Ekkor a legkisebb következetes viselkedés érvényes: a panel "legfeljebb a sáv fele" szabálya nem változik, a görgethető tartalom nulla magas, és a panel görget, tehát a további sorokhoz görgetni kell (research 7. szekció).
...
3. **A döntés eredménye.** Egy elfogadott, vagy `conflict`/`not_found` hibával végleg elbukott döntés gombjai soha nem kapcsolnak vissza, akkor sem, ha a lista újratöltése elbukik; a kártya és a döntési sor az eredménnyel együtt a panelen marad akkor is, ha a friss lista már nem tartalmazza. **Külön nyugtázás nincs** (user döntés 2026-09-24): az eredmény (siker vagy hiba) a futás nézet elhagyásáig vagy egy másik futásra váltásig látszik. Csak az átmeneti hiba (hálózati hiba, 502, 503) engedi az újrapróbálást, mert ott nem tudni, megtörtént-e a döntés. Az eredmény a döntési sávban, a gombok alatt áll, tehát görgetés nélkül látszik.
4. **"Visszavonhatatlan".** A kártyák fölött a design system `Alert` blokkja (`warning`) mondja ki: "A döntés visszavonhatatlan".
...
**A jóváhagyás panel mindhárom sávban a transcript oldalán áll, a vászon magassága nem függ tőle (T-009-27 javítás, 2026-09-24).** A két osztott sávban a transcript panelje fölött, a `--ep-screen-md` alatti fül sávban a "Transcript" fülön; a fejléc "jóváhagyásra vár" jelvénye mindhárom sávban látszik. A panel a transcript oldalt egyenlő `flex-grow` szerint osztja meg a transcripttel, legfeljebb a saját tartalma magasságáig, és a panelen belül a tartalom görget, a döntési sáv nem (8. szekció 1. pont), tehát media query és kitalált arány nincs hozzá. A korábbi alak (a panel a teljes osztott nézet fölött) a vásznat a jóváhagyások számával zsugorította, négy jóváhagyásnál nullára, és az `.app-content` görgetett; valódi Chromiumban mérve most 0, 1 és 4 jóváhagyással a vászon magassága azonos (`docs/research/2026-09-24-jovahagyas-panel-helye.md`, regresszió: `apps/web/e2e/approval-prompt.spec.ts`).
