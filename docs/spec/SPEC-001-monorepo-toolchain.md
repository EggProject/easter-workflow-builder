...
**Frissítve a SPEC-002-vel (T-002-25).** Az alábbi csomagtérkép és függőségi tábla a
SPEC-002 végrehajtása előtti, 16 csomagos állapotot rögzíti (13 eredeti csomag, plusz a
két utólag felvett `typeguards` és `agent-tools`). A `packages/providers` és a
`packages/agent-tools` csomag azóta 18 új csomagra bomlott, a workspace 32 csomagból áll,
és minden csomag rétegszámot kapott. Az aktuális csomagtérkép és a rétegzett függőségi
irány a
[`SPEC-002-csomag-architektura.md`](SPEC-002-csomag-architektura.md) 4. szekciója. Ez a
szekció itt, lent, változatlanul marad történeti dokumentumként.
...
4. **Az e2e coverage-re 2026-09-05 óta VAN küszöb, mind a négy metrikán.** Ez a spec eredetileg úgy fogalmazott, hogy "E2E küszöböt csak akkor rögzítünk, ha a gyűjtés stabilan működik, és akkor is külön döntéssel". Mindkét feltétel teljesült: a gyűjtés végponttól végpontig működik (V-11 lezárva), és a felhasználó külön döntéssel küszöböt kért. A kikényszerítés helye az `apps/web/package.json` `coverage:e2e:report` scriptjének `--check-coverage` kapcsolója; küszöb alatt az `nyc` `process.exitCode = 1` értéket állít és a hibát stderr-re írja, az összehasonlítás pedig szigorúan kisebb (`coverage < threshold`), tehát a küszöbbel egyenlő érték átmegy. A négy szám **mért** érték, felfelé kerekítés nélkül, tehát ratchet; a származtatás, a nem fedett részek tételes listája és a "nulla fájl kizárás" döntés a [`docs/research/2026-09-05-e2e-lefedettsegi-kuszob.md`](../research/2026-09-05-e2e-lefedettsegi-kuszob.md) fájlban áll. **Az e2e küszöb nem 100 százalék**, és ez felhasználói döntés: marad néhány, e2e-vel elvileg sem elérhető sor (build időben rögzülő `VITE_*` konfiguráció hibaágai, React `useEffect` cleanup, ami csak leszereléskor fut, és egy hibaág, amit felhasználói úton nem lehet előidézni) - mind unit teszttel fedett. A Vitest unit coverage 100 százalékos küszöbét ez nem érinti.

   **Nem tizedik kapu, hanem külön kategória kikényszerített küszöbbel.** A `.claude/CLAUDE.md` 8. szekciója szerinti kilenc kapu a lokális, commit előtti készlet, ami build és böngésző nélkül fut. Az e2e Chromium telepítést és teljes frontend buildet igényel, ezért marad a CI saját `e2e` jobjában. Kapuvá az teszi, hogy az `e2e` job szerepel az összesítő `ci` job `needs` listájában, ami a repository ruleset egyetlen kötelező státuszcsekkje.
...
| Job                | Függ                           | Mit csinál                                                                                      |
| ------------------ | ------------------------------ | ----------------------------------------------------------------------------------------------- |
| `gate` (matrix)    | nincs                          | `format:check`, `typecheck`, `lint`, `docs:check`, `check:casing`, `check:graph` külön legekben |
...
4. A 3. szekció mind a 15 csomagja (a `packages/typeguards` és a `packages/agent-tools` csomaggal együtt) létezik a megadott útvonalon, mindegyiknek van `package.json`, `tsconfig.json` és `CLAUDE.md` fájlja. **[SPEC-002-vel elavult, T-002-25.]** A `packages/agent-tools` és a `packages/providers` csomag megszűnt, a csomagszám 32-re nőtt. Ezt a kritériumot a SPEC-002 2. elfogadási kritériuma (a 18 új csomag létezése) és a Definition of Done 3. pontja (32 csomag, mindegyiknek `package.json`, `tsconfig.json`, `CLAUDE.md`) váltja ki.
...
24. A V-11 (Vite 8 kompatibilitás) eredménye dokumentálva van a `docs/research/` alatt. Ha a plugin nem működik Vite 8 alatt, ez a tény és a halasztás indoka le van írva, és ez nem blokkolja a spec elfogadását. **Kiegészítés (2026-09-05, felhasználói döntés):** a gyűjtés végponttól végpontig működik, ezért az e2e coverage-re **küszöb került**, mind a négy metrikán, a 10. szekció 4. pontja szerint. A `bun run coverage:e2e:report` küszöb alatt nem nulla kilépési kóddal fut, ezt saját mérés igazolja ([`../research/2026-09-05-e2e-lefedettsegi-kuszob.md`](../research/2026-09-05-e2e-lefedettsegi-kuszob.md) 6. szekció), és a bukása az `e2e` jobon keresztül az összesítő `ci` jobot is megbuktatja.
...
27. A GitHub Actions workflow (`ci.yml`) hat jobot tartalmaz: a `gate` mátrix (hat minőségi kapu: `format`, `typecheck`, `lint`, `docs`, `casing`, `graph`), a `test`, a `build`, az `e2e`, a `coverage-comment` és az összesítő `ci` job. A `gate` mátrix hat eleme, a `test` és a `build` job között **nincs** `needs` kapcsolat: ez a nyolc jobfutás egymástól függetlenül, párhuzamosan indul a checkout után. Az `e2e` job kivétel: `needs: build`, user döntés, tehát csak a `build` job sikeres lezárása után indul. A `coverage-comment` job `needs: [test, e2e]`, az összesítő `ci` job `needs: [gate, test, build, e2e]`, tehát ez a két job vár a többire, az `e2e` pedig a `build`-ra. Minden checkoutot végző job a `bun install --frozen-lockfile` alakot a `.github/actions/setup` lokális composite actionön keresztül futtatja.
...
45. A `gate` job `strategy.fail-fast` értéke explicit `false`. Ezt egy szándékosan hibás bemenet igazolja: ha a mátrix egyik eleme (például `typecheck`) elbukik, a többi öt elem (`format`, `lint`, `docs`, `casing`, `graph`) attól függetlenül lefut és jelzi a saját eredményét, nem szakítja meg a teljes mátrixot a dokumentált `fail-fast: true` alapértelmezés szerint.
...
51. A `packages/agent-tools` csomag (utólag felvéve, 3. szekció) létezik `package.json`, `tsconfig.json`, `src/index.ts` és `CLAUDE.md` fájllal, és mind a hét `src/` alkönyvtárában van `CLAUDE.md`. A csomag három MCP eszközt ad (`web_search`, `web_fetch`, `understand_image`), mindegyiket az Agent SDK `tool` helperjével és Zod sémával, és a `createAgentToolBundle` függvény a lépés által kiválasztott azonosítókból állítja elő az `mcpServers` és az `allowedTools` értéket. Ismétlődő azonosító nem duplikálja az eszközt. **[SPEC-002-vel elavult, T-002-25.]** A `packages/agent-tools` könyvtár nem létezik többé: 11 fájlja négy csomagra bomlott (`tool-minimax-web-search`, `tool-firecrawl-web-fetch`, `tool-minimax-understand-image`, `agent-tool-bundle`), a SPEC-002 5.14-5.17 szekciója szerint. A csomag létezésének tilalmát a SPEC-002 1. elfogadási kritériuma váltja ki, a viselkedés (három eszköz, `createAgentToolBundle`, dedupe) változatlan, ezt a T-002-18 .. T-002-21 lépések tesztjei igazolják.
...
54. A `ProviderCapabilityDescriptor` típus `recommendedAgentTools` mezővel bővült, és mindkét provider leírója ki van töltve: a `minimax` esetén `known` állapot mérési bizonyítékkal, a `claude-subscription` esetén `unknown`, mert ehhez a providerhez nincs drótszintű mérés. Bizonyíték nélküli érték egyik leíróban sincs. **[Útvonal SPEC-002-vel elavult, tartalom változatlan, T-002-25.]** A `ProviderCapabilityDescriptor` típus a `packages/providers/src/capability/` helyett a `@easter-workflow-builder/provider-capability` csomag `tool-support/agent-tool-recommendation.ts` fájljában van, a két leíró kitöltött `recommendedAgentTools` mezője a `@easter-workflow-builder/provider-minimax` és a `@easter-workflow-builder/provider-claude-subscription` csomag `tool-support/recommended-agent-tools.ts` fájljában. A tartalmi azonosságot a SPEC-002 30. elfogadási kritériuma (bitre azonos `Fact` értékek, normalizált JSON összehasonlítással) váltja ki.
55. Az `agent-tools` csomag által használt hat környezeti változó mindegyike szerepel a `turbo.json` `globalPassThroughEnv` listájában, és mindegyik alapértelmezése mellett a kódban megjelölve áll, hogy dokumentált forrásból származik-e vagy önkényes. Az önkényes érték kötelezően felülírható környezeti változóval. **[SPEC-002-vel elavult, T-002-25.]** A `MINIMAX_CODING_PLAN_API_KEY` megszűnt (SPEC-002 5.10 szekció, a user döntése), a környezeti változók száma hatról ötre csökkent. Ezt a SPEC-002 27. elfogadási kritériuma váltja ki.
