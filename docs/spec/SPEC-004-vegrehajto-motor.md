...
**A `fail_run` leállítása ugyanaz a mechanizmus, mint a megszakításé (9. szekció 2. és 3. pont), a saját futásra szűkítve.** A motor kiveszi a futás szabályozó sorában álló agent lépéseit (`ConcurrencyGate.denyWaitingForRunIds`), lezárja a futás döntésre váró jóváhagyásainak várakozását (`ApprovalWaitRegistry.cancelWaitingForRunIds`), végül `interrupt()`-ot hív a futó agent lépésein. A kivett lépés el sem indul (`markStepRunning`, `step_started` esemény és provider hívás nélkül); a lezárt várakozású jóváhagyás végrehajtója állapotváltás és esemény nélkül tér vissza; mindkettő sora a futás záró menetében, a `markRunFailed` előtt `cancelled` állapotba megy. Ha a bukott lépés maga foglalt helyet, a kivétel a helye felszabadítása **előtt** történik, mert a felszabaduló helyet a sor következő eleme szinkron kapja meg: enélkül a testvér 4 ms-mal a bukás után elindult (mérve). A sorból kivétel az `interrupt()` hívás **előtt** áll, mert az `AgentQuery` szerződése nem köti ki, hogy a nyugta a folyam vége előtt érkezik: ha utána, a megszakított lépés felszabaduló helyét fordított sorrendben a sorban álló testvér kapja meg (futtatott teszttel igazolva; a valódi SDK nyugtájának időzítése nem mért). Más futás sorban álló lépése és döntésre váró jóváhagyása érintetlen.

**A lezárt lépések végállapota.** A táblázat nem nevezi meg; a SPEC-003 7.2 állapotgépe dönt. A sorban álló, el sem indult lépés `cancelled` (`pending -> cancelled`, "a futás megszakadt, mielőtt a lépés elindult"), a döntésre váró jóváhagyás lépése szintén (`waiting_approval -> cancelled`), ugyanaz a végállapot, amit a megszakítás 5. pontja ír; az `interrupted` az indulási helyreállításnak és a szabályos leállásnak fenntartott (10. szekció), `skipped` állapot nincs. A `human_approval` sornak nincs állapot oszlopa (SPEC-003 4.12): a `decision` NULL marad, és egy utólagos döntés a lépés sorának `cancelled` állapotán bukik `illegal_status_transition` hibával (HTTP 409 `conflict`), semmit nem módosítva. A futó, megszakított lépés a folyama kimerítése után a saját `result` üzenete szerint zár (4. pont a 9. szekcióban), ugyanúgy, mint a felhasználói megszakításnál; a motor terminális sort nem ír át. Mérve (korlát 1, a bukás 300 ms-nál, a testvérek természetes hossza 8000 ms): a javítás előtt 3 agent hívás, és a futás 9013 ms-mal a bukás után lett `failed`; utána 1 hívás, 6 ms, a két testvér `cancelled`. Döntésre váró, korlátlan várakozású testvér jóváhagyással: előtte a futás a döntésig `running` maradt, és jóváhagyáskor a lépés `succeeded` lett egy `failed` futásban; utána a futás 2 ... 6 ms-mal a bukás után `failed`, a jóváhagyás lépése `cancelled` (`docs/research/2026-09-23-megszakitas-leallas-meres.md` 7. szekció).

**Nyitott, mérve, a megszakítás útján is ugyanígy: a lezárt jóváhagyás két nyoma.** (1) A függő jóváhagyások listája (`GET /api/approvals`, `listPendingApprovals`, `decision IS NULL`) a lezárt jóváhagyást továbbra is visszaadja, a döntése pedig 409-cel bukik. (2) A várakozás lezárása és a lépés sorának `cancelled` írása között egy futó testvér folyamának kimerüléséig (9. szekció 4. pont) ablak van: az ekkor érkező döntést a `db` elfogadja, a jóváhagyás lépése `succeeded` lesz egy `failed`, illetve `cancelled` futásban. Mi a viselkedés addig: a fenti. Mi zárná le: termékdöntés arról, hogy a lezárt jóváhagyás hogyan jelenjen meg a listában, és hogy a lépés sorát a várakozás lezárásával egy időben kell-e írni (mindkét útra).
...
51. A megszakított futás és minden nem terminális lépése `cancelled` állapotban zár, egy tranzakcióban, `run_finished` eseménnyel, amit a motor a tranzakció után élőben is kiad. Futtatott teszt igazolja a kiadott esemény alakját és azt, hogy a kiadás pillanatában a sor már a naplóban áll.
