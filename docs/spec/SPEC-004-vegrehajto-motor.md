...
- **Nem tervezi meg a kifejezés nyelvet és a sablon nyelvet.** A `branch.expression`, a `fan_out.itemsExpression`, a `loop.continueExpression`, valamint a `promptTemplate`, a `bodyTemplate` és a `branchLabelTemplate` kiértékelése két befecskendezett porton át történik. A portok szerződése itt áll, a nyelv megválasztása külön döntés volt (O-1). **Az O-1 2026-09-23-án a user döntésével lezárult: a sablon nyelv Mustache, a kifejezés nyelv CEL, és a szállított implementációt a [`SPEC-010-sablon-es-kifejezes-nyelv.md`](SPEC-010-sablon-es-kifejezes-nyelv.md) írja le.** A motor ettől sem ismer nyelvet: a két port szerződése változatlan.
...
| `expressionEvaluator`      | `evaluate(expression, context): Outcome<unknown>` és `compile(expression): Outcome<void>`         | `apps/server`, a CEL implementációval (O-1 lezárva, SPEC-010)      |
| `templateRenderer`         | `render(template, context): Outcome<string>` és `compile(template): Outcome<void>`                | `apps/server`, a Mustache implementációval (O-1 lezárva, SPEC-010) |
...
**A SPEC-010 9.2 ezt a listát egy ellenőrzéssel bővíti:** minden sablon és kifejezés mező lefordítható a port `compile` metódusával, különben a futás indítása `template_render_failed`, illetve `expression_evaluation_failed` osztállyal elutasít, ugyanebben a lépésben, tehát adat írása nélkül. Az ellenőrzés elfogadási kritériumai a SPEC-010 16. szekciójában állnak; a jelen spec 8. kritériumának "tíz ellenőrzés" száma a SPEC-004 saját táblázatára vonatkozik.
...
7. **A szülő megszakítása a gyereket is elviszi**, mert azonos `root_run_id` alatt állnak (SPEC-003 4.8), és a motor a megszakítást a teljes fára alkalmazza (9. szekció). A szülő `fail_run` bukása szintén, ugyanazzal a mechanizmussal, a bukott futás alfájára szűkítve (8.3).
...
**A `fail_run` leállítása ugyanaz a mechanizmus, mint a megszakításé (9. szekció 2 ... 5. pont): a bukott futásra a sorból kivétel, a várakozások lezárása és az `interrupt()`, az al-workflow futásaira a teljes fa mechanizmus.** A bukott futásra a motor kiveszi a futás szabályozó sorában álló agent lépéseit (`ConcurrencyGate.denyWaitingForRunIds`), lezárja a futás döntésre váró jóváhagyásainak várakozását (`ApprovalWaitRegistry.cancelWaitingForRunIds`), végül `interrupt()`-ot hív a futó agent lépésein. A kivett lépés el sem indul (`markStepRunning`, `step_started` esemény és provider hívás nélkül), a sora a futás záró menetében, a `markRunFailed` előtt `cancelled` állapotba megy. A lezárt várakozású jóváhagyás végrehajtója állapotváltás és esemény nélkül tér vissza, a sorát pedig a motor a várakozás lezárásával egy szinkron menetben, az `interrupt()` előtt viszi `cancelled` állapotba, nem a záró menetben: különben egy futó testvér folyamának kimerüléséig (9. szekció 4. pont) tartó leállási ablakban érkező döntést a `db` a még `waiting_approval` soron elfogadná. Ha a bukott lépés maga foglalt helyet, a kivétel a helye felszabadítása **előtt** történik, mert a felszabaduló helyet a sor következő eleme szinkron kapja meg: enélkül a testvér 4 ms-mal a bukás után elindult (mérve). A sorból kivétel az `interrupt()` hívás **előtt** áll, mert az `AgentQuery` szerződése nem köti ki, hogy a nyugta a folyam vége előtt érkezik: ha utána, a megszakított lépés felszabaduló helyét fordított sorrendben a sorban álló testvér kapja meg (futtatott teszttel igazolva; a valódi SDK nyugtájának időzítése nem mért). **A bukott futás al-workflow futásai `cancelled` állapotban zárnak, ugyanazzal a fa mechanizmussal, mint a felhasználói megszakításnál** (user döntés 2026-09-23): a futó `sub_workflow` testvér gyerek futása és annak minden leszármazottja (unoka, ...) sorban álló lépései kiesnek, a döntésre váró jóváhagyásaik sora `cancelled`, a futó agent lépéseik `interrupt()`-ot kapnak, a motor megvárja a leállásukat, majd egy tranzakcióban `cancelled` állapotba viszi a futásokat és a nem terminális lépéseiket, és futásonként élőben kiadja a `run_finished` eseményt. A hatókör a bukott futás alfája, nem a `root_run_id` szerinti teljes fa: a bukott futás maga `failed` (8.4), az ősei és a testvér futások alfái érintetlenek. A leszármazottakat a motor a memóriájából választja ki (a futások szülő lánca), a tranzakció a megnevezett futásokra szűkít. A leszármazottak sorból kivétele is a bukott futás `interrupt()` hívása előtt áll. Kivétel a szabályos leállás közben beágyazott `fail_run`: a leállás által már leállított leszármazott célállapota az első leállításé (9. szekció "A leállított al-workflow futás célállapota"), tehát `interrupted`, és a tranzakció ezt a futást nem zárja `cancelled` állapotba; a leállás saját zárása viszi `interrupted` állapotba (10.2 3. pont). Más futás sorban álló lépése, döntésre váró jóváhagyása és al-workflow futása érintetlen.

**A lezárt lépések végállapota.** A táblázat nem nevezi meg; a SPEC-003 7.2 állapotgépe dönt. A sorban álló, el sem indult lépés `cancelled` (`pending -> cancelled`, "a futás megszakadt, mielőtt a lépés elindult"), a döntésre váró jóváhagyás lépése szintén (`waiting_approval -> cancelled`), ugyanaz a végállapot, amit a megszakítás 5. pontja ír; az `interrupted` az indulási helyreállításnak és a szabályos leállásnak fenntartott (10. szekció), `skipped` állapot nincs. A `human_approval` sornak nincs állapot oszlopa (SPEC-003 4.12): a `decision` NULL marad, és egy utólagos döntés a lépés sorának `cancelled` állapotán bukik `illegal_status_transition` hibával (HTTP 409 `conflict`), semmit nem módosítva; a leállási ablakban érkező döntés is, a `fail_run`, a felhasználói megszakítás és a szabályos leállás útján (utóbbinál a sor `interrupted`, 10.2). A futó, megszakított lépés a folyama kimerítése után a saját `result` üzenete szerint zár (4. pont a 9. szekcióban), ugyanúgy, mint a felhasználói megszakításnál; a motor terminális sort nem ír át. A futó `sub_workflow` testvér lépése ugyanígy a saját útján zár, a gyerek futás lezárulása után: `failed`, `sub_workflow_failed` osztállyal (5.9 6. pont: a nem `succeeded` gyerek után a szülő lépés `failed`), ugyanúgy, mint a felhasználói megszakításnál (mérve). A SPEC-003 7.2 a `running -> cancelled` átmenetet is megengedné ("megszakítás"), a végállapotot tehát nem az állapotgép, hanem az 5.9 6. pontja dönti el. A gyerek futás és leszármazottai `cancelled`, a döntésre váró jóváhagyásaik lépése szintén, és egy utólagos döntés ugyanúgy `illegal_status_transition` hibával bukik. Mérve (korlát 1, a bukás 300 ms-nál, a testvérek természetes hossza 8000 ms): a javítás előtt 3 agent hívás, és a futás 9013 ms-mal a bukás után lett `failed`; utána 1 hívás, 6 ms, a két testvér `cancelled`. Döntésre váró, korlátlan várakozású testvér jóváhagyással: előtte a futás a döntésig `running` maradt, és jóváhagyáskor a lépés `succeeded` lett egy `failed` futásban; utána a futás 2 ... 6 ms-mal a bukás után `failed`, a jóváhagyás lépése `cancelled`. A leállási ablakban (a futó testvér `interrupt()` utáni 1000 ms-os kimerülése alatt) beküldött döntés: előtte HTTP 200, a jóváhagyás lépése `succeeded` egy `failed`, illetve `cancelled` futásban, `decision = 'approved'`, `step_finished` és `approval_decided` esemény nélkül; utána HTTP 409 `conflict`, a lépés `cancelled`, a `decision` NULL. Futó `sub_workflow` testvérrel, aminek a gyereke és az unokája jóváhagyásra vár: előtte a szülő futás és a `sub` lépés 3000 ms-mal a bukás után is `running`, a gyerek és az unoka jóváhagyására küldött döntés HTTP 200, és a `sub` lépés `succeeded` lett egy `failed` futásban; utána a szülő 5 ... 12 ms-mal a bukás után `failed`, a gyerek és az unoka `cancelled`, a jóváhagyásaikra küldött döntés HTTP 409 `conflict`, egy másik futás gyerek fája érintetlen (`docs/research/2026-09-23-megszakitas-leallas-meres.md` 7. szekció).

**A lezárt jóváhagyás nem függő** (user döntés 2026-09-23, a 15. szekció O-9 tétele lezárva). A függő jóváhagyások listája (`GET /api/approvals`, `listPendingApprovals`) a lépés sorának `waiting_approval` állapotából dönt, nem a `decision IS NULL` feltételből (SPEC-003 4.12), tehát a `fail_run`, a megszakítás és a `sub_workflow` fa lezárása által lezárt jóváhagyás kikerül belőle. Az utólagos döntés `conflict` marad, és a már eldöntött jóváhagyás második döntése is az: a szerver a jóváhagyást az azonosítója szerint olvassa (`getApproval`), `not_found` csak nem létező azonosítóra jön (SPEC-005 4.2). Mérve a valódi `apps/server` modulokon, hamis agenttel, valódi API nélkül: a javítás előtt a lezárt jóváhagyás mindhárom úton a listában maradt (1 elem), a döntése 409, a már eldöntött jóváhagyás második döntése 404; utána a lista mindhárom úton üres, a döntés 409, a második döntés 409 `already_decided`, a nem létező azonosító 404.
...
**A `fail_branch` tehát nem a futás állapotát változtatja meg, hanem azt, hogy mikor áll le a futás.** `fail_run` mellett azonnal: a futás egy futó `sub_workflow` testvér gyerek futásának döntésére sem vár, mert a gyerek fa `cancelled` állapotban zár (8.3, user döntés 2026-09-23). `fail_branch` mellett a többi ág befejezése után, egy futó gyerek futás lezárulását is beleértve.
...
2. A motor a futás memóriabeli állapotán **azonnal beállít egy megszakítási jelzést**, tehát a szabályozó ebből a futásból többé nem enged induló lépést, és a sorban álló lépései kiesnek. A két fél két külön mechanizmus. A jelzés (`requestStop`) csak az új példány indítását tartja vissza. A már elindított, de párhuzamossági helyre váró agent lépést a szabályozó veszi ki a sorból, a fa minden futására (`ConcurrencyGate.denyWaitingForRunIds`): a lépés `interrupted` eredménnyel tér vissza, `markStepRunning`, `step_started` esemény és provider hívás nélkül, a sora `pending` marad, és az 5. pont tranzakciója `cancelled` állapotba viszi (a `pending -> cancelled` átmenet érvényes, SPEC-003 7.2). Mérve: a második fél nélkül egy három párhuzamos agent lépéses futás megszakítása után (korlát 1) a két sorban álló lépés a felszabaduló helyen `interrupt()` nélkül végigfutott, három agent hívással, a lépések `succeeded` állapotban zártak egy `cancelled` futásban, és a megszakító kérés 17043 ms-ig nem válaszolt; vele egy agent hívás, a két sorban álló lépés `cancelled`, a válasz 1012 ms (SPEC-006 M-37). A fa döntésre váró `human_approval` lépéseinek várakozása ugyanekkor lezárul, és a soruk ugyanebben a szinkron menetben `cancelled` állapotba megy, nem az 5. pont tranzakciójában: a 4. pont kimerüléséig tartó ablakban érkező döntés így `illegal_status_transition` hibával bukik, semmit nem módosítva (8.3).
...
5. A `db` oldal egy tranzakcióban zár: a futás `cancelled`, minden nem terminális lépés futása `cancelled`, és egy `run_finished` motor esemény (SPEC-003 7.4). A tranzakció után a motor ezt az eseményt minden megszakított futásra **élőben is kiadja** (`publish`), mert a léptető hurok a leállítás után szándékosan nem ír és nem ad ki semmit; enélkül az élő nézet sosem tudná meg, hogy a futás lezárult (mérve, SPEC-006 M-35).
6. Minden foglalt hely felszabadul, és a szabályozó a sor következő elemét indítja.

**A megszakítás válasza a futó lépések leállását várja meg, a sorban állókét nem.** Az `interruptRun` a 4. pont kimerüléséig vár, mert az `InterruptSummary` `cancelledRunIds` mezője az 5. pont tranzakciójának eredménye; azonnali elfogadás és aszinkron lezárás nincs (SPEC-005 4.2, 15. végpont). A várakozás tehát a megszakított, FUTÓ lépések folyamának kimerüléséig tart, a természetes végükig nem, és a sorban álló lépésekre sem, mert azok a 2. pont szerint el sem indulnak. Mérve egy csak sorban álló futás megszakítása, miközben egy másik futás lépése foglalja az egyetlen helyet: a javítás előtt 15030 ms és egy többlet agent hívás, utána 2 ms, agent hívás nélkül (SPEC-006 M-37).
...
**A leállított al-workflow futás célállapota** (user döntés 2026-09-23). A 2. pont jelzése (`requestStop`) a leállító fél célállapotát is hordozza, és az a futás memóriabeli kézikönyvére kerül: a felhasználói megszakítás és a `fail_run` (8.3) `cancelled`, a szabályos leállás (10.2) `interrupted` célállapotot ad, ugyanazt, amit a saját DB zárásuk ír (5. pont, illetve 10.2 3. pont). Ha egy futást többször állítanak le, az első célállapot marad meg, a DB zárásban is: a szabályos leállás közben beágyazott `fail_run` (egy testvér `sub_workflow` lépés a leállás miatt bukik) a leállás által már `interrupted` célú gyereket nem zárja `cancelled` állapotba, azt a leállás 10.2 3. pont szerinti zárása viszi `interrupted` állapotba, `run_interrupted` eseménnyel; fordított sorrendben (előbb `fail_run`, közben leállás) a gyerek `cancelled` marad. Mérve a valódi szerveren, sorrendenként 5 futással: a leállás előbb sorrendben a javítás előtt a gyerek sora `cancelled`, az eseménye `interrupted` volt (0/5 egyezés), utána mindkettő `interrupted` (5/5); a fordított sorrend előtte és utána is `cancelled`, egyezően (5/5), a gyökér mindkét sorrendben `interrupted` (`docs/research/2026-09-23-megszakitas-leallas-meres.md` 7. szekció, tizedik kör). A szülő `sub_workflow` lépése a gyerek sorát a gyerek léptetésének lezárulásakor olvassa, a fa DB zárása előtt: a megszakítás és a szabályos leállás zárása a szülő lezárulását is megvárja, az pedig a `sub_workflow` lépés zárására vár; a `fail_run` zárása csak a leszármazottakét, de a gyerek sorának olvasása a gyerek lezárulásának korábban bejegyzett folytatása, tehát megelőzi. A leállított gyerek sora ezért ekkor még `running` (mérve mindhárom úton), és a lépés a kézikönyv célállapotát adja a `sub_workflow_finished` eseményben és a hibaüzenetében (13. szekció); a lépés végállapota ettől nem függ, a nem `succeeded` gyerek után `failed`, `sub_workflow_failed` osztállyal (5.9 6. pont). Ha a gyerek sora már terminális (a leállítás a saját záró írása után érte), a sor állapota dönt, mert a fa zárása csak a nem terminális sorokat írja. A fa zárása egy tranzakció marad (5. pont), és a futó lépés a saját útján zár (8.3); ez a döntés egyiket sem írja felül.

**Megszakítás szabályos leállás közben** (user döntés 2026-09-24). Ha a szabályos leállás (10.2) már elkezdődött, az `interruptRun` `engine_shutting_down` hibával, olvasás és írás nélkül tér vissza, és a futást a leállás zárja `interrupted` állapotba, `run_interrupted` eseménnyel. Ugyanaz a jelzés dönt, amin a futás indítás elutasítása áll (10.2 1. pont, `RunSupervisor.isAcceptingRuns`), és ugyanaz a hibaosztály, tehát a REST válasz `503`, `service_unavailable` kóddal (SPEC-005 8.3). A már folyamatban lévő megszakítást a később kezdődő leállás nem érinti: fordított sorrendben (előbb megszakítás, közben leállás) a fa a megszakítás szerint `cancelled` marad, egyezően a sorában és az eseményében. Mérve a valódi szerveren, hamis agenttel, esetenként 5 futással: a javítás előtt a leállás alatt befejezett megszakító kérés, ha a fa volt az egyetlen aktív futás, `socket hang up` hibát kapott, mert a válasz a kapcsolatok zárása után készült el (a fa sora és eseménye `interrupted`, 5/5); ha egy másik futás tovább tartotta a leállást, a válasz `200` volt, a fa sora `cancelled`, a szülő lépés eseménye és üzenete `interrupted` (0/5 egyezés, gyerek és unoka szinten is). Utána mindkét esetben `503`, `(engine_shutting_down)`, a fa sora és eseménye `interrupted` (5/5); a fordított sorrend előtte és utána is `200` és `cancelled`, egyezően (5/5) (`docs/research/2026-09-23-megszakitas-leallas-meres.md` 7. szekció, tizenegyedik kör).
...
1. A szabályozó nem enged több lépést indulni, és a motor új futást sem indít. Mindkettő az aktív futások lekérdezése **előtt**, szinkron történik, tehát a lekérdezett lista a leállás teljes hatóköre: a `startRun`, a rá épülő `restartRun` és az al-workflow indítás `engine_shutting_down` hibával, adatírás nélkül utasít el (ugyanígy a leállás alatt érkező felhasználói megszakítás, 9. szekció, user döntés 2026-09-24), a szabályozó pedig lezárul (`ConcurrencyGate.close`), így a sorban álló agent lépések el sem indulnak (`interrupted` eredménnyel térnek vissza, a soruk `pending` marad, és a 3. pont zárja), a felszabaduló helyet pedig senki nem kapja meg. Mérve: e pont megvalósítása előtt egy a jel előtt fejléccel megkezdett, de csak a leállás alatt befejezett indító kérés új futást indított, aminek az agent lépése `interrupt()` nélkül végigfutott, és a kilépést a lépés teljes hosszával késleltette (SPEC-006 8.2, M-36).
2. Minden élő agent lépésen `interrupt()`, majd a folyam kimerítése, ugyanúgy, mint a 9. szekcióban.
3. Minden érintett futás `interrupted` állapotba megy (`markRunInterrupted`), a nem terminális lépéseivel együtt, és futásonként egy `run_interrupted` esemény íródik. A döntésre váró jóváhagyás sora a várakozás lezárásával egy szinkron menetben, a 2. pont `interrupt()` hívása előtt `interrupted`, így a leállási ablakban érkező döntés `illegal_status_transition` hibával bukik (8.3). A leállított al-workflow futás szülő lépésének `sub_workflow_finished` eseménye és hibaüzenete `interrupted` (9. szekció "A leállított al-workflow futás célállapota", user döntés 2026-09-23). Ez a leállás alatt beágyazott `fail_run` gyerek fájára is áll: a leállás által már leállított gyerek sora is `interrupted`, nem `cancelled` (8.3, 9. szekció).
...
**A leállított gyerek futás `sub_workflow_finished` eseménye a leállítás célállapotát hordozza** (user döntés 2026-09-23). A `status` mező és a szülő lépés hibaüzenete a gyerek futás terminális állapotát mondja. A leállított (megszakított, `fail_run` miatt lezárt vagy szabályos leállásban álló) gyerek sorát viszont a szülő `sub_workflow` lépése még a fa DB zárása (9. szekció 5. pont, 10.2 3. pont) előtt olvassa, tehát a sor ekkor még `running`. Ilyenkor az esemény és az üzenet a leállítás célállapotát adja: felhasználói megszakításnál és `fail_run`-nál `cancelled`, szabályos leállásnál `interrupted`, pontosan azt, amit a fa zárása a gyerek sorába ír. Két leállításnál az elsőét, és a fa zárása is azt írja: a szabályos leállás közben beágyazott `fail_run` után az esemény és a gyerek sora egyaránt `interrupted` (9. szekció). A leállás alatt érkező felhasználói megszakítás nem második leállítás, mert írás nélkül, hibával tér vissza (9. szekció "Megszakítás szabályos leállás közben", user döntés 2026-09-24): az esemény és a sor ilyenkor is egyezően `interrupted`. A mechanizmust a 9. szekció "A leállított al-workflow futás célállapota" bekezdése írja le. A nem leállított gyerek eseménye változatlanul a sorának terminális állapota. Mérve a valódi szerveren, gyerekre és unokára, mindhárom úton: a javítás előtt az esemény és az üzenet `running`, utána a célállapot, ami mind a 30 esetben egyezik a gyerek sorával a fa zárása után (`docs/research/2026-09-23-megszakitas-leallas-meres.md` 7. szekció, kilencedik kör).
...
| --- | ------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| O-1 | A kifejezés nyelv és a sablon nyelv megválasztása                         | **Lezárva, 2026-09-23, a user döntésével: a sablon nyelv Mustache, a kifejezés nyelv CEL.** A szállított implementációt, a futás indítási fordítást és a hibák kezelését a [`SPEC-010-sablon-es-kifejezes-nyelv.md`](SPEC-010-sablon-es-kifejezes-nyelv.md) írja le. A lezárás előtti állapot, a kód olvasása szerint pontosítva: a motor a `compile` metódust sehol nem hívta, ezért a két elutasító port egyike sem az indításkor jelentkezett, hanem a lépés futásakor, a kifejezés portnál `expression_evaluation_failed`, a sablon portnál `template_render_failed` osztállyal (SPEC-010 M-103; a sablon port mérve, `docs/research/2026-09-23-megszakitas-leallas-meres.md` 4. szekció). A korábbi "indítása `expression_evaluator_unavailable` hibával elutasít" megfogalmazás ettől eltért | lezárva                                                                                                                        |
...
| O-9 | A lezárt jóváhagyás a függő jóváhagyások listájában (8.3)                 | **Lezárva, 2026-09-23, a user döntésével: a lépés állapota dönt.** A lista csak a `waiting_approval` lépésű jóváhagyást adja (SPEC-003 4.12), a lezárt vagy már eldöntött jóváhagyás döntése `conflict`, `not_found` csak nem létező azonosítóra (SPEC-005 4.2, e spec 8.3). A lezárás előtti állapot: a lista (`decision IS NULL`) a `fail_run` vagy a megszakítás által lezárt jóváhagyást is visszaadta, a döntése 409-cel bukott, a már eldöntött jóváhagyás második döntése viszont 404 volt                                                                                                                                                                                                                                                                                                  | lezárva                                                                                                                        |
...
| A kifejezés port nélkül a workflow-k fele indíthatatlan                           | a termék nem használható végponttól végpontig                        | az O-1 lezárva: a SPEC-010 szállított Mustache és CEL portot köt be, és a futás indítása minden sablon és kifejezés mezőt lefordít (SPEC-010 9.2)          |
...
43. `fail_run` mellett a kezeletlen hiba azonnal leállítja a testvér ágakat; `fail_branch` mellett a testvér ágak befejeződnek. Mindkét irány futtatott teszttel igazolt, korlátozott szabályozóval is: `fail_run` után a sorban álló testvér agent lépések nem indulnak el (nulla `step_started`, nulla agent hívás, a soruk `cancelled`), korlát 1 és korlát 2 mellett, akkor is, ha a bukott lépés nem foglalt helyet; a futó testvér `interrupt()`-ot kap, és a futás a természetes végét meg nem várva `failed`; egy másik futás sorban álló lépése érintetlen. A döntésre váró `human_approval` testvér lépése `cancelled`, a futás döntés nélkül `failed`, egy utólagos döntés `illegal_status_transition` hibát ad és semmit nem módosít, akkor is, ha a futó testvér folyamának kimerülése alatt érkezik (ugyanez a felhasználói megszakítás útján is), egy másik futás jóváhagyása érintetlen; a sorból kivétel az `interrupt()` előtt áll, akkor is, ha a nyugta a folyam vége után érkezik. A futó `sub_workflow` testvér gyerek futása és annak leszármazottai `cancelled`, a döntésre váró jóváhagyásaik lépése szintén, egy utólagos döntés `illegal_status_transition` hibát ad, a `sub` lépés `failed` (`sub_workflow_failed`), és a futás a gyerek döntése nélkül `failed`; egy másik futás gyerek futása érintetlen. `fail_branch` mellett a sorban álló testvérek lefutnak, és a döntésre váró jóváhagyás, illetve a futó gyerek futás a döntésig vár (8.3).
...
49. A megszakítás minden élő agent lépésen meghívja az `interrupt()` metódust, a futás fáján, tehát az al-workflow futásainak lépésein is, és a fa szabályozó sorában álló agent lépései el sem indulnak (9. szekció 2. pont). Futtatott teszt igazolja korlátozott szabályozóval, hogy a megszakítás után egyetlen agent hívás történik, és a sorban álló lépések `cancelled` állapotban zárnak.
...
51. A megszakított futás és minden nem terminális lépése `cancelled` állapotban zár, egy tranzakcióban (a döntésre váró jóváhagyás lépése már a várakozás lezárásakor, 9. szekció 2. pont), `run_finished` eseménnyel, amit a motor a tranzakció után élőben is kiad. Futtatott teszt igazolja a kiadott esemény alakját és azt, hogy a kiadás pillanatában a sor már a naplóban áll.
...
54. A szabályos leállás és a durva leállás utáni indulási helyreállítás **azonos** adatbázis állapotot hagy: minden érintett futás és nem terminális lépés `interrupted`, futásonként egy `run_interrupted` eseménnyel. Futtatott teszt igazolja mindkét úton. A szabályos leállás ezen felül minden érintett futás `run_interrupted` eseményét élőben is kiadja; futtatott teszt igazolja, a kézikönyv nélküli futásra is. A leállási ablakban (a futó testvér folyamának kimerülése alatt) érkező jóváhagyási döntés a jóváhagyás lépését `interrupted` állapotban hagyja és `illegal_status_transition` hibával bukik, mindkét úton: futtatott teszt igazolja ("REGRESSZIÓ: a leállási ablakban (a futó testvér folyamának kimerülése alatt) érkező jóváhagyási döntés illegal_status_transition hibát ad, semmit nem módosít, és a jóváhagyás lépése interrupted (SPEC-004 10.2 3. pont)"), és a leállás utáni újraindítás helyreállítására is ("a leállás utáni újraindítás helyreállítása (runStartupRecovery) a jóváhagyás lépését interrupted állapotban hagyja, és egy új motor példányon érkező döntés is illegal_status_transition (SPEC-004 10.1, 10.2)"). A leállás alatt érkező felhasználói megszakítás `engine_shutting_down` hibával, írás nélkül bukik, és a fa (gyökér, gyerek, unoka) sora és eseménye egyezően `interrupted`; előbb megszakítás, közben leállás sorrendben a fa egyezően `cancelled` (9. szekció, user döntés 2026-09-24). Futtatott teszt igazolja mindkét sorrendre a `create-engine.spec.ts`-ben, a `503` választ pedig valódi HTTP kapcsolaton a `run-shutdown-sequence.spec.ts`.
...
68. A 15. szekció mind a kilenc kérdése nyitottként áll, a "mi a viselkedés addig" és a "mi zárná le" mezővel kitöltve, vagy lezárt futtatott méréssel, a `docs/research/` alá vezetve, vagy a user döntésével lezárt és átvezetett (O-1: SPEC-010; O-9: SPEC-003 4.12 és SPEC-005 4.2, user döntés 2026-09-23). Tippeléssel lezárt pont nincs.
