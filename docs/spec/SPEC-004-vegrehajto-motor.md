...
**Nyitott, mérve: a `sub_workflow` testvér gyerek futása.** A `fail_run` a saját `runId`-jára szűkít, a testvér `sub_workflow` lépés gyerek futását nem állítja le: ha a gyerek jóváhagyásra vár, a bukott szülő futás a bukás után sem terminális. A megszakítás a fa minden futására hat (`root_run_id`), a `fail_run` nem. Mi a viselkedés addig: a szülő `running` marad, amíg a gyerek futás le nem zárul. Mi zárná le: a gyerek futás záró állapotának kimondása `fail_run` esetén, és a leállítás kiterjesztése a futás alfájára.
...
68. A 15. szekció mind a nyolc nyitott kérdése nyitottként áll, a "mi a viselkedés addig" és a "mi zárná le" mezővel kitöltve, vagy lezárt futtatott méréssel, a `docs/research/` alá vezetve, vagy a user döntésével lezárt és átvezetett (O-1: SPEC-010). Tippeléssel lezárt pont nincs.
