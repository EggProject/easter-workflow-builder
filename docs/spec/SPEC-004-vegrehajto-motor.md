...
**A `fail_run` leállítása ugyanaz a mechanizmus, mint a megszakításé (9. szekció 2. és 3. pont), a saját futásra szűkítve.** A motor előbb kiveszi a futás szabályozó sorában álló agent lépéseit (`ConcurrencyGate.denyWaitingForRunIds`), utána `interrupt()`-ot hív a futó agent lépésein. A kivett lépés el sem indul (`markStepRunning`, `step_started` esemény és provider hívás nélkül), a sora a futás záró menetében, a `markRunFailed` előtt `cancelled` állapotba megy. Ha a bukott lépés maga foglalt helyet, a kivétel a helye felszabadítása **előtt** történik, mert a felszabaduló helyet a sor következő eleme szinkron kapja meg: enélkül a testvér 4 ms-mal a bukás után elindult (mérve). Más futás sorban álló lépése érintetlen, és a felszabaduló helyet megkapja.

**A lezárt lépések végállapota.** A táblázat nem nevezi meg; a SPEC-003 7.2 állapotgépe dönt. A sorban álló, el sem indult lépés `cancelled` (`pending -> cancelled`, "a futás megszakadt, mielőtt a lépés elindult"); az `interrupted` az indulási helyreállításnak és a szabályos leállásnak fenntartott (10. szekció), `skipped` állapot nincs. A futó, megszakított lépés a folyama kimerítése után a saját `result` üzenete szerint zár (4. pont a 9. szekcióban), ugyanúgy, mint a felhasználói megszakításnál; a motor terminális sort nem ír át. Mérve (korlát 1, a bukás 300 ms-nál, a testvérek természetes hossza 8000 ms): a javítás előtt 3 agent hívás, és a futás 9013 ms-mal a bukás után lett `failed`; utána 1 hívás, 6 ms, a két testvér `cancelled` (`docs/research/2026-09-23-megszakitas-leallas-meres.md` 7. szekció).

**Nyitott: a várakozó `human_approval` testvér.** A `fail_run` a döntésre váró jóváhagyást nem zárja le, tehát egy korlátlan várakozású (`timeoutMs: null`) testvér jóváhagyás a futást a döntésig nyitva tartja. Mi a viselkedés addig: a futás a döntés után zár `failed` állapotban. Mi zárná le: a jóváhagyás várakozásának feloldása a `fail_run` ágon is (`ApprovalWaitRegistry.cancelWaitingForRunIds`), a `waiting_approval` sor záró állapotának kimondásával.
