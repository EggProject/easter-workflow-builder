...
`run-control` (T-009-23, huszonnegyedik), `run-event-row` (T-009-24, huszonötödik) és
`transcript-panel` (T-009-25, huszonhatodik). A SPEC-008 11.2 szekciója tíz új témát ír elő
összesen (a maradék egy, az `approval-prompt`, a PLAN-009 hátralévő lépéseiben érkezik); a végleges
"huszonnégy téma" végösszeg (SPEC-008 DoD 2.) és a jelen tényleges darabszám (tizenkettő
SPEC-007 UI + öt invariáns + tíz SPEC-008 = huszonhét) közötti eltérés a PLAN-009 T-009-32
(F8) zárásának hatóköre, nem ezé a lépésé. A `run-control` téma egyben a SPEC-008 11.2
...
kapcsoló következménye (átmeneti sorok) a T-009-26 hatóköre; a `run_event_transient` keretet a
panel ma figyelmen kívül hagyja.
...
`graph-editor-layout.ts` `isLayoutSizePair` guardját használja. A transcript panel TARTALMA a
...
| `e2e/`                                 | Playwright tesztek, a coverage fixture, a közös REST/SSE mockolási segédfüggvények (T-008-27..29), a transcript `run_event` fixtúrái (`transcript-fixture.ts`, T-009-25), a bemutató gráf fixtúra (`showcase-graph.ts`), a per-él pixel mérés (`edge-paint-measurement.ts`), a képernyőkép készítés (`capture-screenshots.ts`) és annak bizonyíték manifesztje (`screenshot-manifest.json`, generált, de commitolt)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
