...
nem ad. A panel (virtualizáció, görgetés) a T-009-25 hatóköre; ez a téma a soronkénti komponenst
adja, amit a panel majd felhasznál.
...
| `src/greppable-invariants/`            | tizenöt, megvalósítás nélküli, greppel ellenőrizhető invariáns teszt egy `describe` blokkban (T-008-31, SPEC-002 6.2 5. pont mintája); a tizenötödik a mért csomópont geometriától való függést tiltja (`measured.` olvasás és `getBoundingClientRect(` hívás, T-009-15, SPEC-008 12.2); a T-009-24-gyel érkezett tizenhatodik (költség tiltás) a user 2026-09-23-i döntésével törölve                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `src/run-event-row/`                   | a `run_event.kind` mind a huszonöt értékének kimerítő `switch` leképezése egy sor fejléc-szövegére (`run-event-row-summary.ts`) és a kinyitható `RunEventRow` komponens, a `packages/ui` `AccordionItem` panelje felett, az `icon` szlotban álló eredet jelölő oszloppal, az `sdk_result` "Költség (SDK becslés)" mezőjével és a nyers `payload` formázott JSON kinyitásával (T-009-24, SPEC-008 7.1, 7.2, AC36, AC37, AC38)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
