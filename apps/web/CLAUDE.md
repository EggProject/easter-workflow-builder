...
`run-control` (T-009-23, huszonnegyedik) és `run-event-row` (T-009-24, huszonötödik). A
SPEC-008 11.2 szekciója tíz új témát ír elő összesen (a maradék kettő, a `transcript-panel` és
az `approval-prompt`, a PLAN-009 hátralévő lépéseiben érkezik); a végleges
"huszonnégy téma" végösszeg (SPEC-008 DoD 2.) és a jelen tényleges darabszám (tizenkettő
SPEC-007 UI + öt invariáns + kilenc SPEC-008 = huszonhat) közötti eltérés a PLAN-009 T-009-32
...
billentyűzetről is működik), mert a `title` prop kizárólag `string`: a spec 7.2 negyedik pontja
szerinti önálló, színes "jelölő oszlop" ezzel nem valósítható meg szó szerint, ezért az eredet
(`sdk`/`engine`) szövegként a fejléc szövegében és egy kiegészítő bal szegély színben jelenik
meg, nem önálló DOM oszlopként - ez a design system címzett hiánya, nem saját komponens
gyártása. Az `sdk_assistant` sor a `toolName`-t és a `toolUseId`-t, az `sdk_result` sor mind a
négy token számot és a `numTurns` értéket mutatja; **költség sehol nem jelenik meg**, a
`greppable-invariants` téma (16). tesztje őrzi. A panel (virtualizáció, görgetés) a T-009-25
hatóköre; ez a téma a soronkénti komponenst adja, amit a panel majd felhasznál.
...
| `src/greppable-invariants/`            | tizenhat, megvalósítás nélküli, greppel ellenőrizhető invariáns teszt egy `describe` blokkban (T-008-31, SPEC-002 6.2 5. pont mintája); a tizenötödik a mért csomópont geometriától való függést tiltja (`measured.` olvasás és `getBoundingClientRect(` hívás, T-009-15, SPEC-008 12.2), a tizenhatodik a transzkript sorokban tiltja a költség mezőt (`total_cost_usd`, `cost`, T-009-24, SPEC-008 AC37)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `src/run-event-row/`                   | a `run_event.kind` mind a huszonöt értékének kimerítő `switch` leképezése egy sor fejléc-szövegére (`run-event-row-summary.ts`) és a kinyitható `RunEventRow` komponens, a `packages/ui` `AccordionItem` panelje felett, a nyers `payload` formázott JSON kinyitásával (T-009-24, SPEC-008 7.1, AC36, AC37, AC38)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| `e2e/`                                 | Playwright tesztek, a coverage fixture, a közös REST/SSE mockolási segédfüggvények (T-008-27..29), a bemutató gráf fixtúra (`showcase-graph.ts`), a per-él pixel mérés (`edge-paint-measurement.ts`), a képernyőkép készítés (`capture-screenshots.ts`) és annak bizonyíték manifesztje (`screenshot-manifest.json`, generált, de commitolt)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
