...
**A SPEC-008 F3 és F4 fázisa (PLAN-009) folyamatban bővíti a gráf szerkesztővel és a futás
nézettel.** Eddig elkészült témák: `graph-node-catalog` (T-009-14, tizenhetedik),
`graph-node-card` (T-009-15, tizennyolcadik), `graph-editor` (T-009-16, tizenkilencedik),
`node-inspector` (T-009-18, huszadik), `graph-auto-layout` (T-009-19, huszonegyedik),
`run-graph` (T-009-20, huszonkettedik), `run-view` (T-009-20, huszonharmadik) és `run-control`
(T-009-23, huszonnegyedik). A SPEC-008 11.2
szekciója tíz új témát ír elő összesen (a maradék kettő a PLAN-009 hátralévő lépéseiben
érkezik); a végleges
"huszonnégy téma" végösszeg (SPEC-008 DoD 2.) és a jelen tényleges darabszám (tizenkettő
SPEC-007 UI + öt invariáns + nyolc SPEC-008 = huszonöt) közötti eltérés a PLAN-009 T-009-32
...
| `src/greppable-invariants/`            | tizenöt, megvalósítás nélküli, greppel ellenőrizhető invariáns teszt egy `describe` blokkban (T-008-31, SPEC-002 6.2 5. pont mintája); a tizenötödik a mért csomópont geometriától való függést tiltja (`measured.` olvasás és `getBoundingClientRect(` hívás, T-009-15, SPEC-008 12.2)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
