...
| `src/graph-editor/`                    | a vezérelt `GraphEditorCanvas` (`@xyflow/react` felett), a `GraphEditorScreen` (betöltés, mentés, mentetlen jelző, mentés előtti séma ellenőrzés), a node/él leképezés, a mért csomópont méret nézeti állapota és az `isValidGraphConnection` szerkezeti szabálypár (SPEC-008 5.4, 5.5, AC8-AC13, AC15) |
