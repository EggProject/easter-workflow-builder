...
| `error-mapping/`                | a SPEC-005 8.3 táblázat: az `Outcome` üzenetéből a hibaosztály név kiolvasása, `ProtocolErrorCode` leképezés, és a hiba törzs összeállítása a SPEC-005 8.5 `errorClass` mezővel |
...
| `error-class-drift-protection/` | a SPEC-005 8.5 `ProtocolErrorClass` szótárának megvalósítás nélküli regressziós tesztje: részhalmaza-e a motor `EngineErrorKind` uniójának (típusszinten és futásidőben)        |
