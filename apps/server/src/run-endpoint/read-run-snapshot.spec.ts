...
import { createReadRunSnapshotHandler } from './read-run-snapshot.ts';
...
describe('createReadRunSnapshotHandler', () => {
  it('létező futásra a tárolt pillanatképet változatlanul adja vissza', async () => {
    ...
    expect(result.kind === 'ok' && result.value.body).toMatchObject({ version: 1 });
  });
