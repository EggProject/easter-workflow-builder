...
| `src/casing/`                     | téma mappa: a `casing.sh` mögötti tényleges ellenőrzés (`check-casing.ts`), a rá épülő `.spec.ts` regressziós teszt, a parser alapú specifikátor kinyerő (`find-relative-import-specifiers.ts`: `import`/`export ... from` és a string literál argumentumú dinamikus `import()`, saját `.spec.ts`-sel), és a parser nélküli előszűrő (`find-relative-specifier-candidates.ts`), ami a TypeScript parse-ot csak a gyanús fájlokra futtatja                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `src/screenshot-pipeline/`        | téma mappa, megvalósítás fájl nélkül: a képernyőkép készítés egyetlen szentesített útját őrző hét invariáns (a szentesített fájlon kívül semmi nem ír képet vagy videót lemezre, a kép formátumától függetlenül, a felsorolt írási utakon, az import gráfon át, a CDP és az oldal screencast képein és a trace képernyőképein át sem; a vizsgált kör a nyolc JavaScript és TypeScript kiterjesztés, a `.sh` és a `.bash` script, a YAML és a `package.json` scriptek, a termékkód (`packages/*/src`, `apps/*/src` nem teszt fájlja) csak Playwright importtal; a képernyőkép, a videó, a trace és a trace képernyőkép opció zárt listás, a kontextus videó felvétele tilos; a szentesített script nem tart saját gráfot vagy mockot; az `apps/web/e2e/screenshot-manifest.json` bizonyíték lenyomata friss; a hetedik az első kettő ellenőrzését szintetikus eseteken futtatja). Hatókör és elfogadott korlátok, köztük a teszt saját gyengítése mint code review kérdés: `.claude/CLAUDE.md` 12. szekció |
