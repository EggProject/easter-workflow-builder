...
export const M09: MeasurementCase = {
  ...
  question: 'Q7',
  async run(context) {
    const caseDirectory = path.join(context.outDir, 'M-09');
    const toolInputPath = path.join(caseDirectory, 'a.tool-callback-input.json');
    ...
    const noteTool = tool(
      ...
      (arguments_) => {
        ...
        // sdk-messages.ndjson assistant tool_use inputja ellen. A caseDirectory-t az
        // executeQuery a run() eleje óta már létrehozta.
        writeFileSync(toolInputPath, JSON.stringify(arguments_, undefined, 2), 'utf8');
        return Promise.resolve({ content: [{ type: 'text', text: `rögzítve: ${arguments_.title}` }] });
      },
    );
