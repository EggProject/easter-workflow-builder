...
- **a léptető hurok** (`run-supervisor/advance-run.ts`) a `failRunRequested` igazra váltása után,
  ...
  (`run-interrupt/cancel-waiting-approval-step-runs.ts`), és ez hívja az `interrupt()`-ot. A sor
...
`fail_run` `cancelled`-et, már a várakozás lezárásával egy szinkron menetben
(`cancelWaitingApprovalStepRuns`, hogy a leállási ablakban érkező döntés a sor állapotán bukjon),
a szabályos leállás `interrupted`-et (`recoverInterruptedRuns`) -, amit a végrehajtó nem tudna
eldönteni, és egy saját írás a lezárást kérő fél írásával versenyezne (ugyanaz az elv, mint az
`ActiveRunHandle.requestStop` doksijában a futás záró állapotára).
