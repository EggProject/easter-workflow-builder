...
szerinti teljes, lezárt lista. **A T-005-29 nem nyitott új témát** (a 18-as darabszám tehát
változatlan), hanem az `engine-port` témába tett egy gyűjtő regressziós tesztet
(`config-invariants.spec.ts`), ami a paraméterezhetőség és a token takarékosság hat, fájlrendszer
bejárással ellenőrizhető invariánsát őrzi - lásd a "Fájlok" táblázat `engine-port/` sorát és a
"Szabályok" megfelelő bekezdését.

## Fájlok

| Mappa                   | Felelősség                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ----------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `engine-port/`          | a kilenc befecskendezett port típusa és az összefogó `EngineDependencies` (SPEC-004 3.2, 3.3), **plusz a T-005-28 óta a motor kifelé mutatott felülete**: a `Engine` interfész (SPEC-004 3.1, hét metódus), a `createEngine(dependencies): Engine` összeállító függvény és az öt kis eredmény típus (`ApprovalDecisionInput`, `ConcurrencySuggestion`, `ConnectionTestResult`, `InterruptSummary`, `ShutdownSummary`). A kilenc port típusfájl **típus-only, nincs `.spec.ts`** (SPEC-002 6.3 pont); a `create-engine.ts` viszont valós `:memory:` adatbázis ellen futó, végponttól végpontig tesztelt implementáció, saját `.spec.ts`-sel, lásd "Szabályok". **A T-005-29 óta itt áll a `config-invariants.spec.ts` is**: MEGVALÓSÍTÁS NÉLKÜLI, csomagszintű gyűjtő regressziós teszt, ami fájlrendszer bejárással (`node:fs`, `node:path`) ellenőrzi, hogy a `packages/engine/src` alatti termékkód `.ts` fájljaiban (a `.spec.ts` fájlok kivételével) nincs konkrét provider azonosító, `@anthropic-ai`, `Date.now`, `setTimeout`, `randomUUID`, `process.env`, `Math.random` hivatkozás, `total_cost_usd` mezőnév és `retry-after` szöveg, plusz hogy a csomag `package.json`-ja nem listázza sem a `provider-registry`, sem az Agent SDK csomagot (SPEC-004 17. szekció 6., 22., 40., 56., 57., 58. elfogadási kritérium). Lásd "Szabályok" a téma választás indoklásáért                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| `run-supervisor/`       | a futás életciklusa (SPEC-004 4.8, 4.4 ... 4.6, 8.4, PLAN-005 T-005-25). A `createRunSupervisor` az egyetlen belépési pont: `startRun` (a 4.8 menet hét lépése), háttérben futó léptető hurok (`advanceRun`), és a `ChildWorkflowRunner` két metódusa, **önmagára hivatkozva**. A 4.8 lépései külön, önmagukban futtatható függvények: `collectRunInputs` (1. lépés), `buildSnapshotDocument` (5. lépés), `buildNodePlans` + `validateProviderCapabilities` + `validateRunInput` egyetlen menetben (`prepareRun`, 3. és 4. lépés), plusz a gyerek futás kimenete (`collectTerminalOutput`, `buildChildResult`) és az aktív futások nyilvántartása (`ActiveRunRegistry`). A kézikönyv 2026-09-23 óta a szülő futást is hordozza (`parentRunId`), a `listDescendants` ezen a láncon adja egy futás alfáját, és a `ChildWorkflowRunner` harmadik metódusa, a `cancelChildRunTrees` ezt zárja le a `fail_run`-ban (lásd "Szabályok"). A `run-supervisor.ts` és a `run-execution.ts` **típus-only, nincs `.spec.ts`** **A `restartRun` (`restart-run.ts`) a T-005-27 óta**: a motor `restartRun` művelete (SPEC-004 3.1, 9. szekció zárómondata) - a `Pick<RunSupervisor, 'startRun'>`-t kapja, az eredeti futás `workflowId`-jét és `input`-ját `database.runs.getRun`-nal olvassa ki, és a `startRun`-t hívja ÚJRA, `restartedFromRunId`-vel kiegészítve; a workflow AKTUÁLIS gráfját nem kell külön beolvasnia, mert a `startRun` amúgy is mindig frissen olvassa (lásd "Szabályok")                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `run-interrupt/`        | a felhasználói megszakítás teljes menete (SPEC-004 9. szekció, PLAN-005 T-005-26). Az `AgentQueryRegistry` (`agent-query-registry.ts`) egy élő, `stepRunId` kulcsú `Map`: a `register`/`unregister` a `runAgentStep` (`agent-step` téma) `finally` ágán fut, a `listForRunIds` a megszakítandó fa `AgentQuery` objektumait adja. A `stopAndAwaitRunTree` a közös menet: `requestStop()` minden kézikönyvön, a fa szabályozó sorában álló agent lépéseinek elutasítása (`ConcurrencyGate.denyWaitingForRunIds`, 2026-09-23), a fa VÁRAKOZÓ `human_approval` lépéseinek lezárása (`ApprovalWaitRegistry.cancelWaitingForRunIds`, T-005-31), `interrupt()` minden érintett élő `AgentQuery`-n a `interruptLiveAgentQueries` primitívvel (T-005-31), majd az összes `completion` megvárása. Az `interruptRun` a belépési pont: a cél `runId`-ból olvassa ki a `rootRunId`-t, a `run-supervisor` `listActiveRuns()` listáját erre szűkíti, lefuttatja a `stopAndAwaitRunTree` menetet, majd a `database.recovery.cancelRunTree(rootRunId)` egyetlen tranzakcióban zár, és a `cancelledRunIds` minden futására élőben kiadja a `run_finished` (`status: 'cancelled'`) eseményt. A `shutdownActiveRuns` (`shutdown-active-runs.ts`, T-005-27) a szabályos leállás analóg belépési pontja: előbb letiltja az új futást (`run-supervisor` `stopAcceptingRuns()`) és lezárja a szabályozót (`concurrency-gate` `close()`), majd a `run-supervisor` `listActiveRuns()` TELJES listájának (nem egy fára szűkítve) döntésre váró jóváhagyás sorait `interrupted` állapotba viszi, és a listát végigfuttatja a `stopAndAwaitRunTree` menetén, majd a `database.recovery.recoverInterruptedRuns('graceful_shutdown')` GLOBÁLIS hatókörű hívást zár le, és a `recoveredRunIds` minden futására élőben kiadja a `run_interrupted` eseményt - lásd "Szabályok" a `reason` paraméterezés és az élő kiadás indoklásáért. **A T-005-31 ötödik fájlt tett ide**, az `interrupt-live-agent-queries.ts`-t: ez a megszakítás elemi lépése (`interrupt()` egy `runId` halmaz élő `AgentQuery` objektumain), amit a `stopAndAwaitRunTree` ÉS a `fail_run` hibapolitika is hív - lásd "Szabályok". A hatodik fájl a `close-waiting-approval-step-runs.ts` (2026-09-23): a megnevezett futások `waiting_approval` sorát a hívó által megnevezett záró állapotba viszi; az `interruptRun` (a `cancelActiveRunTree`-n át) és a `fail_run` hurok `cancelled`, a `shutdownActiveRuns` `interrupted` állapottal hívja, mindhárom a várakozás lezárásával egy szinkron menetben. A hetedik fájl a `cancel-active-run-tree.ts` (2026-09-23): a felhasználói megszakítás fa menete a hatókör kiválasztása nélkül (jóváhagyás sorok, `stopAndAwaitRunTree`, a hívó által adott DB zárás, `run_finished` élő kiadása); az `interruptRun` a `cancelRunTree`-vel, a `run-supervisor` `cancelChildRunTrees` művelete (`fail_run`) a `cancelRuns`-szal hívja, lásd "Szabályok". Mind a hét fájl kap `.spec.ts`-t, valós `:memory:` adatbázissal, ahol a téma adatbázist érint. |
...
jóváhagyás szintén `interrupted` eredménnyel, a sora viszont már `cancelled` (lásd a hurkot). Három
hely:
...
- **a bukott, helyet foglaló lépés** (`node-executor/agent-node-lifecycle.ts`) a helye
  ...
  `NodeExecutionInstance.failureStopsRun` igaz (a `run-supervisor` a `resolveErrorRoute`
  döntéséből számítja). A hurok erre nem elég: a `releaseSlot` a felszabaduló helyet szinkron adja
...
- **a léptető hurok** (`run-supervisor/advance-run.ts`) a `failRunRequested` igazra váltása után,
  ...
  (például elutasított jóváhagyás), ez zárja le a testvér jóváhagyások várakozását, és ugyanebben a
  szinkron menetben, az `interrupt()` előtt a sorukat is `cancelled` állapotba viszi
  (`run-interrupt/close-waiting-approval-step-runs.ts`), és ez hívja az `interrupt()`-ot. A sor
  írása azért nem a záró menetben áll, mert a futó testvér folyamának kimerüléséig tartó ablakban
  érkező döntést a `db` a még `waiting_approval` soron elfogadná (mérve: a jóváhagyás `succeeded`
  lett egy `failed` futásban, esemény nélkül); a `cancelled` soron `illegal_status_transition`
  hibával bukik, és a `human_approval` sor írása is visszagördül. A sorból kivétel az `interrupt()` ELŐTT áll: az `AgentQuery` szerződése nem
...
- **a záró menet** (`finishRun`, `cancelStoppedStepRuns`) a `markRunFailed` előtt a futás minden
  `pending` sorát `cancelled` állapotba viszi (SPEC-003 7.2); a hurok ekkorra minden elindított
  példányt megvárt, tehát `running` sor nem maradhat. A `pending` sort a hurok menete szándékosan
  nem zárja: a szabályozó a helyet egy `Promise` feloldásával adja át, a `markStepRunning` egy
  későbbi folytatásban fut, tehát egy a menet előtt helyet kapott testvér sora is lehet még
  `pending`. A `human_approval` sor `decision` mezője NULL marad (nincs állapot oszlopa), egy
  utólagos döntés a lépés sorának `cancelled` állapotán `illegal_status_transition` hibával bukik,
  ugyanúgy, mint a megszakítás után.
...
**A `fail_run` a bukott futás al-workflow futásait a teljes fa mechanizmussal zárja (user döntés
2026-09-23, SPEC-004 8.3).** A felhasználói megszakítás fa menete (jóváhagyás sorok lezárása,
`stopAndAwaitRunTree`, DB zárás egy tranzakcióban, `run_finished` élő kiadása) egyetlen közös
függvény, a `run-interrupt/cancel-active-run-tree.ts`; az `interruptRun` és a `fail_run` is ezt
hívja, csak a hatókör és a DB zárás más. A `fail_run` hatóköre a bukott futás alfája (gyerek,
unoka, ...): a `run-supervisor` `cancelChildRunTrees` művelete (a `ChildWorkflowRunner` harmadik
metódusa) az `ActiveRunRegistry.listDescendants` bejárással, a kézikönyvek `parentRunId` láncán
választja ki, és a `db` `cancelRuns(runIds)` primitívjével zár. **Nem a `cancelRunTree`-vel**, mert
az a `root_run_id` szerint a bukott futást és az őseit is `cancelled` állapotba vinné, holott a
bukott futás `failed`. **Nem az adatbázis `sub_workflow_run_id` oszlopából**, mert az a gyerek
indítása után egy `await`-tel később íródik, a kézikönyv viszont a gyerek indításával egy szinkron
menetben jön létre. A léptető hurok a lezárást a saját `interrupt()` hívása ELŐTT indítja (a
szinkron része a leszármazottak sorból kivétele), és utána várja meg. A futó `sub_workflow` lépés a
gyerek lezárulása után a saját útján zár: `failed`, `sub_workflow_failed` (SPEC-004 5.9 6. pont).
Mérve a valódi `apps/server` modulokon, hamis agenttel, a gyerek és az unoka jóváhagyásra vár:
...
terminális `stepRun` rekordot, és kilenc végrehajtóból nyolc kizárólag ilyen ágat tud adni; ha a
megszakítás annak az uniónak lenne az ága, a `stepRun` mező MINDEN végrehajtó kimenetén
elveszítené a "mindig van" garanciáját, és a hívóknak olyan helyeken is szűkíteniük kellene, ahol
az ág típusilag sosem áll elő. A szélesebb típus ezért csak három helyen jelenik meg: a
`human_approval` végrehajtójánál, a diszpécsernél (`execute-node.ts`) és a léptető huroknál.

**A megszakított `human_approval` lépés sorát nem a végrehajtó zárja le.** Az `interrupted` ágon a
`execute-human-approval.ts` egyetlen állapotváltást és egyetlen eseményt sem ír. Ez nem elhagyás:
a hívók KÜLÖNBÖZŐ záró állapotot írnak ugyanarra a sorra - a felhasználói megszakítás és a
`fail_run` `cancelled`-et, a szabályos leállás `interrupted`-et (SPEC-004 10.2 3. pont, 8.3) -,
amit a végrehajtó nem tudna eldönteni, és egy saját írás a lezárást kérő fél írásával versenyezne
(ugyanaz az elv, mint az `ActiveRunHandle.requestStop` doksijában a futás záró állapotára).
Mindhárom hívó ugyanazt a függvényt hívja, a célállapotra paraméterezve
(`closeWaitingApprovalStepRuns`), a várakozás lezárásával egy szinkron menetben, hogy a leállási
ablakban érkező döntés a sor állapotán bukjon (`illegal_status_transition`). A szabályos leállás
útján e nélkül a jel előtt fejléccel megkezdett, az ablakban befejezett döntési kérés HTTP 200-at
kapott, és a lépés `succeeded` lett egy `interrupted` futásban (mérve,
`docs/research/2026-09-23-megszakitas-leallas-meres.md` 7. szekció, nyolcadik kör).

**A T-005-29 `config-invariants.spec.ts` az `engine-port` témában áll, és nem 19. téma
mappában.** A SPEC-004 12. szekció (17. szekció 1. elfogadási kritériuma) a `packages/engine/src`
alatt PONTOSAN 18 téma mappát ír elő, tehát a T-005-29 nem hozhat létre újat - ugyanaz a döntés,
mint a `startup-recovery` témában álló `engine-never-opens-database.spec.ts`-nél (T-005-27, AC-53):
egy megvalósítás nélküli, csomagszintű invariánst egy MEGLÉVŐ, tartalmilag legközelebb álló témába
kell tenni. Az `engine-port` a legjobb illeszkedés, mert a kilenc befecskendezett port
(`clock`, `idGenerator`, `processEnvironment`, `providerDescriptorLookup`, ...) éppen az a
MECHANIZMUS, amitől a hat ellenőrzött invariáns fennáll: a motor azért nem hív `Date.now()`-t vagy
`randomUUID()`-t, nem olvas `process.env`-et és nem ismer konkrét provider azonosítót, mert minden
ilyen forrás egy porton keresztül érkezik (lásd `provider-descriptor-lookup-port.ts` "Miért port,
nem import" bekezdését). A `startup-recovery/engine-never-opens-database.spec.ts` saját doksija
szándékosan elhatárolja magát ettől a gyűjtőteszttől ("nem a jövőbeli T-005-29 ... gyűjtőtesztjében"),
ezért az `openDatabase` ellenőrzés a `config-invariants.spec.ts`-be nem került át, hogy ne
duplikálódjon - a hat ellenőrzés pontosan a PLAN-005 T-005-29 sorának elfogadási kritérium oszlopa
szerinti lista (provider azonosító, `@anthropic-ai`, `Date.now`/`setTimeout`/`randomUUID`/
`process.env`/`Math.random`, `total_cost_usd`, `retry-after`, `package.json` függőségek). A
próbafuttatás két, jogosan ott álló kommentet talált, ami a `Date.now` szót MAGYARÁZATKÉNT
tartalmazta (`engine-event/write-engine-event.ts`, `node-executor/begin-step-run.ts`, mindkettő
azt írta körül, hogy a motor miért NEM hívja natívan); mindkettő körülírásra került ("natív
rendszeróra hívás"), mert a szabály szerint a forrást javítjuk, nem a tesztet lazítjuk - kódban,
aktív hivatkozásként egyik tiltott szöveg sem volt jogos, csak a kommentek magyarázó szövege
tartalmazta szó szerint.
...
**A sorba állítás idempotens (T-005-30 javítás).** Egy példány egyszerre legfeljebb egyszer állhat
az érkezési sorban; az `advanceScheduler` belső `enqueue` segédje ezt kikényszeríti. Az adverzariális
átvizsgálás előtt nem így volt, és a hiba **az élek pillanatképbeli sorrendjétől** függött: egy
`branch` node halott és élő ága ugyanabba a lentebbi node-ba visszatalálkozva kétszer állította sorba
ugyanazt a példányt, ha a halott él állt elöl az él listában (a halott ág terjesztése a beágyazott
menetben tette futtathatóvá, majd a külső menet ugyanazt a példányt még egyszer kiértékelte). Két
következménye volt: két `step_run` sor keletkezett volna ugyanarra a példányra, és a `run-supervisor`
`inFlight` térképe (példány kulcs -> `Promise`) az egyik futó lépés eredményét el is veszítette
volna, tehát a futás lezárulhatott volna egy még futó lépés mellett. A sorrendfüggés a SPEC-004 4.4 2. pontjával is ütközött: a futtathatóság a jelöléseken áll, nem az élek felsorolásán. Regressziós
teszt: `advance-scheduler.spec.ts` "a halott ág terjesztése akkor sem futtatja kétszer a
visszatalálkozó node-ot, ha a halott él áll elöl".

**A nem végleges sorsú példány jelölése elhalasztódik (T-005-30 utáni javítás).** A SPEC-004 4.4 5. pontja a hibára futó példány nem menekülő éleire **azonnali** `dead` jelölést mond, a 8.2 5. pontja viszont azt, hogy sikeres újrapróbálkozás után a vezérlés a **megismételt** példány saját
kimenő élein megy tovább. A kettő együtt csak úgy áll meg, ha a jelölés megvárja, amíg a példány
sorsa véglegessé válik: enélkül egy több bejövő élű leszármazott a 4.4 2. pontja szerint (egyetlen
`live` jelölés elég) már a hibás lefutás jelölésével futtathatóvá vált, lefutott, majd a sikeres
újrapróbálkozás után MÁSODSZOR is lefutott, azonos `attempt` értékkel. **Ez nem ugyanaz, mint a
fenti idempotencia javítás**: az egyetlen `advanceScheduler` híváson belüli duplikálást oldotta
meg, ez viszont két, egymástól távoli hívás közötti állapot kérdése, tehát a sorbaállítás
idempotenciája nem fedte le.

A feloldás két ütemező esemény, mindkettő a `run-supervisor` `advance-run.ts` fájljából jön:

| Esemény                   | Mikor                                                                                                       | Mit tesz                                                                                           |
| ------------------------- | ----------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| `outgoing_marks_deferred` | kezelt hibájú példány (`escapeKey: 'on_error'`), illetve újabb kísérletet ütemező `error_handler`           | csak a `liveEdgeIds` élek kapnak `live` jelölést; a többi él és a `fan_out` kibontás bejegyzés vár |
| `deferred_marks_settled`  | a hibakontextust fogyasztó `error_handler` NEM `retry_scheduled` eredménnyel zárt (`settleDeferredFailure`) | a `keptEdgeIds` (a már `live` menekülő élek) kivételével minden kimenő él `dead` jelölést kap      |

A halasztás egyik ága sem marad örökre nyitva. Sikeres újrapróbálkozás után a megismételt példány
saját `node_completed` eseménye jelöl, és az `on_error` élére kerülő `dead` jelöléstől a kezelő
példány halottá válik, tehát a kezelő `exhausted` élét a halott ág terjesztése jelöli meg. Kimerült
kísérletek (vagy `unhandled_error_kind`) után a `deferred_marks_settled` teszi ki a `dead`
jelölést, különben a hibás ág leszármazottja soha nem indulna el, holott a 4.4 2. pontja szerint
futtatható. Regressziós tesztek: `advance-scheduler.spec.ts` két `outgoing_marks_deferred` /
`deferred_marks_settled` esete, és `advance-run.spec.ts` két végponttól végpontig futó
"REGRESSZIÓ" tesztje.
...
**A `concurrency-gate` téma öt tervezési döntése.** A SPEC-004 7. szekciója a szabályozó
...
- **Ismeretlen azonosító felszabadítása hibaág (`unknown_concurrency_slot`), nem csendes
  ...
  útvonalat. Ami **nem** hiba: a még sorban álló, helyet nem kapott kérés felszabadítása; ilyenkor a
  bejegyzés kiesik a sorból, és a visszahívása soha nem fut le. A motor ezt az ágat nem használja,
  mert a várakozó hívó visszahívás nélkül örökre várna: a megszakítás útja a
  `denyWaitingForRunIds` (lásd lent).
- **A lezárt szabályozó elutasító visszahívással jelez, nem hallgat** (SPEC-004 10.2 1. pont,
  2026-09-23). A `close` után minden sorban álló és minden új kérés az `onDenied` visszahívást
  kapja, szinkron. Egy csendben el nem bírált kérés a hívó `await`-jét örökre nyitva hagyná, tehát
  a leállás a futás `completion` Promise-ára várva elakadna. Az agent lépés életciklusa az
  elutasításra `interrupted` eredménnyel tér vissza, `markStepRunning` és provider hívás nélkül.
  Mérve: a lezárás nélkül a leállás alatt felszabaduló helyet a sorban álló lépés megkapta, és
  `interrupt()` nélkül végigfutott (SPEC-006 8.2).
- **A megszakított futás sorban álló kérései ugyanígy elutasítást kapnak** (SPEC-004 9. szekció 2.
  pont, 2026-09-23). A `requestSlot` ezért a kérő futás `runId`-ját is átveszi, és a
  `denyWaitingForRunIds` a megnevezett futások minden várakozóját érkezési sorrendben kiveszi és
  `onDenied` visszahívással zárja; a foglalt helyek és más futások várakozói érintetlenek. A
  lezárástól eltérően ez nem végleges: a futás léptető hurka a `requestStop()` után úgysem kér
  újabb helyet. Mérve a valódi szerveren: e nélkül egy három párhuzamos agent lépéses futás
  megszakítása (korlát 1) után a két sorban álló lépés `interrupt()` nélkül végigfutott, három
  agent hívással, és a megszakító kérés 17043 ms-ig nem válaszolt; vele egy hívás, 1012 ms
  (`docs/research/2026-09-23-megszakitas-leallas-meres.md` 7. szekció). Ugyanezt a műveletet
  hívja a `fail_run` politika a bukott futásra (SPEC-004 8.3, lásd "Szabályok" eleje): a
  szabályozónak nincs külön `fail_run` ága.
...
1. **A `failed` kimenet után** a `resolveErrorRoute`-ot kell hívni a node configjának
   ...
   `handled` ág `liveEdgeIds` halmaza a `SchedulingEvent` `liveEdgeIds` mezője - `escapeKey:
'on_error'` esetén az `outgoing_marks_deferred`, minden más esetben a `node_completed` eseményé,
   lásd "Szabályok"; a `fail_branch` ághoz **üres** `liveEdgeIds` tartozik (minden kimenő él
...
`requestStop()`: a hurok új példányt nem indít. A második felét a `requestStop()` **nem** fedi le,
mert a már elindított, helyre váró példányt nem éri el; ezt a korábbi állítást a mérés cáfolta
(2026-09-23: a sorban álló lépések a megszakítás után `interrupt()` nélkül végigfutottak). A
sorból való kivételt a `stopAndAwaitRunTree` 2. pontja végzi (`ConcurrencyGate.denyWaitingForRunIds`).
A 3. pontja ("minden élő agent lépéshez tartozó `AgentQuery` objektumon
...
végigfut. Ez megnevezett hiány volt; a futó testvér megszakítását a T-005-31, a sorban álló
testvérek kivételét és lezárását a 2026-09-23-i javítás pótolta (lásd "Szabályok" eleje).
...
- **A lezáró eseményt a megszakító fél adja ki élőben, nem a léptető hurok** (2026-09-23). A
  `cancelRunTree` és a `recoverInterruptedRuns` a `run_finished`/`run_interrupted` sort a saját
  tranzakciójában írja, a `db` réteg viszont nem publikál, a léptető hurok pedig a `stopRequested`
  ágon szándékosan sem ír, sem kiad (`advance-run.ts` `finishRun`). Az `interruptRun` és a
  `shutdownActiveRuns` ezért a sikeres DB zárás UTÁN, a `db` által visszaadott azonosító listán
  (`cancelledRunIds`, `recoveredRunIds`) hívja az `eventPublisher.publish`-t, `writeEngineEvent`
  nélkül, ugyanaz a minta, mint a `run_started` kiadása a `run-supervisor`-ban. Enélkül a kliens a
  megszakítás után sosem kapta meg a lezáró keretet (mérve, SPEC-006 M-35). A szabályos leállásnál
  a kiadott keret csak akkor éri el a klienst, ha az SSE kapcsolat a `shutdown` alatt még nyitva
  van: ez a szerver leállási sorrendjének dolga (SPEC-006 8.2).
- **Ismert, ebben a témában szándékosan lefedetlen két él.** (1) Egy `human_approval` lépésnek
  ...
  párhuzamossági helyre váró, még el sem indult lépésnek nincs `AgentQuery`-je, tehát az
  `interrupt()` nem éri el; a várakozó Promise-t csak a szabályozó elutasító visszahívása oldja
  fel (a `releaseSlot` sorból való eltávolítása nem jelez a várakozónak). Korlátlan (`null`)
  párhuzamossági korláttal ez nem exponálódik. 2026-09-23 óta ez a szabályos leállásra (a
  `close` elutasítással oldja fel a várakozókat) és a felhasználói `interruptRun` útra is megoldott
  (a `stopAndAwaitRunTree` a `denyWaitingForRunIds` elutasításával), ugyanígy a `fail_run` ágra
  is, lásd a `concurrency-gate` téma döntéseit és a "Szabályok" elejét. Az (1) megnevezett hiány,
  nem elfogadott végállapot.
