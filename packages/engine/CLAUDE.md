...
- egy testvér agent lépés, ami a hívás pillanatában még **párhuzamossági helyre vár**, tehát az
  `AgentQuery`-je még nincs a regiszterben. Ez nem a `fail_run` sajátja: a T-005-26 óta a KÜLSŐ
  megszakítás is pontosan így viselkedik (`interrupt-live-agent-queries.ts` zárómondata).
...
- **Ismert, ebben a témában szándékosan lefedetlen két él.** (1) Egy `human_approval` lépésnek
  ...
  (`null`) párhuzamossági korláttal nem exponálódik. A szabályos leállásra ez 2026-09-23 óta
  megoldott (a `close` elutasítással oldja fel a várakozókat, lásd a `concurrency-gate` téma
  döntéseit), a felhasználói `interruptRun` útra viszont nem: ott a sorban álló lépés a
  felszabaduló helyet megkaphatja. Mindkettő megnevezett hiány, nem elfogadott végállapot.
