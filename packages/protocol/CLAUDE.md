...
| `src/protocol-error/` | a `ProtocolErrorCode` szótár, a `ProtocolErrorBody` séma, a HTTP státusz leképezés, a Zod hiba lista fordítása a boríték alakra, és a csomagszintű greppes invariáns teszt: nincs Authorization/süti/token szöveg, nincs `.parse(`, nincs `.default(`/`.transform(`     |
