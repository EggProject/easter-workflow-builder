...
| `src/protocol-error/` | a `ProtocolErrorCode` szótár, a `ProtocolErrorClass` zárt hibaosztály szótár (a REST hiba törzs elhagyható `errorClass` mezője, SPEC-005 8.5), a `ProtocolErrorBody` séma, a HTTP státusz leképezés, a Zod hiba lista fordítása a boríték alakra, és a csomagszintű greppes invariáns teszt: nincs Authorization/süti/token szöveg, nincs `.parse(`, nincs `.default(`/`.transform(` |
...
| `src/run/`            | a futás és a lépés futás állapot felsorolása, a futás rekord, az indítás, a listázás, a megszakítás, az újraindítás, a pillanatkép alakja, és a megszakítás oka                                                                                                                                                                                                                      |
...
regressziós tesztje adja. **Ugyanígy a `protocol-error` téma `ProtocolErrorClass` szótára**
(SPEC-005 8.5): a motor `EngineErrorKind` uniójának szándékos részhalmaza, plusz a `db`
`already_decided` ága, amit az L1 réteg nem importálhat; a sodródás védelmét az `apps/server`
`error-class-drift-protection` regressziós tesztje adja. Az `errorClass` mező elhagyható (nem
`null`), és csak a REST hiba törzsben áll, a `protocol_error` SSE keretben nem (SPEC-005 8.1).
