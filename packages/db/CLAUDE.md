...
| `workflow-graph/node-config/`       | a node config unió és mind a tíz ága, az `isNodeConfig` guard, a `script-config.ts` alobjektum, plusz az `is-string-array.ts` segéd (T-003-11)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
`node-config.ts` uniót (mind a tíz ág), az `is-node-config.ts` guardot, a `script-config.ts`
alobjektumot és az `is-string-array.ts` segédet, amit a téma több guardja használ; az
`agent-step-config/` az `agent-step-config.ts` típust, az `is-agent-step-config.ts` guardot és
a mezőalakjait (`storable-mcp-server.ts`, `sandbox-config.ts`, `engine-hook-id.ts`,
`session-mode.ts`). Egyik fájl sem kerül a barrelbe: a publikus felületet a repository réteg
adja (T-003-12).
