...
describe('isNil', () => {
  ...
  it('should work as type guard to narrow null/undefined', () => {
    // Fuggvenyhivason at kapjuk az erteket, hogy a linter statikus
    // ertekkovetese ne tudja "mindig igaz" allitaskent felismerni az `expect`
    // lentebbi, ertekre vonatkozo osszehasonlitasat (sonarjs/no-trivial-assertions).
    const value: string | undefined = getTestStringValue();
    if (!isNil(value)) {
  ...
  it('should work with filter to remove nil values', () => {
    const values = [POSITIVE_INT_SMALL, undefined, POSITIVE_INT_SMALL, undefined, POSITIVE_INT_SMALL, undefined];
