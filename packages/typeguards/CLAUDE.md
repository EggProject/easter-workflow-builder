...
| `src/is-string-array/is-string-array.ts` (+ `.spec.ts`)                                                                                                                                                                    | `isStringArray`, `readonly string[]` szűkítése; a `packages/db` `workflow-graph` tárgyköréből költözött ide, mert nincs domain témája (PLAN-004 4.5 szekció) |
